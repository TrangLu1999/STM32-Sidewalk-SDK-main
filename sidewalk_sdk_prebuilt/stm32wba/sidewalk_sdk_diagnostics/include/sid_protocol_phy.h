 ...
 *  @param    period in micro seconds the phy should be in sleep.
 *
 *  @return   On success RADIO_ERROR_NONE, on error a negative number is returned
 */
int32_t sid_phy_sleep(uint32_t sleep_us);
...
int32_t sid_phy_start_cad(sid_pal_radio_cad_param_exit_mode_t cad_exit_mode,
                          const sid_pal_radio_packet_cfg_t *rx_packet_cfg);
...
/** @brief    Configure radio to transmit a continuous preamble wave.
 *
 *  @return   On success RADIO_ERROR_NONE, on error a negative number is returned
 */
int32_t sid_phy_set_tx_continuous_preamble(void);
 ...
 *  @return   rssi in dBm
 */
int16_t sid_phy_radio_rssi(void);
