...
// packet parameters for fsk default mode
#define SID_RADIO_FSKDEF_CRC_SEED 0xFFFF
#define SID_RADIO_FSKDEF_CRC_POLYNOMIAL 0x8005
#define SID_RADIO_FSKDEF_CRC_TYPE SID_PAL_RADIO_FSK_CRC_2_BYTES
#define SID_RADIO_FSKDEF_SYNC_WORD_LENGTH 4
#define RADIO_FSK_BR_100KBPS                    100000
#define RADIO_FSK_BR_50KBPS                     50000
#define RADIO_FSK_BR_150KBPS                    150000
#define RADIO_FSK_BR_250KBPS                    250000

#define RADIO_FSK_FDEV_190KHZ                   190000
...
//transactions.
#define FSK_CAD_TRANS_TIME_US 800

// FSK tx rx initiate overhead by the modem. This is an approximate value for Ti and Semtech chipsets
#define SID_FSK_OVERHEAD_US MSEC_TO_USEC(1)

enum sid_fsk_phy_cfg_index {
    SID_FSK_CFG_SIDEWLAK,
    SID_FSK_CFG_DFLT,
    SID_FSK_CFG_RAW,
    SID_FSK_CFG_MAX,
    SID_FSK_CFG_INVALID = SID_FSK_CFG_MAX,
};
 ...
 */
uint32_t rnet_phy_get_fsk_tx_process_delay(void);

/**
 * @brief Get the time of FSK Rx process delay
 *
 * @return a positive number indicating the delay in micro second
 */
uint32_t rnet_phy_get_fsk_rx_process_delay(void);
...
/** @brief    Configure radio to transmit a continuous preamble wave in fsk mode.
 *
 *  @return   On success RADIO_ERROR_NONE, on error a negative number is returned
 */
int32_t rnet_phy_set_fsk_tx_continuous_preamble(void);
...
/** @brief sets the header type parameters for phy operation
 *
 *  This API sets the phy cfg index in phy module
 *  This API needs to be called before tx and rx in FSK mode.
 *
 *  @param[in] index of the phy cfg to be used
 *  @retval  RADIO_ERROR_NONE is returned
 */
int32_t rnet_phy_set_header_type(enum sid_fsk_phy_cfg_index cfg_index);

/** @brief set phy header for fsk custom header packets
 *
 *  This API is used to set the phy header for custom header
 *
 *  @param[in] phy_hdr phy header bytes
 *  @param[in] phy_hdr_len length of phy header
 *  @retval None
 */
void rnet_phy_set_fsk_phy_hdr(const uint8_t *phy_hdr, uint8_t phy_hdr_len);
