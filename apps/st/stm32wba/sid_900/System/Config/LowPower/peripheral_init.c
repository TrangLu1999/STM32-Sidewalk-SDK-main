...
#  if (CFG_LED_SUPPORTED != 0)
    /* Restore LED states */
    led_indication_exit_standby();
#  endif /* (CFG_LED_SUPPORTED != 0) */
    sid_pal_serial_bus_ext_ifc_notify_standby_exit();
    sid_pal_radio_notify_host_standby_exit();
    MX_APPE_StandbyExit();
...
{
#  if (CFG_LED_SUPPORTED != 0)
    /* Backup LED state and turn off the LEDs before entering Standby mode */
    led_indication_enter_standby();
#  endif /* (CFG_LED_SUPPORTED != 0) */
}
#endif /* (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED != 0) */
