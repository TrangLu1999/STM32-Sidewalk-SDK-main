...
static inline void _flush_log_buffer(void)
{
    ...
    while ((UTIL_ADV_TRACE_IsBufferEmpty() == 0u) && ((HAL_GetTick() - start_timestamp) < EXHA_LOGS_PRINTOUT_TIMEOUT_MS))
    {
        ...
        while ((HAL_NVIC_GetPendingIRQ(LOG_UART_HANDLER_TX_DMA_IRQn) == 0u)
            && (HAL_NVIC_GetPendingIRQ(LOG_UART_HANDLER_IRQn) == 0u)
        ...
        if (HAL_NVIC_GetPendingIRQ(LOG_UART_HANDLER_TX_DMA_IRQn) != 0u)
        {
            _call_irq_handler_by_number(LOG_UART_HANDLER_TX_DMA_IRQn);
        }

        /* Artificially call UART ISR if a UART IRQ is pending */
        if (HAL_NVIC_GetPendingIRQ(LOG_UART_HANDLER_IRQn) != 0u)
        {
            _call_irq_handler_by_number(LOG_UART_HANDLER_IRQn);
        }
    }
}
#endif /* SID_PAL_LOG_ENABLED */
...
#warning "Hard Fault handler code is not implemented for this MCU."
     ...
     */
#if defined(DEBUG) && (SID_PAL_LOG_ENABLED != 0)
...
#endif /* defined(DEBUG) && (SID_PAL_LOG_ENABLED != 0) */
