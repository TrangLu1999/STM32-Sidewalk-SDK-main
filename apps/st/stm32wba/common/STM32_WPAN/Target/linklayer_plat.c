...
{
  /*vvv PATCH STMC-485 vvvvvvvvvv*/
  sid_pal_delay_us(delay);
  /*^^^ END OF PATCH STMC-485 ^^^*/
  /*vvv PATCH STMC-485 vvvvvvvvvv*/
  /**
   * @details using DWT cycle counter is a bad practice because DWT is a debug peripheral. Its purpose
   * is mainly code profiling. DWT is disabled by default and requires writing to the TRCENA bit of the
   * DCB->DEMCR register, which is typically performed by a debugger when the target is attached. Messing
   * with debug peripherals in the application code may lead to undesired and catastrophic outcomes, as
   * the same registers are silently accessed by the debug probe. This means the system behavior may
   * drastically differ depending on the debug probe being attached. Also, using the DWT cycle counter
   * here destroys the capabilities for the code profiling in entire application.
   */
...
{
  /*vvv PATCH STMC-485 vvvvvvvvvv*/
  if (FALSE == ll_sys_is_initialized())
  {
    return;
  }
  /*^^^ END OF PATCH STMC-485 ^^^*/
...
void LINKLAYER_PLAT_NotifyWFIExit(void)
{
  ...
  /*vvv PATCH STMC-485 vvvvvvvvvv*/
  // if (AHB5_SwitchedOff)
  if ((ll_sys_is_initialized() != FALSE) && (AHB5_SwitchedOff))
  /*^^^ END OF PATCH STMC-485 ^^^*/
