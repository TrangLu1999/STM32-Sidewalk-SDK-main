...
static uint32_t backup_MSPLIM;
static uint32_t backup_PSP;
static uint32_t backup_PSPLIM;
static uint32_t backup_CONTROL;
static uint32_t backup_prio_RTC_IRQn;
static uint32_t backup_prio_SysTick_IRQn;
static uint32_t backup_prio_SVCall_IRQn;
static uint32_t backup_prio_PendSV_IRQn;

static uint32_t backup_clock_source_SysTick;
static uint32_t backup_ahb_prescaler;
static uint32_t backup_ahb5_divider;
static uint32_t backup_ahb5_prescaler;
static uint32_t backup_apb1_prescaler;
static uint32_t backup_apb2_prescaler;
static uint32_t backup_apb7_prescaler;
...
/* Application Install Manager (bootloader) Interface */
uint32_t rcc_reset_flags;
...
__WEAK SID_STM32_SPEED_OPTIMIZED void Standby_Restore_GPIO(void)
{
  uint32_t temp;
...
#if (CFG_SCM_SUPPORTED == 0)
SID_STM32_SPEED_OPTIMIZED static void Clock_Switching(void)
{
  /* Activate HSE clock */
  LL_RCC_HSE_Enable();
...
SID_STM32_SPEED_OPTIMIZED static void Enter_Stop_Standby_Mode(void)
{
  /* Disabling ICACHE */
  LL_ICACHE_Disable();
  ...
  /* Stop HAL tick to avoid tick timer IRQ waking up the system */
  HAL_SuspendTick();
...
SID_STM32_SPEED_OPTIMIZED static void Exit_Stop_Standby_Mode(void)
{
  LL_ICACHE_Enable();
  ...
  if (LL_PWR_IsActiveFlag_STOP() == 1U)
  {
    ...
    if ((isRadioActive () == SCM_RADIO_NOT_ACTIVE) || (LL_RCC_HSE_IsReady() == 0u))
    {
  ...
  {
    /* Apply wait states for HSE32 configuration */
    __HAL_FLASH_SET_LATENCY(FLASH_LATENCY_0);
    while(__HAL_FLASH_GET_LATENCY() != FLASH_LATENCY_0);
    HAL_RAMCFG_ConfigWaitState(&sram1_ns, RAMCFG_WAITSTATE_0);
    HAL_RAMCFG_ConfigWaitState(&sram2_ns, RAMCFG_WAITSTATE_0);
  ...
  /* Resume HAL tick after returning back to RUN mode */
  HAL_ResumeTick();
}

/*----------------------------------------------------------------------------*/

SID_STM32_SPEED_OPTIMIZED void PWR_EnterOffMode( void )
{
#if (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED != 0)
  ...
  /* Save selected CPU peripheral registers */
  backup_PSP               = __get_PSP();
  backup_PSPLIM            = __get_PSPLIM();
  backup_MSPLIM            = __get_MSPLIM();
  backup_prio_RTC_IRQn     = NVIC_GetPriority(RTC_IRQn);
  ...
  /* Backup HCLK configuration */
  backup_clock_source_SysTick = HAL_SYSTICK_GetCLKSourceConfig();
  backup_ahb_prescaler        = LL_RCC_GetAHBPrescaler();
  backup_ahb5_divider         = LL_RCC_GetAHB5Divider();
  backup_ahb5_prescaler       = LL_RCC_GetAHB5Prescaler();
  backup_apb1_prescaler       = LL_RCC_GetAPB1Prescaler();
  backup_apb2_prescaler       = LL_RCC_GetAPB2Prescaler();
  backup_apb7_prescaler       = LL_RCC_GetAPB7Prescaler();

  /* Call app-specific peripheral backup method */
  MX_StandbyEntry_PeripheralBackup();

  /* Perform pre-entry preparations as a last step. Enter_Stop_Standby_Mode(0 changes wait states so calling before all backups are made is not desirable */
...
SID_STM32_SPEED_OPTIMIZED void PWR_ExitOffMode( void )
{
#if (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED != 0)
  ...
  if ( 1UL == boot_after_standby )
  {
    ...
    /* Restore settings of the essential IRQs */
    ...
    HAL_NVIC_SetPriority(RTC_IRQn, backup_prio_RTC_IRQn, 0);
    ...
    /* Restore HCLK configuration */
    LL_RCC_SetAHBPrescaler(backup_ahb_prescaler);
    LL_RCC_SetAHB5Divider(backup_ahb5_divider);
    LL_RCC_SetAHB5Prescaler(backup_ahb5_prescaler);
    LL_RCC_SetAPB1Prescaler(backup_apb1_prescaler);
    LL_RCC_SetAPB2Prescaler(backup_apb2_prescaler);
    LL_RCC_SetAPB7Prescaler(backup_apb7_prescaler);
  ...
  __HAL_FLASH_PREFETCH_BUFFER_ENABLE();
...
#endif /* CFG_LPM_WAKEUP_TIME_PROFILING */
    {
      /* Restore SysTick clock source */
      HAL_SYSTICK_CLKSourceConfig(backup_clock_source_SysTick);
    }

    /* Initialize HAL Tick since SCM and peripheral re-init may use it */
    ...
    /* Call app-specific peripheral store function. Important note: this shall happen after the system clock is restored to ensure correct operation (e.g., UART kernel clock may depend on APB clock speed) */
...
#endif /* GPIOC */
#ifdef GPIOD
    HAL_PWREx_DisableStandbyRetainedIOState(PWR_GPIO_D, PWR_GPIO_PIN_MASK);
#endif /* GPIOD */
#ifdef GPIOE
    HAL_PWREx_DisableStandbyRetainedIOState(PWR_GPIO_E, PWR_GPIO_PIN_MASK);
#endif /* GPIOE */
    /* GPIOF is currently not available on STM32WBA platform */
#ifdef GPIOG
    HAL_PWREx_DisableStandbyRetainedIOState(PWR_GPIO_G, PWR_GPIO_PIN_MASK);
#endif /* GPIOG */
#ifdef GPIOH
...
SID_STM32_SPEED_OPTIMIZED void PWR_EnterStopMode( void )
{
  ...
  /* Indicate system is going into LPM */
  SIDEWALK_RF_TIMINGS_WBA_AWAKE_GPIO_Port->BRR = SIDEWALK_RF_TIMINGS_WBA_AWAKE_Pin;
...
SID_STM32_SPEED_OPTIMIZED void PWR_ExitStopMode( void )
{
  SYSTEM_DEBUG_SIGNAL_SET(LOW_POWER_STOP_MODE_EXIT);
#if SID_SDK_CONFIG_RF_TIMINGS_MEASUREMENT
  /* Indicate system is up and running */
  SIDEWALK_RF_TIMINGS_WBA_AWAKE_GPIO_Port->BSRR = SIDEWALK_RF_TIMINGS_WBA_AWAKE_Pin;
#endif /* SID_SDK_CONFIG_RF_TIMINGS_MEASUREMENT */
...
SID_STM32_SPEED_OPTIMIZED void PWR_EnterSleepMode( void )
{
...
SID_STM32_SPEED_OPTIMIZED void PWR_ExitSleepMode( void )
{
...
SID_STM32_SPEED_OPTIMIZED void PWR_EnableSleepMode(void)
{
...
SID_STM32_SPEED_OPTIMIZED void PWR_DisableSleepMode(void)
{
  UTILS_ENTER_CRITICAL_SECTION();
  sleep_mode_disabled++;
  UTILS_EXIT_CRITICAL_SECTION();
...
SID_STM32_SPEED_OPTIMIZED uint32_t is_boot_from_standby(void)
{
#if (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED != 0)
  ...
  if( (LL_PWR_IsActiveFlag_SB() == 1UL ) &&
      (rcc_reset_flags == 0U) /* Check stored copy of RCC_CSR to keep compatibility with Application Install Manager (bootloader) */
    )
  {
    if ((LL_PWR_GetSRAM1SBRetention() == 0u) && (LL_PWR_GetSRAM2SBRetention() == 0u))
    {
      /**
        * @attention If SRAM retention is disabled completely a full reset sequence shall be executed even if the MCU
        *            is waking up from the Standby state. The reason is that no retention means there are no chances
        *            the context and peripheral config are preserved, meaning full-scale MCU reset (including all the
        *            peripherals) and re-initialization is a must
        */
