...
/* UART2 (Sensor/G0B1) RX ring buffer */
#ifdef SENSOR_UART_HANDLER
static char     uart2_rx_buf[UART_IO_RX_BUFER_SIZE];
static char *   uart2_rx_buf_read_ptr = &uart2_rx_buf[0];
static uint32_t uart2_rx_buf_write_wrap_flag;
#endif /* SENSOR_UART_HANDLER */
...
#ifdef SENSOR_UART_HANDLER
static void     uart_io_uart2_rx_buffer_wrap(UART_HandleTypeDef *huart);
static uint32_t uart_io_uart2_rx_buffer_has_data(void);
#endif /* SENSOR_UART_HANDLER */
...
#ifdef SENSOR_UART_HANDLER
SID_STM32_SPEED_OPTIMIZED static void uart_io_uart2_rx_buffer_wrap(UART_HandleTypeDef *huart)
{
    (void)huart;

    UTILS_ENTER_CRITICAL_SECTION();
    uart2_rx_buf_write_wrap_flag = TRUE;
    UTILS_EXIT_CRITICAL_SECTION();
}

SID_STM32_SPEED_OPTIMIZED static uint32_t uart_io_uart2_rx_buffer_has_data(void)
{
    UTILS_ENTER_CRITICAL_SECTION();
    register uint32_t has_data = FALSE;
    register uint32_t dma_write_ptr = SENSOR_UART_HANDLER.hdmarx->Instance->CDAR;
    __COMPILER_BARRIER();
    if ((dma_write_ptr > (uint32_t)(void *)uart2_rx_buf_read_ptr) || (uart2_rx_buf_write_wrap_flag != FALSE))
    {
        has_data = TRUE;
    }
    UTILS_EXIT_CRITICAL_SECTION();
...
static int uart_io_getchar_from_uart2(void)
{
    int ret = EOF;
    ...
    do
    {
        if (__HAL_DMA_GET_FLAG(SENSOR_UART_HANDLER.hdmarx, DMA_FLAG_TC) != 0U)
        {
            break;
        }
        ...
        {
            /* Buffer overflow */
            uart2_rx_buf_read_ptr = &uart2_rx_buf[0];
            uart2_rx_buf_write_wrap_flag = FALSE;
            break;
        }

        if ((dma_write_ptr > (uint32_t)(void *)uart2_rx_buf_read_ptr) || (uart2_rx_buf_write_wrap_flag != FALSE))
        {
            ret = (int)(*uart2_rx_buf_read_ptr);

            uart2_rx_buf_read_ptr++;
            if (uart2_rx_buf_read_ptr >= (uart2_rx_buf + SID_STM32_UTIL_ARRAY_SIZE(uart2_rx_buf)))
            {
                uart2_rx_buf_read_ptr = &uart2_rx_buf[0];
                uart2_rx_buf_write_wrap_flag = FALSE;
            }
...
#ifdef SENSOR_UART_HANDLER
void uart_io_uart2_init(void)
{
    HAL_StatusTypeDef status;

    if (NULL == SENSOR_UART_HANDLER.hdmarx)
    {
        SID_PAL_LOG_ERROR("No DMA channel assigned for UART2 Rx");
        Error_Handler();
    }

    /* Configure DMA ring buffer for continuous Rx on UART2 */
    SENSOR_UART_HANDLER.RxCpltCallback = uart_io_uart2_rx_buffer_wrap;
    uart2_rx_buf_write_wrap_flag = FALSE;

    status = HAL_UART_Receive_DMA(&SENSOR_UART_HANDLER, (uint8_t *)uart2_rx_buf, sizeof(uart2_rx_buf));
    if (status != HAL_OK)
    {
        SID_PAL_LOG_ERROR("Failed to start UART2 Rx. HAL error 0x%02X", status);
        Error_Handler();
    }

    SID_PAL_LOG_INFO("Sensor UART2 initialized (G0B1 input)");
}
#endif /* SENSOR_UART_HANDLER */
...
SID_STM32_SPEED_OPTIMIZED int getchar(void)
{
    ...
    if (SID_STM32_UTIL_IS_IRQ() == FALSE)
    {
         ...
         * Now also checks UART2 (sensor/G0B1) buffer for data.
         */
        while ((uart_io_rx_buffer_has_data() == FALSE)
#ifdef SENSOR_UART_HANDLER
               && (uart_io_uart2_rx_buffer_has_data() == FALSE)
#endif
               && (accumulated_ticks < timeout_ticks))
        {
...
#ifdef SENSOR_UART_HANDLER
    /* If no data from USART1 (terminal), try USART2 (G0B1 sensor) */
    if (ret == EOF)
    {
        ret = uart_io_getchar_from_uart2();
    }
#endif /* SENSOR_UART_HANDLER */
