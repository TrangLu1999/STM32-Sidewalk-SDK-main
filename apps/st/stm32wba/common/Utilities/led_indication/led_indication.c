...
static osMessageQueueId_t indication_queue           = NULL;
static osThreadId_t       indication_thread          = NULL;
static uint8_t            led_indication_initialized = FALSE;

#if (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED == 1)
static uint8_t backup_led_states;
...
sid_error_t led_indication_init(void)
{
    ...
    do
    {
        if (led_indication_initialized != FALSE)
        {
        ...
        led_indication_initialized = TRUE;
    ...
    {
        if (indication_thread != NULL)
        {
            (void)osThreadTerminate(indication_thread);
            indication_thread = NULL;
        }

        if (indication_queue != NULL)
        {
            (void)osMessageQueueDelete(indication_queue);
            indication_queue = NULL;
        }

        led_indication_initialized = FALSE;
    }
...
#if (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED == 1)
SID_STM32_SPEED_OPTIMIZED void led_indication_enter_standby(void)
{
    if (led_indication_initialized != FALSE)
    {
        /* Backup current LED states. Note: LED_BLUE is not handled by this module */
        backup_led_states = (BSP_LED_GetState(LED_GREEN) << LED_GREEN)
                          | (BSP_LED_GetState(LED_RED)   << LED_RED);

        /**
         * Do not turn off LEds here. If Standby is entered, GPIO pins will be tri-stated and LEds will go off.
         * If Stop or Sleep mode is actually entered (e.g., due to pending IRQ), the LED state will be preserved
         * and led_indication_exit_standby() won't be called
         */
    }
}
#endif /* (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED == 1) */
...
#if (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED == 1)
SID_STM32_SPEED_OPTIMIZED void led_indication_exit_standby(void)
{
    if (led_indication_initialized != FALSE)
    {
        /* Restore LED GPIO config after leaving Standby mode. Note: LED_BLUE is not handled by this module */
        BSP_LED_Init(LED_GREEN);
        BSP_LED_Init(LED_RED);

        /* Restore last known LED states */
        (backup_led_states & (1u << LED_GREEN)) != 0u ? BSP_LED_On(LED_GREEN) : BSP_LED_Off(LED_GREEN);
        (backup_led_states & (1u << LED_RED  )) != 0u ? BSP_LED_On(LED_RED)   : BSP_LED_Off(LED_RED);
    }
}
#endif /* (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED == 1) */
