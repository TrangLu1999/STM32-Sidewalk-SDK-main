...
#define DEMO_MESSAGE_DELAY_MS        (2u * 60000u) /* Note: setting this to 60 seconds or less prevents the system from entering Standby LPM because a new uplink round starts before BLE link is switched to the slow advertisement mode */
        ...
        if (osMessageQueueGet (app_context->event_queue, &event, NULL, osWaitForever) == osOK)
        {
            switch (event)
            {
                case EVENT_TYPE_START_LINK:
                    /* Starting Sidewalk-over-BLE link */
