...
```
    ...
    ble set_tx_power <dBm>                      - set BLE Tx power level (EIRP)
                                                    Accepted range is [-20..+10]dBm if not external PA is used. Parameter value for
                                                    this command specifies the Effective Isotropic Radiated Power (EIRP) level, meaning
                                                    you should specify what you expect to be radiated from antenna. Any required
                                                    adjustments (like accounting for antenna gain, Tx path loss or external PA gain)
                                                    are addressed automatically by the driver.
