/**
  ******************************************************************************
  * @file    app_sidewalk.c
  * @brief   Demo of Sidewalk and BLE coexistence capabilities of STM32WBA5x
  ******************************************************************************
  * @attention
  *
...
#define SID_MAIN_THREAD_PWR_MEAS_UNLOCKED_FLAG (1u << 0)
 ...
 || (defined(SID_RADIO_PLATFORM_SX126X) && (SX126X_RADIO_CFG_USE_STATUS_LED != 0u)) \
 || (defined(SID_RADIO_PLATFORM_S2_LP) && (S2LP_RADIO_CFG_USE_STATUS_LED != 0u)) \
 || (defined(SID_RADIO_PLATFORM_STM32WLXX_APP) && (STM32WLxx_RADIO_CFG_USE_STATUS_LED != 0u))
#  define RADIO_CFG_USE_STATUS_LED
#endif /* SID_RADIO_PLATFORM_LR11XX || SID_RADIO_PLATFORM_SX126X || SID_RADIO_PLATFORM_S2_LP || SID_RADIO_PLATFORM_STM32WLXX_APP */
...
#  if defined(SID_RADIO_PLATFORM_S2_LP) && (S2LP_RADIO_CFG_USE_STATUS_LED != 0u)
    const s2_lp_radio_device_config_t * const radio_cfg = get_radio_cfg();
#  endif /* SID_RADIO_PLATFORM_S2_LP */
...
void SID_APP_Init(void)
{
    ...
    ret_code = sid_ble_coexistence_cli_init();
    if (ret_code != SID_ERROR_NONE)
    {
        SID_PAL_LOG_ERROR("Sidewalk BLE coexistence cli init failed err: %d", ret_code);
        SID_PAL_ASSERT(0);
    }
