...
#define SID_STM32_BLE_COEXIST_CLI_FACTORY_RESET_H       "Erase all BLE data (bonding, config parameters, etc.)"
#define SID_STM32_BLE_COEXIST_CLI_SET_TX_POWER_H        "Set BLE Tx power (EIRP)"
...
#define SID_STM32_BLE_COEXIST_CLI_SET_TX_POWER_CMD      SID_STM32_BLE_COEXIST_CLI_ROOT" set_tx_power"
...
static ace_status_t sid_ble_coexist_cli_set_tx_power_cmd(int32_t argc, const char **argv);
...
SID_CLI_REGISTER_COMMAND(m_sub_ble)
{
    ...
    SID_CLI_DEFINE_COMMAND(        set_tx_power,  NULL, SID_STM32_BLE_COEXIST_CLI_SET_TX_POWER_H,  sid_ble_coexist_cli_set_tx_power_cmd),
...
static ace_status_t sid_ble_coexist_cli_set_tx_power_cmd(int32_t argc, const char **argv)
{
    ...
    do
    {
        int32_t desired_tx_power;

        if (argc != 1)
        {
            CLI_LOG_ERROR(SYNTAX_ERR);
            CLI_LOG_WARNING(SID_STM32_BLE_COEXIST_CLI_SET_TX_POWER_CMD " takes exactly one argument");
            sid_err = SID_ERROR_INVALID_ARGS;
            break;
        }
        else
        {
            desired_tx_power = sid_ble_coexist_cli_utils_parse_input_num(argv[0]);
        }
        ...
        if ((desired_tx_power < INT8_MIN) || (desired_tx_power > INT8_MAX))
        {
            CLI_LOG_ERROR("Requested Tx power value is out of range");
            sid_err = SID_ERROR_PARAM_OUT_OF_RANGE;
            break;
        }

        sid_err = ble_ext_ifc->set_tx_power((int8_t)desired_tx_power);
    } while (0);
