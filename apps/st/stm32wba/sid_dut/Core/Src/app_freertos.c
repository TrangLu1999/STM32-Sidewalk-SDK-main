...
#if (configUSE_TICKLESS_IDLE != 0) && (CFG_LPM_LEVEL != 0)
...
#endif /* (configUSE_TICKLESS_IDLE != 0) && (CFG_LPM_LEVEL != 0) */
...
SID_STM32_SPEED_OPTIMIZED uint32_t freertosPreSuppressTickAndSleepProcessing(uint32_t ulExpectedIdleTimeIn)
{
  ...
  const uint64_t ulSystemSleepDurationMs    = MX_APPE_GetSystemIdleTime() / (uint64_t)1000u; /* MX_APPE_GetSystemIdleTime() reports time in microseconds. Using math floor here since the wakeup shall take place not later than the planned time. Earlier wakeup is fine */
  const uint64_t ulFreeRTOSSleepDurationMs  = (uint64_t)portTICK_PERIOD_MS * (uint64_t)ulExpectedIdleTimeIn;

  /* Select the closest wakeup time if 2.4GHz radio or RTC event will wake up the MCU before the planned FreeRTOS wakeup event */
...
SID_STM32_SPEED_OPTIMIZED void vPortSuppressTicksAndSleep(uint32_t xExpectedIdleTime)
{
  ...
  * method as that will mask interrupts that should exit sleep mode. */
  ...
  expectedSystemSleepDurationUs = MX_APPE_GetSystemIdleTime();
  ...
  {
    /* Abort sleep since 2.4GHz radio or RTC event is pending shortly */
  ...
  else
  {
    ...
    if( eSleepStatus == eNoTasksWaitingTimeout )
    {
      /* It is not necessary to configure an interrupt to bring the
        ...
        future. */
      expectedSleepDurationUs = MIN(APPE_OS_SLEEP_NO_WAKEUP, expectedSystemSleepDurationUs);
    }
    else
    {
    ...
    MX_APPE_EnterLPM(expectedSleepDurationUs); /* WFI instruction call is inside this API */
    MX_APPE_LeaveLPM();
    ...
    lowPowerStepTicks = (uint32_t)((TIMER_IF_Convert_Tick2us(lowPowerTimeAfterSleep - lowPowerTimeBeforeSleep) + ((uint64_t)portTICK_PERIOD_MS * (uint64_t)1000u / (uint64_t)2u)) / ((uint64_t)portTICK_PERIOD_MS * (uint64_t)1000u));
