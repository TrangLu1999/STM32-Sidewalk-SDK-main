...
static uint32_t         crcLockCounter;
...
static void SystemPower_Config(const uint32_t initial_config);
...
#if (CFG_LPM_LEVEL != 0)
  /* Create a SW timer to wakeup the system from low power state */
...
static void SystemPower_Config(const uint32_t initial_config)
{
  if (initial_config != FALSE)
  {
    ...
    UTIL_LPM_Init();
    /* Disable Stand-by mode by default */
...
static void APPE_FLASH_MANAGER_Init(void)
{
  ...
  HAL_NVIC_SetPriority(FLASH_IRQn, FLASH_INTR_PRIO, 0u);
...
SID_STM32_SPEED_OPTIMIZED CRCCTRL_Cmd_Status_t CRCCTRL_MutexTake(void)
{
  ...
  if (osOK == os_status)
  {
    if (0u == crcLockCounter)
    {
      /* Enable CRC clock if this is the initial lock request. No actions required for any further recursive locks */
...
SID_STM32_SPEED_OPTIMIZED CRCCTRL_Cmd_Status_t CRCCTRL_MutexRelease(void)
{
  ...
  if (osOK == os_status)
  {
    ...
    if (0u == crcLockCounter)
    {
      /* Disable CRC clock to save power after the last recursive lock is released */
...
SID_STM32_SPEED_OPTIMIZED uint32_t UTIL_ADV_TRACE_EnterCriticalSection(void)
{
  ...
  if (osOK == os_status)
  {
    ...
    HAL_NVIC_DisableIRQ(LOG_UART_HANDLER_IRQn);
    HAL_NVIC_DisableIRQ(LOG_UART_HANDLER_TX_DMA_IRQn);
...
SID_STM32_SPEED_OPTIMIZED void UTIL_ADV_TRACE_ExitCriticalSection(const uint32_t mutex_used)
{
  ...
  {
    sid_pal_enter_critical_region();
    HAL_NVIC_EnableIRQ(LOG_UART_HANDLER_TX_DMA_IRQn);
    HAL_NVIC_EnableIRQ(LOG_UART_HANDLER_IRQn);
    sid_pal_exit_critical_region();
...
#if (CFG_LPM_LEVEL != 0)
SID_STM32_SPEED_OPTIMIZED void MX_APPE_EnterLPM(const uint64_t expectedSleepDurationUs)
{
  UTIL_LPM_LegacyMode_t target_lpm_mode = (UTIL_LPM_LegacyMode_t)UTIL_LPM_GetMaxMode();

  if (APPE_OS_SLEEP_NO_WAKEUP == expectedSleepDurationUs)
  {
  ...
  else
  {
    ...
    if (UTIL_LPM_OFFMODE == target_lpm_mode)
    {
      ...
      if ((expectedSleepDurationUs <= wakeupAheadTimeUs) || ((expectedSleepDurationUs - wakeupAheadTimeUs) < (CFG_LPM_STDBY_MIN_DURATION_MS * 1000u)))
      {
        ...
        UTIL_LPM_SetMaxMode(1u << CFG_LPM_SYS, UTIL_LPM_STOPMODE);
    ...
    if (UTIL_LPM_STOPMODE == target_lpm_mode)
    {
      ...
      if ((expectedSleepDurationUs <= wakeupAheadTimeUs) || ((expectedSleepDurationUs - wakeupAheadTimeUs) < (CFG_LPM_STOP_MIN_DURATION_US)))
      {
        ...
        UTIL_LPM_SetMaxMode(1u << CFG_LPM_SYS, UTIL_LPM_SLEEPMODE);
    ...
    const UTIL_TIMER_Status_t timerStatus = UTIL_TIMER_StartWithPeriodUs(&TimerOSwakeup_Id, actualSleepDurationUs);
    (void)timerStatus;
    assert_param(UTIL_TIMER_OK == timerStatus);
  }

  LL_PWR_ClearFlag_STOP(); /* Standby, Stop 1, and Stop 2 share the same flag, so this single call is sufficient */
  LL_PWR_ClearFlag_WU(); /* Clear all related wakeup flags */

  if ((UTIL_LPM_OFFMODE == target_lpm_mode) && (ll_sys_is_initialized() != FALSE))
  {
...
SID_STM32_SPEED_OPTIMIZED void MX_APPE_LeaveLPM(void)
{
  ...
  UTIL_LPM_SetMaxMode(1u << CFG_LPM_SYS, UTIL_LPM_OFFMODE);
}
#endif /* (CFG_LPM_LEVEL != 0) */
...
#if (CFG_LPM_LEVEL != 0)
SID_STM32_SPEED_OPTIMIZED uint64_t MX_APPE_GetSystemIdleTime(void)
{
  uint64_t systemIdleTimeUs;
  uint32_t nextRtcEventTicks;
  uint64_t nextRtcEventUs;
  uint64_t nextBleEventUs;

  /* Get remaining time till the nearest RTC event */
  nextRtcEventTicks = UTIL_TIMER_GetFirstRemainingTime();
  nextRtcEventUs = (APPE_RTC_TICKS_NO_WAKEUP == nextRtcEventTicks) ? APPE_OS_SLEEP_NO_WAKEUP : TIMER_IF_Convert_Tick2us(nextRtcEventTicks);

  /* Get remaining time till the nearest 2.4GHz radio event */
  if (ll_sys_is_initialized() != FALSE)
  {
    ble_stat_t cmd_status;
    uint32_t   next_event_tmp;

    cmd_status = ll_intf_le_get_remaining_time_for_next_event(&next_event_tmp);
    UNUSED(cmd_status);
    assert_param(cmd_status == SUCCESS);

    nextBleEventUs = (LL_DP_SLP_NO_WAKEUP == next_event_tmp) ? APPE_OS_SLEEP_NO_WAKEUP : (uint64_t)next_event_tmp;
  }
  else
  {
    nextBleEventUs = APPE_OS_SLEEP_NO_WAKEUP;
  }

  systemIdleTimeUs = MIN(nextRtcEventUs, nextBleEventUs);

  return systemIdleTimeUs;
}
#endif /* (CFG_LPM_LEVEL != 0) */
...
SID_STM32_SPEED_OPTIMIZED void MX_APPE_StandbyExit(void)
{
  /* Restore system power optimizations */
  SystemPower_Config(FALSE);

  /* Restore flash IRQ for ECC events handling */
  __HAL_FLASH_CLEAR_FLAG(FLASH_FLAG_ECCC);
  __HAL_FLASH_ENABLE_IT(FLASH_IT_ECCC);
  HAL_NVIC_SetPriority(FLASH_IRQn, FLASH_INTR_PRIO, 0u);
  HAL_NVIC_EnableIRQ(FLASH_IRQn);
  ...
  HAL_NVIC_SetPriority(WKUP_IRQn, 5u, 0u);
  HAL_NVIC_EnableIRQ(WKUP_IRQn);

  /* App-specific exit actions */
  SID_APP_StandbyExit();
}
