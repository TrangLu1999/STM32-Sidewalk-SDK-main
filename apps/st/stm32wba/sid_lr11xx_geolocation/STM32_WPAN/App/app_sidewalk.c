...
#define GEOLOCATION_DEMO_TLV_TAG_NAV3_NO_AP         (0x62u) /*!< GNSS scan results (NAV3 message) for cold scan (no assist position is available). This record provides NAV3 message(s) along with their timestamps (GPST scan time) and timestamp accuracy (in ms) */
#define GEOLOCATION_DEMO_TLV_TAG_NAV3_AP            (0x63u) /*!< GNSS scan results (NAV3 message) whenever assisted mode was used. It adds assist position reported by the LR11xx and the rest of the payload is identical to cold-start message */
...
} sid_uplink_gnss_nav3_scan_data_t;

typedef __PACKED_STRUCT {
    uint16_t                         time_accuracy; /*!< Timestamp accuracy (in ms) */
    sid_uplink_gnss_nav3_scan_data_t gnss_scan_results[1];
} sid_uplink_gnss_nav3_no_ap_record_t;
...
typedef __PACKED_STRUCT {
    ...
    sid_uplink_gnss_nav3_no_ap_record_t gnss_scan_data; /*!< The rest of the payload is identical to sid_uplink_gnss_nav3_no_ap_record_t to simplify deserialization */
} sid_uplink_gnss_nav3_ap_record_t;
#endif /* GEOLOCATION_DEMO_USE_GNSS */
...
#endif /* GEOLOCATION_DEMO_USE_WIFI */
#if GEOLOCATION_DEMO_USE_GNSS
        sid_uplink_gnss_nav3_no_ap_record_t gnss_no_ap_scan_results;
        sid_uplink_gnss_nav3_ap_record_t    gnss_ap_scan_results;
#endif /* GEOLOCATION_DEMO_USE_GNSS */
...
#endif /* GEOLOCATION_DEMO_USE_WIFI */
#if GEOLOCATION_DEMO_USE_GNSS
static sid_error_t  serialize_gnss_nav3_scan_data(const smtc_modem_gnss_event_data_scan_done_t * const scan_data, uint8_t * buffer, const size_t buffer_size, size_t * const out_serialized_len);
#endif /* GEOLOCATION_DEMO_USE_GNSS */
...
{
    size_t serialized_record_size = sizeof(sid_uplink_gnss_nav3_scan_data_t) - SID_STM32_UTIL_STRUCT_MEMBER_SIZE(sid_uplink_gnss_nav3_scan_data_t, nav_msg) + scan_data->nav_size;
...
#if GEOLOCATION_DEMO_USE_GNSS
static inline sid_uplink_gnss_nav3_scan_data_t * get_next_serialized_gnss_scan_data_ptr(const sid_uplink_gnss_nav3_scan_data_t * const current_ptr)
{
    const size_t offset = sizeof(*current_ptr) - sizeof(current_ptr->nav_msg) + current_ptr->nav_size;
    sid_uplink_gnss_nav3_scan_data_t * next_ptr = (sid_uplink_gnss_nav3_scan_data_t *)(void *)((size_t)(void *)current_ptr + offset);
    return next_ptr;
}
#endif /* GEOLOCATION_DEMO_USE_GNSS */
...
static sid_error_t serialize_gnss_nav3_scan_data(const smtc_modem_gnss_event_data_scan_done_t * const scan_data, uint8_t * buffer, const size_t buffer_size, size_t * const out_serialized_len)
{
    ...
    do
    {
        ...
        /* Calculate the required buffer size and ensure we have enough space */
        size_t serialized_data_length = sizeof(serialized_payload->header) + (sizeof(sid_uplink_gnss_nav3_no_ap_record_t) - SID_STM32_UTIL_STRUCT_MEMBER_SIZE(sid_uplink_gnss_nav3_no_ap_record_t, gnss_scan_results));
        if (assist_pos_valid != false)
        {
            /* Ad overhead to store assist position */
            serialized_data_length += sizeof(sid_uplink_gnss_nav3_ap_record_t) - sizeof(sid_uplink_gnss_nav3_no_ap_record_t);
        }
        for (uint32_t i = 0u; i < scan_data->nb_scans_valid; i++)
        {
        ...
        #pragma GCC diagnostic push
        #pragma GCC diagnostic ignored "-Warray-bounds"
        serialized_payload->header.tag = (false == assist_pos_valid) ? GEOLOCATION_DEMO_TLV_TAG_NAV3_NO_AP : GEOLOCATION_DEMO_TLV_TAG_NAV3_AP;
        serialized_payload->header.length = (uint8_t)(serialized_data_length - sizeof(serialized_payload->header));

        /* Populate common data */
        sid_uplink_gnss_nav3_scan_data_t * serialized_gnss_scan_data;
        if (false == assist_pos_valid)
        {
            sid_uplink_gnss_nav3_no_ap_record_t * gnss_payload = &serialized_payload->payload.gnss_no_ap_scan_results;
        ...
        {
            sid_uplink_gnss_nav3_ap_record_t * gnss_payload = &serialized_payload->payload.gnss_ap_scan_results;
                ...
                case EVENT_TYPE_SIDEWALK_SEND_GEOLOCATION_DATA:
                    ...
                    if (app_context->state == STATE_SIDEWALK_READY)
                    {
                        ...
                        if (lbm_gnss_scan_done_data.nb_scans_valid > 0u)
                        {
                            ...
                            ret = serialize_gnss_nav3_scan_data(&lbm_gnss_scan_done_data, working_buf_ptr, remaining_buf_space, &serialized_gnss_data_len);
                            if (SID_ERROR_NONE == ret)
                            {
...
static void geolocation_demo_task_entry(void *context)
{
    ...
    (void)app_context; /* Not used in this demo, but may be potentially useful if Sidewalk API is called from this thread */
...
#  endif /* GEOLOCATION_DEMO_USE_GNSS && LR11XX_RADIO_CFG_GNSS_SCAN_USE_BLANKING */
        ...
        /* 11. Get MCU temperature to send it together with GNSS data ----------------*/
        ...
        /* 12. Send out geolocation scan results via Sidewalk link -------------------*/
        ...
        /* 13. Idle time, do nothing till the next location update cycle -------------*/
