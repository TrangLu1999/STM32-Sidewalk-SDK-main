...
static sid_error_t lr11xx_enter_bootloader_mode(void)
{
    ...
    do
    {
        ...
        /* Configure the output stage of Busy pin GPIO on MCU side to open-drain mode */
        ...
        /* Step 2 - drive NRESET - this will hold NRESET low for at least 100us and return back immediately since Busy is held low by MCU */
...
static ace_status_t lr11xx_cli_run_mode_cmd(int32_t argc, const char **argv)
{
    ...
    do
    {
        ...
        else
        {
            ...
            {
                CLI_LOG_ERROR("\"%s\" is not a recognized LR11xx run mode. Valid values are: bootloader, app", desired_mode_str);
                sid_err = SID_ERROR_INVALID_ARGS;
            }
        }
    } while (0);
...
static ace_status_t lr11xx_cli_fw_auto_update_cmd(int32_t argc, const char **argv)
{
    ...
    do
    {
        ...
        if ((LR11XX_RADIO_BOOTLOADER_MODE_ID == drv_ctx->ver.type) || ((drv_ctx->last.stat2 & SID_STM32_LR11XX_HAL_STAT2_BOOTLOADER_MASK) == SID_STM32_LR11XX_HAL_STAT2_BOOTLOADER_MODE))
        {
            CLI_LOG_ERROR("LR11xx update to firmware %02X.%02X rejected. LR11xx will stay in bootloader mode until a valid firmware is flashed");
            sid_err = SID_ERROR_TRY_AGAIN;
            break;
        }

        /* Ensure the reported firmware version matches the expected one */
