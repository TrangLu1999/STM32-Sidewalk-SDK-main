...
static void SystemPower_Config(void)
{
  ...
  /* RAM retention in Standby mode configuration */
...
void MX_RTC_Init(void)
{
  ...
  */
  ...
  /* Ensure RTC APB clock is enabled to access RTC registers */
  __HAL_RCC_RTCAPB_CLK_ENABLE();

  /* Check if RTC is running already */
  if (__HAL_RCC_GET_RTC_SOURCE() != RCC_RTCCLKSOURCE_DISABLE)
  {
    /* RTC is running, check if its config matches app config */
    if ((((READ_REG(RTC->PRER) & RTC_PRER_PREDIV_A_Msk) >> RTC_PRER_PREDIV_A_Pos) == hrtc.Init.AsynchPrediv)
    && ((READ_REG(RTC->ICSR) & (RTC_ICSR_BIN_Msk | RTC_ICSR_BCDU_Msk)) == (hrtc.Init.BinMode | hrtc.Init.BinMixBcdU))
#if defined(RTC_CR_OSEL_Msk)
    && ((READ_REG(RTC->CR) & (RTC_CR_OSEL_Msk | RTC_CR_TAMPOE_Msk)) == hrtc.Init.OutPut)
#endif /* RTC_CR_OSEL_Msk */
    )
    {
      /* RTC is initialized already by the bootloader and its configuration matches the app config - skip full re-init to preserve RTC SSR value */
  ...
  /* RTC is disabled or its configuration does not match app config - in both cases SSR value cannot be used and full RTC (re)initialization is required */
  ...
  */
  ...
  /** Enable wake up from Standby by RTC IRQ
  */
  #  if defined (__ARM_FEATURE_CMSE) && (__ARM_FEATURE_CMSE == 3U)
  /* Enable wakeup line 6 for RTC */
...
void MX_SPI1_Init(void)
{
  ...
  /* FIXME: HAL_SPIEx_SetConfigAutonomousMode() fires assert if TriggerSelection is not valid even if autonomous mode is disabled */
  HAL_SPI_AutonomousMode_Cfg_Struct.TriggerSelection = SPI_GRP1_GPDMA_CH0_TCF_TRG; /* Can be any valid value */
...
/**
  * @brief USART2 Initialization Function (Sensor UART - G0B1 communication)
  * @param None
  * @retval None
  */
...
/* USER CODE BEGIN MX_GPIO_Init_2 */
  ...
  GPIO_InitStruct.Pin = SIDEWALK_RF_TIMINGS_RXTX_START_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(SIDEWALK_RF_TIMINGS_RXTX_START_GPIO_Port, &GPIO_InitStruct);

  SIDEWALK_RF_TIMINGS_WBA_AWAKE_GPIO_Port->BSRR = SIDEWALK_RF_TIMINGS_WBA_AWAKE_Pin;
  GPIO_InitStruct.Pin = SIDEWALK_RF_TIMINGS_WBA_AWAKE_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(SIDEWALK_RF_TIMINGS_WBA_AWAKE_GPIO_Port, &GPIO_InitStruct);
