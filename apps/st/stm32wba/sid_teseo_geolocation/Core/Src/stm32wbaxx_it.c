...
extern void sid_pal_gpio_wakeup_irq_handler(void);
#endif /* (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED != 0) */
extern void sid_pal_uart_irq_handler(UART_HandleTypeDef * huart);
...
void NMI_Handler(void)
{
  ...
  if (__HAL_FLASH_GET_FLAG(FLASH_FLAG_ECCD))
  {
    ...
    else if ((error_address >= APP_CONFIG_SNVMA_FLASH_START) && (error_address < APP_CONFIG_SNVMA_FLASH_END))
    {
      ...
      /* BLE NVM blocks are covered by CRC. Any uncorrectable ECC detection will lead to a CRC mismatch and subsequent erasure of the problematic page by SNVMA module, there's no need to erase the page here */

      /* Clear ECC detection flag to allow the program to proceed */
      __HAL_FLASH_CLEAR_FLAG(FLASH_FLAG_ECCD);

      /* Return from here and let the SNVMA to handle the ECC error */
      return;
    }
#endif /* BLE */
...
void LPUART1_IRQHandler(void)
{
  ...
  sid_pal_uart_irq_handler(&hlpuart);
...
void USART2_IRQHandler(void)
{
  ...
  sid_pal_uart_irq_handler(&huart2);
...
void WKUP_IRQHandler(void)
{
  ...
  sid_pal_gpio_wakeup_irq_handler();
