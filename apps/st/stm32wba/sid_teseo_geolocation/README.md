# STM32WBAxx Geolocation Sample App

This is a sample application demonstrating geolocation capabilities of ST's [Teseo](https://www.st.com/en/positioning.html) GNSS modules running with **STM32WBAxx microcontrollers** based on **STMicroelectronics' Amazon Sidewalk stack**. The demo showcases GNSS-based geolocation and consists of two main components:

1. **Embedded Device Firmware** – Runs on an STM32WBAxx microcontroller and interfaces with a Teseo module (e.g., LIV3F) for geolocation scanning.
2. **AWS Cloud Backend** – Processes geolocation scan results from the device, reconstructs fragmented uplink messages, and publishes device positions via MQTT.

> [!NOTE]
> This sample app does not use Sidewalk Location functionality, but rather an ST's proprietary implementation to send both the position and telemetry data in a single uplink message.
>
>The custom Lambda function on the IoT Core side updates [Resource Position](https://docs.aws.amazon.com/iot-wireless/latest/apireference/API_UpdateResourcePosition.html) property of the Sidewalk device, providing the outcomes that are functionally equivalent to the Sidewalk Location functionality.
...
- Periodically performs position reading from the attached Teseo module:
- Periodically captures MCU temperature to send it out together with the position information
...
- Lambda reassembles the complete message, extracts **GNSS position data**.
- Publishes the position to an **MQTT topic** for data visualization.

~~~text
+----------------------+       +-------------------------+       +--------------------------+
| STM32WBAxx + Teseo + | ----> | AWS IoT Core (Sidewalk) | ----> |  AWS Lambda (Processing) |
| Sub-GHz radio        |       |                         |       |  - Reassemble fragments  |
| - Amazon Sidewalk    |       | AWS IoT Rule            |       |  - Extract position data |
|   communication      |       |                         |       |  - Extract sensors data  |
| - GNSS scanning      |       +-------------------------+       +--------------------------+
+----------------------+                                                       |
                                                                               v
                                                             +------------------------------+
...
| *AWS_ACCESS_KEY_ID*         | *null* **(needs to be overwritten)**  | Your AWS_ACCESS_KEY_ID. This parameter is used when *AWS_PROFILE* is null
| *AWS_SECRET_ACCESS_KEY*     | *null* **(needs to be overwritten)**  | Your AWS_SECRET_ACCESS_KEY. This parameter is used when *AWS_PROFILE* is null
| *AWS_SESSION_TOKEN*         | *null* **(needs to be overwritten)**  | Your AWS_SESSION_TOKEN. This parameter is used when *AWS_PROFILE* is null
...
| *SID_DEMO_NAME*             | *SidewalkTeseoGeolocation*            | Name of the demo app. This name is used as prefix for the AWS resources that are deployed for this app
...
| *SID_DEMO_PROVISION_DEVICE* | *True*                                | Automatically register a new Sidewalk device, assigns it to this demo app and generates manufacturing data image to be flashed to the STM32WBAxx board
| *TARGET_MCU*                | *["WBA65xI", "WBA55xG", "WBA52xG"]*   | List of target MCUs for generating manufacturing data images
...
- **Windows**:
```
python deploy_geolocation_demo_stack.py
```

Type `y` when asked to proceed. The script will automatically install all the necessary Python dependencies and then it will proceed with the deployment. Wait for the deployment to complete (it usually takes a few minutes).
...
The device will automatically perform Sidewalk Device Registration procedure using BLE or FSK link (depending on the selected build configuration) and then it will switch to the normal link. It may take some time to receive the first scan results, especially for GNSS cold start with outdated Almanac data. You can observe arriving Sidewalk uplinks and resolved position information by subscribing to the respective MQTT topics:
- `{SID_DEMO_MQTT_TOPIC_ROOT}/+/raw` (e.g. `sidewalk/geolocation/+/raw`): raw Sidewalk uplinks received from the device. This topic is most useful for debugging and checking whether the device is sending anything at all.
- `{SID_DEMO_MQTT_TOPIC_ROOT}/+/telemetry` (e.g. `sidewalk/geolocation/+/telemetry`): decoded telemetry and location data. This topic displays the final result, providing both sensor readings and device position in a single JSON payload
- `{SID_DEMO_MQTT_TOPIC_ROOT}/+/positioning` (e.g. `sidewalk/geolocation/+/positioning`): this topic published the updates to the Resource Position property of the Sidewalk device.

## Removing the demo from the cloud

It is recommended to delete the backend resources after you've finished with this demo to avoid additional charges for AWS resource usage. To simplify the tear-down procedure you can use `delete_geolocation_demo_stack.py` script in `AWS` folder to automatically delete all the associated cloud resources
...
- **Windows**:
```
python delete_geolocation_demo_stack.py
```

Type `y` when asked to proceed. Wait for the deletion to complete (it usually takes a few minutes).
