...
static void _on_incoming_user_data_cb(const uint8_t * const data, const uint32_t data_len, void * user_arg)
{
    ...
    {
        /* Store the received data into a static buffer - original data may get corrupted/overwritten by the moment UART printout is processed */
...
static void user_data_transfer_task_entry(void *context)
{
    ...
    sid_err = sid_pal_radio_stm32wlxx_set_user_data_received_cb(_on_incoming_user_data_cb, NULL);
