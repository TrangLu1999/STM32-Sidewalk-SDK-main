...
#define SBDT_DEMO_LOG_PREFIX                  "SBDT: "

#define SBDT_DEMO_DFU_RESET_DELAY_S           (5u)
...
/* Declare the necessary data structures to interface with the Application Install Manager (e.g., image header, footer, etc.) */
SID_AIM_IFC_DECLARE_OTA_METADATA();
...
static void        trigger_delayed_dfu_reset(void);
...
#endif
            /* Mark current app as verified after the successful connection to the Sidewalk network */
            (void)sid_aim_ifc_confirm_app_is_verified();
            break;
...
static void reset_delay_timer_cb(void *cookie, sid_pal_timer_t *originator)
{
    ...
    sid_hal_reset(SID_HAL_RESET_DFU);
}
...
static void trigger_delayed_dfu_reset(void)
{
    ...
    struct sid_timespec sid_tsec = {
        .tv_sec  = SBDT_DEMO_DFU_RESET_DELAY_S,
        .tv_nsec = 0u,
    };
    ...
    SID_PAL_LOG_INFO("Scheduled reboot to start update via Application Install Manager");
}
...
static void platform_flash_init(void)
{
    ...
    SID_PAL_ASSERT((APP_CONFIG_AIM_STAGING_SLOT_START - FLASH_BASE) % FLASH_PAGE_SIZE == 0u);

    err = sid_pal_timer_init(&reset_delay_timer, reset_delay_timer_cb, NULL);
    if (err != SID_ERROR_NONE)
    {
        SID_PAL_LOG_ERROR(SBDT_DEMO_LOG_PREFIX "Failed to initialize reset delay timer. Error: %d", (int32_t)err);
    }

    err = sid_aim_ifc_init();
    if (err != SID_ERROR_NONE)
    {
        SID_PAL_LOG_ERROR(SBDT_DEMO_LOG_PREFIX "Failed to initialize AIM interface. Error: %d", (int32_t)err);
    }
}
...
static uint32_t platform_flash_get_free_space(void)
{
    uint32_t free_bytes = APP_CONFIG_AIM_STAGING_SLOT_SIZE;
    return free_bytes;
}
...
static bool platform_flash_write(uint32_t offset, void *data, size_t size)
{
    ...
    do
    {
        ...
        err = sid_aim_ifc_store_ota_data_chunk(data, offset, size);
        if (err != SID_ERROR_NONE)
        {
            SID_PAL_LOG_INFO(SBDT_DEMO_LOG_PREFIX "Failed to store %u bytes at address 0x%08X. Error %d", size, write_start_addr, (int32_t)err);
            break;
        }

        SID_PAL_LOG_INFO(SBDT_DEMO_LOG_PREFIX "%u bytes written to address 0x%08X", size, write_start_addr);
        success = true;
    } while (0);
...
static bool platform_flash_check(uint32_t size, uint32_t expected_crc)
{
    const uint32_t calculated_crc = crc32_compute((uint8_t *)(void *)APP_CONFIG_AIM_STAGING_SLOT_START, size, NULL);
    SID_PAL_LOG_INFO(SBDT_DEMO_LOG_PREFIX "calculated CRC: 0x%08X, expected: 0x%08X, length = %u", calculated_crc, expected_crc, size);
...
static bool platform_flash_finalize(uint32_t size, uint32_t crc)
{
    SID_PAL_LOG_INFO(SBDT_DEMO_LOG_PREFIX "OTA image received. Scheduling MCU reset in %u seconds", SBDT_DEMO_DFU_RESET_DELAY_S);

    /* Release AIM interface resources */
    (void)sid_aim_ifc_deinit();

    /* Schedule DFU reset */
    trigger_delayed_dfu_reset();
    return true;
}
...
static void sidewalk_stack_task_entry(void *context)
{
    ...
    /* Pre-populate event callback wrappers with original function pointers */
...
void SID_APP_Init(void)
{
    ...
    ret_code = sid_pal_gpio_set_irq(GPIO_PORT_PIN_TO_NUM(B2_GPIO_PORT, B2_PIN), SID_PAL_GPIO_IRQ_TRIGGER_RISING, button2_irq_handler, NULL);
    SID_PAL_ASSERT(ret_code == SID_ERROR_NONE);
...
#  else
    ret_code = sid_pal_gpio_set_irq(GPIO_PORT_PIN_TO_NUM(B3_GPIO_PORT, B3_PIN), SID_PAL_GPIO_IRQ_TRIGGER_RISING, button3_irq_handler, NULL);
#  endif /* SID_RADIO_PLATFORM_LR11XX || SID_RADIO_PLATFORM_SX126X */
    SID_PAL_ASSERT(ret_code == SID_ERROR_NONE);
#endif /* CFG_BUTTON_SUPPORTED */

    /**
     * app_context_t keeps a full SBDT scratch buffer, so avoid static initialization as it will result in about 20-25KB flash footprint increase
     * due to the need to store this object in .data section init image
     */
    static app_context_t demo_app_context = {0};

    /* Initialize only the necessary fields in the runtime */
    demo_app_context.hw_ifc.platform_reset                = platform_reset,
    demo_app_context.hw_ifc.platform_flash_init           = platform_flash_init,
    demo_app_context.hw_ifc.platform_flash_get_free_space = platform_flash_get_free_space,
    demo_app_context.hw_ifc.platform_flash_write          = platform_flash_write,
    demo_app_context.hw_ifc.platform_flash_check          = platform_flash_check,
    demo_app_context.hw_ifc.platform_flash_finalize       = platform_flash_finalize,
