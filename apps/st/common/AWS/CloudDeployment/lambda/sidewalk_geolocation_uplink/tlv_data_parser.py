...
class TlvParser:
    ...
    __TAG_MCU_TEMPERATURE      = '37'  # MCU temperature - just a sample of numerical value, has no relation to geolocation functionality
    ...
    __TAG_NAV3_NO_AP_SCAN_DATA = '62'  # NAV3 message (GNSS scan data) from LR11xx when a cold start (no Assist Position) scan method was used
    __TAG_NAV3_AP_SCAN_DATA    = '63'  # NAV3 message (GNSS scan data) from LR11xx when Assist Position scan mode was used - includes Assist Position used by LR11xx during the scan
    __TAG_GNSS_POSITION        = '60'  # Resolved GNSS position data as reported by the GNSS receiver (e.g., Teseo)
    ...
    def parse(payload: str):
        ...
        while payload:
            ...
            elif tag == TlvParser.__TAG_NAV3_NO_AP_SCAN_DATA:
                print(f'Parsing {length} bytes of NAV3 GNSS (no assist position) scan data: {raw_data}')
                geolocation_data['nav3_messages'] = TlvParser.__parse_nav3_scan_data(raw_data)
            elif tag == TlvParser.__TAG_NAV3_AP_SCAN_DATA:
                print(f'Parsing {length} bytes of NAV3 GNSS (with assist position) scan data: {raw_data}')
                geolocation_data['nav3_messages'] = TlvParser.__parse_nav3_with_ap_scan_data(raw_data)
            ...
            elif tag == TlvParser.__TAG_GNSS_POSITION:
                print(f'Parsing {length} bytes of GNSS position data: {raw_data}')
                geolocation_data['position'] = TlvParser.__parse_gnss_position_data(raw_data)
            elif tag == TlvParser.__TAG_COUNTER_VALUE:
                geolocation_data['demo_counter'] = int(raw_data, 16)
                print(f'Received demo counter value: { geolocation_data["demo_counter"]}')
            elif tag == TlvParser.__TAG_MCU_TEMPERATURE:
                geolocation_data['mcu_temperature'] = int.from_bytes(bytes.fromhex(raw_data[:4]), byteorder="big", signed=True)
                print(f'Received MCU temperature value: {geolocation_data["mcu_temperature"]}')
            else:
    ...
    @staticmethod
    def __parse_nav3_scan_data(raw_data: str):
        gnss_scans=[]
    ...
    @staticmethod
    def __parse_nav3_with_ap_scan_data(raw_data: str):
        ...
        gnss_scans = TlvParser.__parse_nav3_scan_data(raw_data)
    ...
    @staticmethod
    def __parse_gnss_position_data(raw_data: str):
        # Bytes 0-3 - UTC time expressed as Unix epoch timestamp
        timestamp_epoch = int.from_bytes(bytes.fromhex(raw_data[:8]), byteorder="big", signed=False)
        raw_data = raw_data[8:]
        print(f'Parsed GNSS position raw timestamp: {timestamp_epoch}')
        timestamp = datetime.fromtimestamp(timestamp_epoch, timezone.utc)
        print(f'Parsed GNSS position timestamp: {timestamp}')

        # Bytes 4-7 - Latitude value
        latitude = struct.unpack('>f', bytes.fromhex(raw_data[:8]))[0]
        raw_data = raw_data[8:]
        print(f'Parsed GNSS latitude value: {latitude}')

        # Bytes 8-11 - Longitude value
        longitude = struct.unpack('>f', bytes.fromhex(raw_data[:8]))[0]
        raw_data = raw_data[8:]
        print(f'Parsed GNSS longitude value: {longitude}')

        # Bytes 12-15 - Elevation value
        elevation = struct.unpack('>f', bytes.fromhex(raw_data[:8]))[0]
        raw_data = raw_data[8:]
        print(f'Parsed GNSS elevation value: {elevation}')

        # Bytes 16-19 - Horizontal accuracy
        horizontal_accuracy = struct.unpack('>f', bytes.fromhex(raw_data[:8]))[0]
        raw_data = raw_data[8:]
        print(f'Parsed GNSS horizontal accuracy: {horizontal_accuracy}')

        # Bytes 20-23 - Horizontal accuracy
        vertical_accuracy = struct.unpack('>f', bytes.fromhex(raw_data[:8]))[0]
        print(f'Parsed GNSS vertical accuracy: {vertical_accuracy}')
