...
#
##############################################################################
# file:    cloud_formation_client.py
# brief:   Deployment of the CloudFormation template
##############################################################################
#
...
class CloudFormationClient:
    ...
    # Deploy
    # -------
    def create_stack(self, stack_name: str, template_params: dict, tag: str, template: str|None=None, template_url: str|None=None, lambda_sources: dict|None=None):
        """
        Creates CloudFormation stack.

        :param template:        CloudFormation template stored locally.
        :template_url:          CloudFormation template stored on an external server
        :param stack_name:      Stack name.
        :param template_params: CloudFormation template parameters
        :param tag:             Tag assigned to created resources; describes application.
        """
        log_info(f'Creating {stack_name} from CloudFormation template...')
        stack_already_exists = False
        start_time = datetime.now(timezone.utc)

        parameters = []
        for param_key, param_value in template_params.items():
            parameter_record = {
                'ParameterKey': str(param_key)
            }
            if isinstance(param_value, bool):
                parameter_record['ParameterValue'] = 'true' if param_value else 'false'
            else:
                parameter_record['ParameterValue'] = str(param_value)
            parameters.append(parameter_record)
        if lambda_sources:
            for lambda_name, lambda_info in lambda_sources.items():
                parameters.append({
                    'ParameterKey': f'{lambda_name}LambdaSrc',
                    'ParameterValue': f'{lambda_info['src_folder']}_{lambda_info['hash']}.zip',
                })
    ...
    def delete_stack(self, stack_name: str):
        ...
        :param stack_name:    Stack name.
        """
        log_info(f'Deleting {stack_name} from AWS account...')
        try:
    ...
    def check_if_resource_is_managed_by_stack(self, stack_name: str, logical_resource_id: str) -> bool:
        """
        Checks if given destination already exists.

        :param stack_name:          The logical name of the resource as specified in the template
        :param logical_resource_id: The name or the unique stack ID that’s associated with the stack
        :return:                    True if the resource is managed by the specified stack, False otherwise.
        """
        try:
            #log_info(f'Checking if {name} destination exists...')
            response = self._client.describe_stack_resource(StackName=stack_name, LogicalResourceId=logical_resource_id)
            if response['ResponseMetadata']['HTTPStatusCode'] == 200:
                return True
            else:
                raise Exception(f'Failed to check if AWS resource is managed by CloudFormationStack')
        except ClientError as e:
            if e.response['Error']['Code'] == 'ResourceNotFoundException':
                return False
            else:
                terminate(f'Unable to check if the resource is manged by CloudFormation stack: {e}.', ErrCode.EXCEPTION)
