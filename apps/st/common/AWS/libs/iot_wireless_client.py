...
class IoTWirelessClient:
    ...
    def create_sidewalk_device(self, device_name: str, profile_name: str, destination_name: str, positioning_destination_name: str):
        try:
            ...
            response = self._wireless_client.create_wireless_device(
                ...
                Sidewalk={
                    ...
                    'Positioning': {
                        'DestinationName': positioning_destination_name,
                    },
                },
                Positioning='Enabled',
                Tags=[
    ...
    def delete_sidewalk_device(self, device_name: str, device_id: str):
        try:
            ...
            except self._wireless_client.exceptions.ResourceNotFoundException:
                pass
    ...
    def enable_sidewalk_device_positioning(self, device_name: str, device_id: str, positioning_destination_name: str):
        try:
            log_info(f'Enabling Sidewalk Location feature for device {device_name}, positioning data destination: {positioning_destination_name}')

            if not self.check_if_destination_exists(positioning_destination_name):
                raise AttributeError(f'Sidewalk destination {positioning_destination_name} does not exist')
            ...
            eval_client_response(response, f'Successfully enabled Sidewalk Location for device {device_name}, position data destination: {positioning_destination_name}')
        except Exception as e:
            terminate(f'Failed to enable Sidewalk Location for device {device_name}, destination {device_name}: {e}.', ErrCode.EXCEPTION)
