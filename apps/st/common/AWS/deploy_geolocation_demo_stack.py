...
def main():
    ...
    try:
        ...
        parser = argparse.ArgumentParser(
            description='Unified AWS deployment tool for geolocation demo apps'
        )

        parser.add_argument(
            '--config',
            required=True,
            help='Path to the deployment configuration YAML file'
        )
        ...
        # Read config file
        # -----------------
        config = Config(Path(args.config).resolve())
        ...
        # ---------------------------------------------------
        # Check if given Positioning destination already exists
        # ---------------------------------------------------
        positioning_dest_already_exists = wireless_client.check_if_destination_exists(name=constants.SIDEWALK_POSITIONING_DESTINATION_NAME)
        if positioning_dest_already_exists:
            positioning_dest_managed_by_current_stack = cf_client.check_if_resource_is_managed_by_stack(constants.SIDEWALK_DEMO_STACK_NAME, 'IoTWirelessDestination00GeolocationAppPositioningDestination')
            if not positioning_dest_managed_by_current_stack:
                log_warn(f'Sidewalk destination {constants.SIDEWALK_POSITIONING_DESTINATION_NAME} already exists and will not be included in the {constants.SIDEWALK_DEMO_STACK_NAME}')
            else:
                log_info(f'Sidewalk destination {constants.SIDEWALK_POSITIONING_DESTINATION_NAME} exist, but it is manged by the current CloudFormation stack. It will be updated as a part of the deployment')
                positioning_dest_already_exists = False
        else:
            log_success(f'Sidewalk destination {constants.SIDEWALK_POSITIONING_DESTINATION_NAME} does not exist and will be included in the {constants.SIDEWALK_DEMO_STACK_NAME}')
        ...
        cf_template_parameters = {
            ...
            'SidewalkGeolocationMqttRootTopic':            config.sid_demo_mqtt_topic_root,
            ...
            'SidewalkPositioningDestinationName':          constants.SIDEWALK_POSITIONING_DESTINATION_NAME,
            'SidewalkPositioningDestinationRoleName':      constants.SIDEWALK_POSITIONING_DESTINATION_ROLE,
            'SidewalkPositioningDestinationAlreadyExists': positioning_dest_already_exists,
        }
        cf_lambda_sources = {
            'SidewalkGeolocationUplink': {'src_folder': 'sidewalk_geolocation_uplink'},
        }
        ...
        if config.manage_device_instance:
            ...
            if device is None:
                # Create the device
                device = wireless_client.create_sidewalk_device(config.sid_demo_device_name, constants.SIDEWALK_DEVICE_PROFILE, constants.SIDEWALK_DESTINATION_NAME, constants.SIDEWALK_POSITIONING_DESTINATION_NAME)
            else:
                if config.interactive_mode:
                ...
                # Ensure Sidewalk Location is enabled for the device
                wireless_client.enable_sidewalk_device_positioning(
                    device_name=device['Name'],
                    device_id=device['Id'],
                    positioning_destination_name=constants.SIDEWALK_DESTINATION_NAME)
            ...
            for chip in config.target_mcu:
                ret = subprocess.call([sys.executable, os.path.join(config.provisioning_tool_dir, 'provision.py'),
                                       'st', 'aws',
                                       f'--certificate_json={os.path.join(config.provisioning_output_dir, provisioning_file_name)}',
                                       f'--output_hex={os.path.join(config.provisioning_output_dir, "mfg_"+str(device["Name"]).lower()+"_"+str(chip).lower()+".hex")}',
                                       f'--output_bin={os.path.join(config.provisioning_output_dir, "mfg_"+str(device["Name"]).lower()+"_"+str(chip).lower()+".bin")}',
                                    ])
                if ret != 0:
        ...
        log_success('---------------------------------------------------------------')
        log_success(f'{config.sid_demo_name if config is not None else "Geolocation"} Demo has been successfully deployed to your AWS account')
        log_success('---------------------------------------------------------------')

    except Exception as e:
        print(f'Failed to deploy {config.sid_demo_name if config is not None else "Geolocation"} Demo. Error {e}')
