...
class GeolocationDemoConstants:
    ...
    def __init__(self, config: Config|None=None):
        if not GeolocationDemoConstants.__init_done and config is not None:
            ...
            self.CLOUDFORMATION_TEMPLATE_BUCKET_NAME  = f'{self.AWS_ACCOUNT_ID}-{str(config.sid_demo_name).lower()}-cf'
            ...
            self.SIDEWALK_POSITIONING_DESTINATION_NAME = f'{config.sid_demo_name}PositioningDestination'
            self.SIDEWALK_POSITIONING_DESTINATION_ROLE = f'{config.sid_demo_name}PositioningDestinationRole'
