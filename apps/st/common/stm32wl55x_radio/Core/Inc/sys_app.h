...
/* Exported constants --------------------------------------------------------*/
#define SYS_APP_OS_SLEEP_NO_WAKEUP  ((uint64_t)UINT64_MAX)
#define SYS_APP_RTC_TICKS_NO_WAKEUP (UINT32_MAX)
/* USER CODE BEGIN EC */
...
/**
 * @brief Get expected system idle time
 *
 * @retval Expected system idle time in microseconds. Whenever there's no scheduled wakeup event, SYS_APP_OS_SLEEP_NO_WAKEUP is returned
 */
uint64_t SystemApp_GetSystemIdleTime(void);
