...
SID_STM32_SPEED_OPTIMIZED uint64_t SystemApp_GetSystemIdleTime(void)
{
  ...
  nextRtcEventUs = (SYS_APP_RTC_TICKS_NO_WAKEUP == nextRtcEventTicks) ? SYS_APP_OS_SLEEP_NO_WAKEUP : TIMER_IF_Convert_Tick2us(nextRtcEventTicks);

  return nextRtcEventUs;
}
#endif /* (LOW_POWER_DISABLE == 0) */
