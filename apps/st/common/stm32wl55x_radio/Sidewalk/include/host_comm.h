...
typedef enum {
    ...
    SID_HOST_COMM_INVALID_ARGS         = 2, /*!< Invalid argument supplied by the caller function */
 ...
 *
 * @param [in] data     Pointer to the buffer containing the received data
 * @param [in] data_len Size of the buffer
 * @param [in] user_arg User-defined argument
 */
typedef void (*sid_host_comm_on_incoming_user_data)(const uint8_t * const data, const uint32_t data_len, void * user_arg);
...
/**
 * @brief Notify Host Comm about the GPIO handshake request from the host
 */
void sid_host_comm_notify_handshake_requested(void);

/**
 * @brief Initialize Host Comm component
 *
...
/**
 * @brief Notify Host Comm about the reception of an SPI frame
 */
void sid_host_comm_on_spi_frame_received(void);

/**
 * @brief Enqueue data to be sent over the SPI bus to the host MCU
 *
 * @param [in] data      Data to be sent to the remote side
 * @param [in] data_size Size of the data to be sent
 * @returns              Non-zero value in case of error
 */
 ...
 * @param [in] user_arg User-defined argument that will be passed to the callback
 * @returns             Non-zero value in case of error
 */
sid_host_comm_error_t sid_host_comm_set_user_data_received_cb(sid_host_comm_on_incoming_user_data callback, void * user_arg);
