...
#  define STM32WLxx_SUBGHZ_MIN_CHANNEL_FREE_DELAY_US      (1u)
#  define STM32WLxx_SUBGHZ_MIN_CHANNEL_NOISE_DELAY_US     (30u)
#  define STM32WLxx_SUBGHZ_NOISE_SAMPLE_SIZE              (32u)
...
static inline sid_radio_error_t  _sid_radio_indicate_request_processed(const uint16_t request_id, const uint16_t request_status, const stm32wlxx_req_completed_compact_data_t * const compact_data, const uint8_t * const extended_data, const uint32_t extended_data_len);
...
static inline sid_radio_error_t  _sid_radio_subghz_set_rx_duty_cycle(const uint32_t rx_time_in_ms, const uint32_t sleep_time_in_ms);
...
static inline sid_radio_error_t  _sid_radio_set_lora_cad_params(const stm32wlxx_rcp_set_lora_cad_params_req_t * const request);
static inline sid_radio_error_t  _sid_radio_start_tx(const uint32_t timeout);
static inline sid_radio_error_t  _sid_radio_start_rx(const stm32wlxx_rcp_start_rx_req_t * const params);
...
static inline sid_radio_error_t  _sid_radio_start_fsk_carrier_sense(const stm32wlxx_rcp_start_fsk_cs_req_t * const request);
...
static inline sid_radio_error_t  _sid_radio_start_cw_tx(const stm32wlxx_rcp_start_cw_tx_req_t * const request);
static inline sid_radio_error_t  _sid_radio_start_infinite_preamble_tx(const stm32wlxx_rcp_start_inf_pbl_tx_req_t * const request);
static inline sid_radio_error_t  _sid_radio_start_continuous_rx(void);
static inline sid_radio_error_t  _sid_radio_start_rx_duty_cycle(const stm32wlxx_rcp_start_rx_dc_req_t * const request);
static inline sid_radio_error_t  _sid_radio_start_lora_cad(void);
static inline sid_radio_error_t  _sid_radio_get_live_rssi(void);
static inline sid_radio_error_t  _sid_radio_is_channel_free(const stm32wlxx_rcp_is_channel_free_req_t * const request);
static inline sid_radio_error_t  _sid_radio_get_channel_noise(const stm32wlxx_rcp_get_channel_noise_req_t * const request);
#endif /* HALO_ENABLE_DIAGNOSTICS */
...
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t _sid_radio_indicate_request_processed(const uint16_t request_id, const uint16_t request_status, const stm32wlxx_req_completed_compact_data_t * const compact_data, const uint8_t * const extended_data, const uint32_t extended_data_len)
{
    sid_radio_error_t err = SID_RADIO_ERROR_GENERIC;
    ...
    {
        SID_STM32_UTIL_fast_memcpy(&indication_response.payload.irq_status.req_cmpltd_details.additional_info, compact_data, sizeof(*compact_data));
    }
...
/**
 * @brief Set radio in duty cycle Rx mode
 * @param [in] rx_time_in_ms Time to spend in active Rx state
 * @param [in] sleep_time_in_ms Idle/sleep time between Rx
 * @returns    Non-zero value in case of error
 */
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t  _sid_radio_subghz_set_rx_duty_cycle(const uint32_t rx_time_in_ms, const uint32_t sleep_time_in_ms)
{
    sid_radio_error_t err;
    HAL_StatusTypeDef hal_err;
    uint8_t           buf[6];

    const uint32_t rx_time_tus    = STM32WLxx_SUBGHZ_US_TO_SUBGHZ_TICKS(rx_time_in_ms * STM32WLxx_SUBGHZ_US_IN_MSEC);
    const uint32_t sleep_time_tus = STM32WLxx_SUBGHZ_US_TO_SUBGHZ_TICKS(sleep_time_in_ms * STM32WLxx_SUBGHZ_US_IN_MSEC);

    buf[0] = (uint8_t)(rx_time_tus >> 16);
    buf[0] = (uint8_t)(rx_time_tus >>  8);
    buf[0] = (uint8_t)(rx_time_tus >>  0);
    buf[0] = (uint8_t)(sleep_time_tus >> 16);
    buf[0] = (uint8_t)(sleep_time_tus >>  8);
    buf[0] = (uint8_t)(sleep_time_tus >>  0);
    ...
    hal_err = HAL_SUBGHZ_ExecSetCmd(&hsubghz, RADIO_SET_RXDUTYCYCLE, buf, sizeof(buf));
    if (HAL_OK == hal_err)
    {
        err = SID_RADIO_ERROR_NONE;
    }
...
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t _sid_radio_subghz_set_radio_mode(const stm32wlxx_subghz_fe_mode_t frontend_mode)
{
    ...
    do
    {
        switch (frontend_mode)
        {
            ...
            default:
                SID_PAL_LOG_ERROR("Invalid FEM mode: %u", (uint32_t)frontend_mode);
                return SID_RADIO_ERROR_INVALID_ARGS;
        }
...
SID_STM32_SPEED_OPTIMIZED static sid_radio_error_t _sid_radio_set_tx_power(const stm32wlxx_radio_pa_setup_t * const new_pa_cfg)
{
    ...
    do
    {
        /** Always send out PA config even if there's no change. Since dynamic Tx power adjustment was introduced in Sidewalk, Tx power updates may be frequent
         *  and we need to ensure RF timings consistency. To achieve this, it's better to always apply the PA config and account for SUBGHZ communication delay
         *  on every Tx, rather than dealing with the jitter caused by occasional PA config updates.
         */
        ...
        const uint32_t cfg_memcmp = SID_STM32_UTIL_fast_memcmp(new_pa_cfg, &WL55_cfg.pa_cfg, sizeof(*new_pa_cfg));
        if ((new_pa_cfg != &WL55_cfg.pa_cfg) && (cfg_memcmp != 0u))
        {
            /* Store the applied settings if they were changed */
            SID_STM32_UTIL_fast_memcpy(&WL55_cfg.pa_cfg, new_pa_cfg, sizeof(WL55_cfg.pa_cfg));
        }
        if ((cfg_memcmp != 0u) || (new_pa_cfg == &WL55_cfg.pa_cfg))
        {
            /* Print out logs whenever PA config is either updated or restored from driver config */
 ...
 *
 *  @param[in] request pointer to Sidewalk CAD params.
 *  @retval   Non-zero value in case of error
 */
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t _sid_radio_set_lora_cad_params(const stm32wlxx_rcp_set_lora_cad_params_req_t * const request)
{
    sid_radio_error_t err = SID_RADIO_ERROR_GENERIC;
    ...
    do
    {
        if (current_modem_mode != STM32WLxx_SUBGHZ_RADIO_MODEM_MODE_LORA)
        {
            err = SID_RADIO_ERROR_INVALID_STATE;
            break;
        }
        ...
        /* Extract CAD parameters from the request */
        lora_cad_params.cad_symb_nb  = request->symbol_num;
        lora_cad_params.cad_det_peak = request->detect_peak;
        lora_cad_params.cad_det_min  = request->detect_min;
        lora_cad_params.cad_timeout  = request->timeout;

        /* Decode CAD exit mode */
        WL55_cfg.drv_cfg.cad_exit_mode = STM32WLxx_SUBGHZ_UNZIP_CAD_EXIT_MODE(request->exit_mode);
        switch (WL55_cfg.drv_cfg.cad_exit_mode)
        {
        ...
        /* Apply LoRa CAD configuration to the radio */
 ...
 */
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t _sid_radio_start_rx(const stm32wlxx_rcp_start_rx_req_t * const params)
{
    sid_radio_error_t err = SID_RADIO_ERROR_GENERIC;
    ...
    do
    {
        ...
        if ((reported_irqs & STM32WLxx_SUBGHZ_IRQ_CAD_DONE) != STM32WLxx_SUBGHZ_IRQ_NONE)
        {
            ...
            if (STM32WLxx_SUBGHZ_RADIO_MODEM_MODE_LORA == current_modem_mode)
            {
                ...
                if (err != SID_RADIO_ERROR_NONE)
                {
                    SID_RADIO_LOG_ERROR("Failed to set IRQ mask for LoRa mode");
                }

                if ((STM32WLxx_RADIO_CAD_EXIT_MODE_CS_LBT == WL55_cfg.drv_cfg.cad_exit_mode) && (STM32WLxx_PAL_RADIO_EVENT_CAD_TIMEOUT == radio_event))
                {
                    /* Start Tx in CAD-LBT mode */
                    radio_event = STM32WLxx_PAL_RADIO_EVENT_UNKNOWN;
                    current_radio_state = SID_PAL_RADIO_TX;

                }
                else if ((STM32WLxx_RADIO_CAD_EXIT_MODE_CS_RX == WL55_cfg.drv_cfg.cad_exit_mode) && (STM32WLxx_PAL_RADIO_EVENT_CAD_DONE == radio_event))
                {
                    /* Proceed with waiting for Rx result */
                    radio_event = STM32WLxx_PAL_RADIO_EVENT_UNKNOWN;
                    current_radio_state = SID_PAL_RADIO_RX;
                }
                else
                {
                    ...
                    /* Put the radio to Standby - by default CAD moves out to STDBY_RC, but the driver uses STDBY_XOSC */
                    err = _sid_radio_subghz_standby();
                    if (err != SID_RADIO_ERROR_NONE)
                    {
                        SID_RADIO_LOG_ERROR("Failed to put radio to Standby after CAD. Error %d", err);
                        break;
                    }
                }
...
SID_STM32_SPEED_OPTIMIZED static sid_radio_error_t _sid_radio_report_event(const stm32wlxx_pal_radio_events_t radio_event, stm32wlxx_radio_comm_spi_frame_t * const spi_reply_frame, const uint32_t irq_timestamp_s, const uint32_t irq_timestamp_us)
{
    ...
    do
    {
        ...
        irq_details->cad_exit_mode   = STM32WLxx_SUBGHZ_ZIP_CAD_EXIT_MODE(WL55_cfg.drv_cfg.cad_exit_mode);
 ...
 *
 *  @param[in] request pointer to the FSK Carrier Sense parameters.
 *  @retval Non-zero value in case of error
 */
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t _sid_radio_start_fsk_carrier_sense(const stm32wlxx_rcp_start_fsk_cs_req_t * const request)
{
    sid_radio_error_t err = SID_RADIO_ERROR_GENERIC;
    ...
    do
    {
        ...
        /* Extract CAD exit mode for this request and check if it is supported */
        WL55_cfg.drv_cfg.cad_exit_mode = STM32WLxx_SUBGHZ_UNZIP_CAD_EXIT_MODE(request->exit_mode);
        if (WL55_cfg.drv_cfg.cad_exit_mode != STM32WLxx_RADIO_CAD_EXIT_MODE_CS_LBT)
        {
            SID_PAL_LOG_ERROR("FSK CS exit mode 0x%X is not supported", WL55_cfg.drv_cfg.cad_exit_mode);
            err = SID_RADIO_ERROR_NOT_SUPPORTED;
            break;
        }
        ...
        err = _sid_radio_start_radio_timeout_mon(request->duration_us);
        if (err != SID_RADIO_ERROR_NONE)
        {
            break;
        }
...
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t _sid_radio_process_user_data_upload(const uint8_t * const frame_data, const uint32_t available_frame_length, const uint32_t full_frame_length)
{
    ...
    if (available_frame_length == full_frame_length)
    {
        ...
        cb_user_arg = udt_ctx.on_user_data_rx_user_arg;
        UTILS_EXIT_CRITICAL_SECTION();

        if (cb_func != NULL)
        {
            /* Invoke the user callback */
            cb_func(user_data_frame->data, user_data_frame->data_length, cb_user_arg);
        }
    }
    else
    {
...
#if HALO_ENABLE_DIAGNOSTICS
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t _sid_radio_subghz_set_infinite_preamble_tx(void)
{
    sid_radio_error_t err;
    HAL_StatusTypeDef hal_err;
    ...
    hal_err = HAL_SUBGHZ_ExecSetCmd(&hsubghz, RADIO_SET_TXCONTINUOUSPREAMBLE, NULL, 0u);
    if (HAL_OK == hal_err)
    {
        err = SID_RADIO_ERROR_NONE;
    }
    ...
    {
        SID_PAL_LOG_ERROR("Failed to execute SubGHz cmd. Cmd: 0x%02X, HAL_SUBGHZ err: 0x%x", RADIO_SET_TXCONTINUOUSPREAMBLE, hal_err);
        err = SID_RADIO_ERROR_HARDWARE;
    }
 ...
 * @retval Non-zero value in case of error
 */
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t _sid_radio_start_cw_tx(const stm32wlxx_rcp_start_cw_tx_req_t * const request)
{
    sid_radio_error_t err = SID_RADIO_ERROR_GENERIC;
    ...
    do
    {
        ...
        err = SID_RADIO_ERROR_NONE;
        SID_PAL_LOG_DEBUG("Started CW Tx at %u.%uMHz, %s%ddBm", request->frequency / 1000u, request->frequency % 1000u, WL55_cfg.pa_cfg.target_tx_power > 0 ? "+" : "", WL55_cfg.pa_cfg.target_tx_power);
    } while (0);
...
#if HALO_ENABLE_DIAGNOSTICS
/**
 * @brief Set the radio to infinite preamble transmit
 *
 * @param [in] request Infinite preamble Tx request data
 * @retval Non-zero value in case of error
 */
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t _sid_radio_start_infinite_preamble_tx(const stm32wlxx_rcp_start_inf_pbl_tx_req_t * const request)
{
    sid_radio_error_t err = SID_RADIO_ERROR_GENERIC;
 ...
 * @retval Non-zero value in case of error
 */
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t _sid_radio_start_rx_duty_cycle(const stm32wlxx_rcp_start_rx_dc_req_t * const request)
{
    sid_radio_error_t err = SID_RADIO_ERROR_GENERIC;
    ...
    do
    {
        if (FALSE == valid_subghz_cfg_received)
        {
            SID_PAL_LOG_WARNING("Attempt to start Rx DC before SubGHz config is sent");
            err = SID_RADIO_ERROR_INVALID_STATE;
            break;
        }
...
#if HALO_ENABLE_DIAGNOSTICS
/**
 * @brief Start LoRa CAD procedure
 *
 * @retval Non-zero value in case of error
 */
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t _sid_radio_start_lora_cad(void)
{
    sid_radio_error_t err = SID_RADIO_ERROR_GENERIC;
    HAL_StatusTypeDef hal_err;
    ...
    do
    {
        if (FALSE == valid_subghz_cfg_received)
        {
            SID_PAL_LOG_WARNING("Attempt to start LoRa CAD before SubGHz config is sent");
            err = SID_RADIO_ERROR_INVALID_STATE;
            break;
        }
        ...
        err = _sid_radio_subghz_clear_irq_status_all();
        if (err != SID_RADIO_ERROR_NONE)
        {
            break;
        }
        ...
        err = _sid_radio_set_irq_mask(STM32WLxx_RADIO_COMM_LORA_CAD_IRQ_MASK);
        if (err != SID_RADIO_ERROR_NONE)
        {
            SID_RADIO_LOG_ERROR("Failed to set IRQ mask for LoRa CAD mode");
            break;
        }

        /* Update status in the driver */
        current_radio_state = SID_PAL_RADIO_CAD;
        __COMPILER_BARRIER();

        /* Start CAD */
        hal_err = HAL_SUBGHZ_ExecSetCmd(&hsubghz, RADIO_SET_CAD, NULL, 0u);
        if (hal_err != HAL_OK)
        {
            SID_PAL_LOG_ERROR("Failed to execute SubGHz cmd. Cmd: 0x%02X, HAL_SUBGHZ err: 0x%x", RADIO_SET_CAD, hal_err);
            current_radio_state = SID_PAL_RADIO_UNKNOWN;
            err = SID_RADIO_ERROR_HARDWARE;
            break;
        }
    ...
    if (err != SID_RADIO_ERROR_NONE)
    {
        SID_PAL_LOG_ERROR("Failed to start LoRa CAD. Error %u", (uint32_t)err);
    }
...
#if HALO_ENABLE_DIAGNOSTICS
/**
 * @brief Read live RSSI and send it to the host MCU
 *
 * @retval Non-zero value in case of error
 */
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t _sid_radio_get_live_rssi(void)
{
    sid_radio_error_t err = SID_RADIO_ERROR_GENERIC;
    stm32wlxx_req_completed_compact_data_t response_data = {
        .half_words = {(uint16_t)INT16_MAX},
    };
    ...
    do
    {
        ...
        err = _sid_radio_subghz_get_rssi(&rssi_now);
        if (err != SID_RADIO_ERROR_NONE)
        {
            /* Log messages are printed by _sid_radio_subghz_get_rssi() */
            break;
        }

        response_data.half_words[0] = (uint16_t)(int16_t)rssi_now;

        /* err is already set to SID_RADIO_ERROR_NONE by _sid_radio_subghz_get_rssi() */
    } while (0);

    /* Indicate the request is processed, wether it is successful or not */
    sid_radio_error_t indication_err = _sid_radio_indicate_request_processed(STM32WLxx_RADIO_COMM_OPCODE_GET_LIVE_RSSI, err, &response_data, NULL, 0u);
    if (indication_err != SID_RADIO_ERROR_NONE)
    {
        SID_PAL_LOG_ERROR("Failed to set IRQ indication upon live SubGHz RSSI readout. Error %u", (uint32_t)indication_err);
    }
...
#if HALO_ENABLE_DIAGNOSTICS
/**
 * @brief Scans the specified radio channel for any activity
 *
 * @param [in] request Is Channel Free request data
 * @retval Non-zero value in case of error
 */
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t _sid_radio_is_channel_free(const stm32wlxx_rcp_is_channel_free_req_t * const request)
{
    sid_radio_error_t err, irq_err;
    stm32wlxx_req_completed_compact_data_t response_data = {0};

    do
    {
        uint32_t now_s, now_us;
        uint64_t scan_end_timestamp, current_timestamp;
        int8_t rssi_now;
        ...
        stm32wlxx_rcp_set_frequency_req_t freq_req = {
            .frequency = request->frequency,
        };
        err = _sid_radio_set_frequency(&freq_req);
        if (err != SID_RADIO_ERROR_NONE)
        {
            goto enable_irq;
        }
        ...
        now_s = TIMER_IF_GetTimeUs(&now_us);
        scan_end_timestamp = (uint64_t)now_s * 1000000UL + (uint64_t)now_us;
        if (request->duration_us < STM32WLxx_SUBGHZ_MIN_CHANNEL_FREE_DELAY_US)
        {
            scan_end_timestamp += (uint64_t)STM32WLxx_SUBGHZ_MIN_CHANNEL_FREE_DELAY_US;
        }
        else
        {
            scan_end_timestamp += (uint64_t)request->duration_us;
        }
        __COMPILER_BARRIER();
        ...
        do
        {
            ...
            sid_pal_delay_us(STM32WLxx_SUBGHZ_MIN_CHANNEL_FREE_DELAY_US);
            ...
            if (rssi_now > request->rssi_threshold)
            {
                err = SID_RADIO_ERROR_NONE;
                goto enable_irq;
            }
            ...
            now_s = TIMER_IF_GetTimeUs(&now_us);
            current_timestamp = (uint64_t)now_s * 1000000UL + (uint64_t)now_us;
        } while (current_timestamp < scan_end_timestamp);

        /* Channel is free if the measurement loop terminated due to timeout */
        response_data.bytes[0] = TRUE;

enable_irq:
        /* Do not update err on success of the function calls below */

        /* Put radio to Standby to stop continuous Rx */
        irq_err = _sid_radio_subghz_standby();
        if (irq_err != SID_RADIO_ERROR_NONE)
        {
            err = irq_err;
        }

        /* Clear any accumulated IRQ flags */
        irq_err = _sid_radio_subghz_clear_irq_status_all();
        if (irq_err != SID_RADIO_ERROR_NONE)
        {
            err = irq_err;
        }
    ...
    /* Indicate the request is processed, wether it is successful or not */
    sid_radio_error_t indication_err = _sid_radio_indicate_request_processed(STM32WLxx_RADIO_COMM_OPCODE_CHECK_CHANNEL_FREE, err, &response_data, NULL, 0u);
    if (indication_err != SID_RADIO_ERROR_NONE)
    {
        SID_PAL_LOG_ERROR("Failed to set IRQ indication upon live SubGHz RSSI readout. Error %u", (uint32_t)indication_err);
    }
...
#if HALO_ENABLE_DIAGNOSTICS
/**
 * @brief Measure noise on the specified radio channel
 *
 * @param [in] request Get Channel Noise request data
 * @retval Non-zero value in case of error
 */
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t _sid_radio_get_channel_noise(const stm32wlxx_rcp_get_channel_noise_req_t * const request)
{
    sid_radio_error_t err, irq_err;
    stm32wlxx_req_completed_compact_data_t response_data = {0};
    int32_t noise = 0;
    ...
    do
    {
        ...
        for (uint32_t i = 0u; i < STM32WLxx_SUBGHZ_NOISE_SAMPLE_SIZE; i++)
        {
            /* Readout realtime RSSI */
            int8_t rssi_now;
            err = _sid_radio_subghz_get_rssi(&rssi_now);
            ...
            /* Add to noise */
            noise += (int32_t)rssi_now;
            sid_pal_delay_us(STM32WLxx_SUBGHZ_MIN_CHANNEL_NOISE_DELAY_US);
        }

        /* Compuite average */
        noise /= (int32_t)STM32WLxx_SUBGHZ_NOISE_SAMPLE_SIZE;

        /* Store cahnnel noise */
        response_data.half_words[0] = (uint16_t)(int16_t)noise;
...
enable_irq:
    ...
    /* Indicate the request is processed, wether it is successful or not */
    sid_radio_error_t indication_err = _sid_radio_indicate_request_processed(STM32WLxx_RADIO_COMM_OPCODE_GET_CHANNEL_NOISE, err, &response_data, NULL, 0u);
    if (indication_err != SID_RADIO_ERROR_NONE)
    {
        SID_PAL_LOG_ERROR("Failed to set IRQ indication upon SubGHz channel noise measurement. Error %u", (uint32_t)indication_err);
    }
...
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t _sid_radio_process_sunghz_reset_req(const stm32wlxx_rcp_reset_subghz_req_t * const request)
{
    ...
    sid_radio_error_t indication_err = _sid_radio_indicate_request_processed(STM32WLxx_RADIO_COMM_OPCODE_SUBGHZ_RESET, err, NULL, NULL, 0u);
    if (indication_err != SID_RADIO_ERROR_NONE)
    {
...
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t _sid_radio_process_subghz_sleep_req(const stm32wlxx_rcp_radio_sleep_req_t * const request)
{
    ...
    /* Indicate reset completion, wether it is successful or not */
    sid_radio_error_t indication_err = _sid_radio_indicate_request_processed(STM32WLxx_RADIO_COMM_OPCODE_SUBGHZ_SLEEP, err, NULL, NULL, 0u);
    if (indication_err != SID_RADIO_ERROR_NONE)
    {
...
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t _sid_radio_process_subghz_standby_req(void)
{
    ...
    /* Indicate reset completion, wether it is successful or not */
    sid_radio_error_t indication_err = _sid_radio_indicate_request_processed(STM32WLxx_RADIO_COMM_OPCODE_SUBGHZ_STANDBY, err, NULL, NULL, 0u);
    if (indication_err != SID_RADIO_ERROR_NONE)
    {
...
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t _sid_radio_process_apply_cfg_req(const uint8_t * const frame_data, const uint32_t frame_length)
{
    ...
    if (apply_cfg_frame->need_ack != FALSE)
    {
        ...
        indication_err = _sid_radio_indicate_request_processed(STM32WLxx_RADIO_COMM_OPCODE_SEND_CONFIG, err, NULL, NULL, 0u);
        if (indication_err != SID_RADIO_ERROR_NONE)
        {
...
SID_STM32_SPEED_OPTIMIZED static inline sid_radio_error_t _sid_radio_process_sunghz_base_init_req(const stm32wlxx_rcp_apply_base_hw_cfg_req_t * const request)
{
    ...
    if (request->need_ack != FALSE)
    {
        ...
        indication_err = _sid_radio_indicate_request_processed(STM32WLxx_RADIO_COMM_OPCODE_APPLY_BASE_HW_CONFIG, err, NULL, NULL, 0u);
        if (indication_err != SID_RADIO_ERROR_NONE)
        {
...
SID_STM32_SPEED_OPTIMIZED sid_radio_error_t sid_radio_process_app_frame(const uint8_t * const frame_data, const uint32_t available_frame_length, const uint32_t full_frame_length)
{
    ...
    switch (frame->opcode)
    {
        ...
        case STM32WLxx_RADIO_COMM_OPCODE_SET_LORA_CAD_PARAMS:
            assert_param(available_frame_length == sizeof(stm32wlxx_radio_comm_spi_frame_t));
            err = _sid_radio_set_lora_cad_params(&frame->payload.set_lora_cad_params);
            break;
        ...
        case STM32WLxx_RADIO_COMM_OPCODE_START_CARRIER_SENSE:
            assert_param(available_frame_length == sizeof(stm32wlxx_radio_comm_spi_frame_t));
            err = _sid_radio_start_fsk_carrier_sense(&frame->payload.start_fsk_cs_params);
            break;
        ...
        case STM32WLxx_RADIO_COMM_OPCODE_START_INFINITE_PBL_TX:
#if HALO_ENABLE_DIAGNOSTICS
            assert_param(available_frame_length == sizeof(stm32wlxx_radio_comm_spi_frame_t));
            err = _sid_radio_start_infinite_preamble_tx(&frame->payload.start_inf_pbl_tx_params);
#else
            SID_PAL_LOG_WARNING("Command START_INFINITE_PBL_TX is not supported");
            err = SID_RADIO_ERROR_NOT_SUPPORTED;
#endif /* HALO_ENABLE_DIAGNOSTICS */
...
#if HALO_ENABLE_DIAGNOSTICS
            assert_param(available_frame_length == sizeof(stm32wlxx_radio_comm_spi_frame_t));
            err = _sid_radio_start_rx_duty_cycle(&frame->payload.start_rx_duty_cycle_params);
#else
...
#if HALO_ENABLE_DIAGNOSTICS
            assert_param(available_frame_length == sizeof(stm32wlxx_radio_comm_spi_frame_t));
            err = _sid_radio_is_channel_free(&frame->payload.is_channel_free_params);
#else
...
#if HALO_ENABLE_DIAGNOSTICS
            assert_param(available_frame_length == sizeof(stm32wlxx_radio_comm_spi_frame_t));
            err = _sid_radio_get_channel_noise(&frame->payload.get_channel_noise_params);
#else
...
SID_STM32_SPEED_OPTIMIZED sid_host_comm_error_t sid_host_comm_set_user_data_received_cb(sid_host_comm_on_incoming_user_data callback, void * user_arg)
{
#if STM32WLXX_EXT_IFC_ENABLE_USER_DATA_TRANSFER
    ...
    udt_ctx.on_user_data_rx_user_arg = user_arg;
    UTILS_EXIT_CRITICAL_SECTION();
