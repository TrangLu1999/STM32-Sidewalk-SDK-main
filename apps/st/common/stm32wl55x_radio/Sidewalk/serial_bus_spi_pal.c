...
#define SPI_RINGBUF_FRAME_PATTERN_SET(__start_addr__, __pattern__)  do\
                                                                    {\
...
/* SPI buffer pointers */
static volatile uint32_t tx_ringbuf_ingest_addr;
static volatile uint32_t rx_ringbuf_extract_addr;
...
static HAL_StatusTypeDef _hal_like_spi_instance_init(void)
{
    ...
    volatile uint32_t * const overrun_watermark_ptr = (volatile uint32_t *)(void *)overrun_check_addr;
...
SID_STM32_SPEED_OPTIMIZED static void _hal_like_spi_dma_transmit_cplt(DMA_HandleTypeDef * const hdma)
{
    ...
    if (FALSE == dma_dummy_tx_pending)
    {
        ...
        if (next_dma_src_addr == tx_ringbuf_ingest_addr_copy)
        {
             ...
             */
            SPI_RINGBUF_FRAME_PATTERN_SET(next_dma_src_addr, 0x00000000u);
        }
    ...
    else
    {
        ...
        if (prev_dma_src_addr != tx_ringbuf_ingest_addr_copy)
        {
...
SID_STM32_SPEED_OPTIMIZED serial_bus_spi_pal_err_t serial_bus_spi_clear_fifos(const uint32_t timeout_ms)
{
    ...
    do
    {
        ...
        const register uint32_t current_rx_cndtr_bytes = (DMA_MDATAALIGN_HALFWORD == SIDEWALK_RADIO_SPI.hdmarx->Init.MemDataAlignment) ? /* Capture the CNDTR register of the DMA Rx operation. We cannot use CMAR register since it's not updated in circular mode */
                                                         (SIDEWALK_RADIO_SPI.hdmarx->Instance->CNDTR * 2u) :                             /* DMA runs in half-word mode, adjust CNDTR value to bytes equivalent */
                                                         SIDEWALK_RADIO_SPI.hdmarx->Instance->CNDTR;                                     /* DMA runs in byte-by-byte mode, no adjustments to the CNDTR value is needed */
        ...
        /* Calculate the nearest CNDTR for the fully completed SPI frames. Note: CNDTR is a decrementing counter that shows the number of remaining transactions */
        const register uint32_t frame_aligned_cndtr_bytes = ((current_rx_cndtr_bytes + (SPI_FRAME_SIZE - 1u)) / SPI_FRAME_SIZE) * SPI_FRAME_SIZE; /* Round up since CNDTR is a decrementing counter */

        /* Compute the ingest pointer */
        current_rx_ingest_addr = rx_ringbuf_end_addr - (sizeof(uint8_t) * frame_aligned_cndtr_bytes);
        ...
        /* There's no active SPI transaction when this function is called, we can safely overwrite the extract and ingest pointers */
         ...
         *            It is essential to ensure this handler completes ASAP
         */
        SPI_RINGBUF_FRAME_PATTERN_SET(current_tx_pos, 0x00000000u);
        dma_dummy_tx_pending = TRUE;
...
SID_STM32_SPEED_OPTIMIZED uint32_t serial_bus_spi_get_tx_fifo_free_level(void)
{
    ...
    /* Ensure there's no systematic SW failure */
    assert_param((tx_ringbuf_ingest_addr_copy >= (uint32_t)&tx_ringbuf[0]) && (tx_ringbuf_ingest_addr_copy <= tx_ringbuf_end_addr));
    assert_param((current_tx_pos >= (uint32_t)&tx_ringbuf[0]) && (current_tx_pos <= tx_ringbuf_end_addr));
...
SID_STM32_SPEED_OPTIMIZED uint32_t serial_bus_spi_get_rx_fifo_level(void)
{
    ...
    /* Compute the expected location of the FIFO overrun guard watermark */
    register uint32_t               overrun_check_addr    = rx_ringbuf_extract_addr_copy;
    SPI_RINGBUF_REWIND_POSITION(overrun_check_addr, sizeof(SPI_RINGBUF_OVERRUN_DIAG_WATERMARK), (uint32_t)&rx_ringbuf[0], rx_ringbuf_end_addr);
    /* Cast the address to the appropriate pointer type */
    const volatile uint32_t * const overrun_watermark_ptr = (volatile uint32_t *)(void *)overrun_check_addr;
    /* Check the presence of the watermark */
...
SID_STM32_SPEED_OPTIMIZED serial_bus_spi_pal_err_t serial_bus_spi_pop_from_rx_fifo(uint8_t * const buf, const uint32_t buf_size, uint32_t * const bytes_copied)
{
    ...
    *bytes_copied = 0u;
    ...
    register uint32_t overrun_check_addr = rx_ringbuf_extract_addr;
    SPI_RINGBUF_REWIND_POSITION(overrun_check_addr, sizeof(SPI_RINGBUF_OVERRUN_DIAG_WATERMARK), (uint32_t)&rx_ringbuf[0], rx_ringbuf_end_addr);
    ...
    for (uint32_t i = 0u; i < frames_to_copy; i++)
    {
        ...
        SID_STM32_UTIL_fast_memcpy(buf_write_ptr, (void *)rx_ringbuf_extract_addr, SPI_FRAME_DATA_SIZE);
        __COMPILER_BARRIER();
         ...
         */
        volatile uint32_t * overrun_watermark_ptr = (volatile uint32_t *)(void *)overrun_check_addr;
        if (*overrun_watermark_ptr != SPI_RINGBUF_OVERRUN_DIAG_WATERMARK)
        {
        ...
        /* Adjust pointers */
        buf_write_ptr += SPI_FRAME_DATA_SIZE;                                                                                 /* Advance the store pointer by the frame payload size (excluding CRC bytes) */
        SPI_RINGBUF_ADVANCE_POSITION(rx_ringbuf_extract_addr, SPI_FRAME_SIZE, (uint32_t)&rx_ringbuf[0], rx_ringbuf_end_addr); /* Advance the source pointer by full SPI frame size, including CRC bytes */
        ...
        *bytes_copied += SPI_FRAME_DATA_SIZE;
        ...
        overrun_watermark_ptr  = (volatile uint32_t *)(void *)overrun_check_addr;
        *overrun_watermark_ptr = SPI_RINGBUF_OVERRUN_DIAG_WATERMARK;
    }
...
SID_STM32_SPEED_OPTIMIZED serial_bus_spi_pal_err_t serial_bus_spi_enqueue_tx(const uint8_t * const data, const uint32_t length)
{
    ...
    /* Capture the tx_ringbuf_ingest_addr value on entry */
    register uint32_t tx_ringbuf_ingest_addr_copy = tx_ringbuf_ingest_addr;
    ...
    do
    {
        ...
        if ((tx_ringbuf_ingest_addr_copy + length) <= tx_ringbuf_end_addr)
        {
        ...
        {
            direct_copy_length  = tx_ringbuf_end_addr - tx_ringbuf_ingest_addr_copy;
        ...
        if (direct_copy_length > 0u)
        {
            ...
            SID_STM32_UTIL_fast_memcpy((void *)tx_ringbuf_ingest_addr_copy, data, direct_copy_length);

            /* Advance the ingestion pointer and wrap it if necessary */
            tx_ringbuf_ingest_addr_copy += direct_copy_length;
            if (tx_ringbuf_ingest_addr_copy == tx_ringbuf_end_addr)
            {
                tx_ringbuf_ingest_addr_copy = (uint32_t)&tx_ringbuf[0];
            }
            else if (tx_ringbuf_ingest_addr_copy > tx_ringbuf_end_addr)
            {
        ...
        {
            SID_STM32_UTIL_fast_memcpy((void *)tx_ringbuf_ingest_addr_copy, (void *)((uint32_t)&data[0] + direct_copy_length), wrapped_copy_length);
            tx_ringbuf_ingest_addr_copy += wrapped_copy_length;
        }

        /* Add padding to complete the frame */
        register uint32_t direct_padding_length;
        register uint32_t wrapped_padding_length;
        if ((tx_ringbuf_ingest_addr_copy + padding_length) <= tx_ringbuf_end_addr)
        {
            /* All the pending data fits linearly into the reminder of the Tx ring buffer */
        ...
        {
            direct_padding_length  = tx_ringbuf_end_addr - tx_ringbuf_ingest_addr_copy;
        ...
        {
            SID_STM32_UTIL_fast_memset((void *)tx_ringbuf_ingest_addr_copy, 0x00u, direct_padding_length);

            /* Advance the ingestion pointer and wrap it if necessary */
            tx_ringbuf_ingest_addr_copy += direct_padding_length;
            if (tx_ringbuf_ingest_addr_copy == tx_ringbuf_end_addr)
            {
                tx_ringbuf_ingest_addr_copy = (uint32_t)&tx_ringbuf[0];
            }
        ...
        {
            SID_STM32_UTIL_fast_memset((void *)tx_ringbuf_ingest_addr_copy, 0x00u, wrapped_padding_length);
            tx_ringbuf_ingest_addr_copy += wrapped_padding_length;
        }

        /* Ensure there's no systematic SW failure */
        assert_param((tx_ringbuf_ingest_addr_copy >= (uint32_t)&tx_ringbuf[0]) && (tx_ringbuf_ingest_addr_copy <= tx_ringbuf_end_addr));
        assert_param(((tx_ringbuf_ingest_addr_copy - (uint32_t)&tx_ringbuf[0]) % SPI_FRAME_DATA_SIZE) == 0u);

        /* Store back the modified tx_ringbuf_ingest_addr */
        tx_ringbuf_ingest_addr = tx_ringbuf_ingest_addr_copy;
