...
> 
> In this repository, the middleware library listed below is **not available** as it is subject to some restrictive license terms requiring the user's approval via a "click thru" procedure.
> * `./platform/sid_mcu/st/stm32common/Middlewares/ST/STM32_Cryptographic`
> 
> It can be found inside the X-CUBE-CRYPTOLIB (v4.5.0) package available on our website `st.com` and downloadable from [here](https://www.st.com/en/embedded-software/x-cube-cryptolib.html#get-software). You will be prompted to login or to register in case you have no account.
---
...
The Sidewalk SDK is an integration of the Sidewalk SDK v1.19.4.20 provided by Amazon, specifically designed to run on the STM32WBAxx MCUs. This package offers a comprehensive set of tools and services to facilitate the development of Amazon Sidewalk applications.
...
|-------------|-------------------|
| [sid_900](/apps/st/stm32wba/sid_900/) | Demonstrates connectivity between the end node and the AWS Sidewalk cloud by initializing the radio, synchronizing, registering and sending a string. - Multiple radio configurations (BLE-FSK, BLE-FSK, BLE-LoRa, FSK-LoRa, FSK-only) - Starting point for Sidewalk application development |
| [sid_application_install_manager](/apps/st/stm32wba/sid_application_install_manager/) | A secure bootloader developed for the STMicroelectronics Sidewalk SDK |
| [sid_ble](/apps/st/stm32wba/sid_ble/) | A sample app similar to sid_900 but utilizing purely the BLE link. This app can run on a STM32WBAxx host without any external sub-GHz radio |
| [sid_demo](/apps/st/stm32wba/sid_demo/) | End to end example of the bi-directional Sidewalk communication with Web UI. More information can be found [here](https://github.com/aws-samples/aws-iot-core-for-amazon-sidewalk-sample-app)|
| [sid_diagnostics](/apps/st/stm32wba/sid_diagnostics/) | Radio evaluation, test and verify radio using CLI commands to the STM32WBAxx device. Application also used in the manufacturing provisioning process |
| [sid_dut](/apps/st/stm32wba/sid_dut/) | Sidewalk stack evaluation through CLI and Sidewalk APIs to test the Sidewalk solution. - Sidewalk API: radio init, connect, registration, reset - Send data over Sidewalk link |
| [sid_dut_ble_coexistence](/apps/st/stm32wba/sid_dut_ble_coexistence/) | Extension to the sid_dut app showcasing the BLE co-existence operation (regular BLE running concurrently with the Sidewalk link). - CLI: creation of virtual device, sending peripheral data |
| [sid_dut_udt](/apps/st/stm32wba/sid_dut_udt/) | Extension to the sid_dut app to demonstrate the User Data Transfer (UDT) feature of the STM32WBAxx + STM32WLx5 platform - showcases how the SPI link between the two MCUs can be used to transfer user/application data in-between Sidewalk-related frames |
| [sid_lr11xx_fw_updater](/apps/st/stm32wba/sid_lr11xx_fw_updater/) | Utility app to simplify firmware updates of the LR11xx transceivers. While built on Sidewalk platform, it is not intended for the cloud communication |
| [sid_lr11xx_geolocation](/apps/st/stm32wba/sid_lr11xx_geolocation/) | Showcases the usage of LR1110/LR1120 geolocation features together with the Sidewalk link |
| [sid_sbdt_demo](/apps/st/stm32wba/sid_sbdt_demo/) | Sidewalk Bulk Data Transfer demonstration - end to end OTA example that utilizes Sidewalk SBDT feature to download the firmware update image over the BLE link |
| [sid_teseo_cli](/apps/st/stm32wba/sid_teseo_cli/) | Utility app to provide direct command line access to Teseo GNSS modules, allows manual Teseo configuration and testing |
| [sid_teseo_geolocation](/apps/st/stm32wba/sid_teseo_geolocation/) | Showcases the usage of Teseo GNSS receivers together with the Sidewalk link |
...
Using chip config : (WBA65xI:STM32WBA65xI address: 0x81fe000)
