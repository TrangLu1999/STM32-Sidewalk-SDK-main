...
UTIL_ADV_TRACE_Status_t UTIL_ADV_TRACE_COND_FSend(uint32_t VerboseLevel, uint32_t Region, uint32_t TimeStampState, const char *strFormat, ...)
{
  ...
  // uint16_t idx;
// #else
#endif /* UTIL_ADV_TRACE_UNCHUNK_MODE */
  /*^^^ END OF PATCH STMC-485 ^^^*/
...
/*^^^ END OF PATCH STMC-485 ^^^*/
  ...
  if (TRACE_AllocateBufer((buff_size),&writepos) != -1)
  /*^^^ END OF PATCH STMC-485 ^^^*/
  {
#if defined(UTIL_ADV_TRACE_OVERRUN)
    ...
    /* copy the data */
    /*vvv PATCH STMC-485 vvvvvvvvvv*/
    SID_STM32_UTIL_fast_memcpy(&ADV_TRACE_Buffer[writepos], buf, buff_size);
...
UTIL_ADV_TRACE_Status_t UTIL_ADV_TRACE_COND_ZCSend_Allocation(uint32_t VerboseLevel, uint32_t Region, uint32_t TimeStampState, uint16_t length, uint8_t **pData, uint16_t *FifoSize, uint16_t *WritePos)
{
  ...
  if (TRACE_AllocateBufer(length+timestamp_size, &writepos) != -1)
  {
    ...
    // }
    SID_STM32_UTIL_fast_memcpy(&ADV_TRACE_Buffer[writepos], timestamp_ptr, timestamp_size);
    writepos += timestamp_size;
    /*^^^ END OF PATCH STMC-485 ^^^*/
...
UTIL_ADV_TRACE_Status_t UTIL_ADV_TRACE_COND_Send(uint32_t VerboseLevel, uint32_t Region, uint32_t TimeStampState, const uint8_t *pData, uint16_t Length)
{
  ...
  if (TRACE_AllocateBufer(Length + timestamp_size, &writepos) != -1)
  {
    ...
    //   writepos = (uint16_t) ((writepos + 1u) % UTIL_ADV_TRACE_FIFO_SIZE);
    // }
    SID_STM32_UTIL_fast_memcpy(&ADV_TRACE_Buffer[writepos], pData, Length);
    /*^^^ END OF PATCH STMC-485 ^^^*/
...
void UTIL_ADV_TRACE_GetBufferInfo(const uint8_t * * const pData, uint16_t * const TraceRdPtr, uint16_t * const TraceWrPtr)
{
  UTIL_ADV_TRACE_ENTER_CRITICAL_SECTION();
  *pData = ADV_TRACE_Buffer;
  *TraceRdPtr = ADV_TRACE_Ctx.TraceRdPtr;
  *TraceWrPtr = ADV_TRACE_Ctx.TraceWrPtr;
  UTIL_ADV_TRACE_EXIT_CRITICAL_SECTION();
}
