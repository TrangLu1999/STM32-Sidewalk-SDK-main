...
/*vvv PATCH STMC-474 vvvvvvvvvv*/
#include <math.h>
/*^^^ END OF PATCH STMC-474 ^^^*/
...
/* Private functions ---------------------------------------------------------*/
/*vvv PATCH STMC-474 vvvvvvvvvv*/
static int32_t NMEA_CheckTalkerIdHeader(const char header[], const char msgId[]);
static int32_t NMEA_CheckGGAMsg(const char header[]);
static int32_t NMEA_CheckGSTMsg(const char header[]);
static int32_t NMEA_CheckRMCMsg(const char header[]);
static int32_t NMEA_CheckGLLMsg(const char header[]);
static int32_t NMEA_CheckVTGMsg(const char header[]);
/*^^^ END OF PATCH STMC-474 ^^^*/
...
/*vvv PATCH STMC-474 vvvvvvvvvv*/
static uint8_t nmea_status(const uint8_t status_char);
/*^^^ END OF PATCH STMC-474 ^^^*/
...
/*
 * Function that checks the header of NMEA sentence that starts with talker ID.
 */
static int32_t NMEA_CheckTalkerIdHeader(const char header[], const char msgId[])
{
  static const char talkerIds[][3] = {
    "GP", "GA", "BD", "QZ", "GN", "GL"
  };
  int32_t is_valid = 1;
  ...
  do
  {
    ...
    if (strcmp(msgId, &header[strlen(header) - strlen(msgId)]) != 0)
    {
      break;
    }
    for (uint32_t i = 0u; i < sizeof(talkerIds) / sizeof(talkerIds[0]); i++)
    {
      if (strncmp(&header[1], talkerIds[i], strlen(talkerIds[i])) == 0)
      {
        if (strlen(header) == (strlen(msgId) + strlen(talkerIds[i]) + 1u))
        {
          is_valid = 0;
          break;
        }
...
/*
 * Function that checks the header of a GGA NMEA sentence.
 */
static int32_t NMEA_CheckGGAMsg(const char header[])
{
  int32_t is_ggamsg = 1;

  is_ggamsg = NMEA_CheckTalkerIdHeader(header, "GGA");

  return is_ggamsg;
}

/*
 * Function that checks the header of a GST NMEA sentence.
 */
static int32_t NMEA_CheckGSTMsg(const char header[])
{
  int32_t is_gstmsg = 1;

  is_gstmsg = NMEA_CheckTalkerIdHeader(header, "GST");

  return is_gstmsg;
}

/*
 * Function that checks the header of a RMC NMEA sentence.
 */
static int32_t NMEA_CheckRMCMsg(const char header[])
{
  int32_t is_rmcmsg = 1;

  is_rmcmsg = NMEA_CheckTalkerIdHeader(header, "RMC");

  return is_rmcmsg;
}

/*
 * Function that checks the header of a GLL NMEA sentence.
 */
static int32_t NMEA_CheckGLLMsg(const char header[])
{
  int32_t is_gllmsg = 1;

  is_gllmsg = NMEA_CheckTalkerIdHeader(header, "GLL");

  return is_gllmsg;
}

/*
 * Function that checks the header of a VTG NMEA sentence.
 */
static int32_t NMEA_CheckVTGMsg(const char header[])
{
  int32_t is_gllmsg = 1;

  is_gllmsg = NMEA_CheckTalkerIdHeader(header, "VTG");
...
static int32_t NMEA_CheckGNSMsg(const char header[])
{
  ...
  //   is_gnsmsg = 0;
  // }
  is_gnsmsg = NMEA_CheckTalkerIdHeader(header, "GNS");
  /*^^^ END OF PATCH STMC-474 ^^^*/
...
static int32_t NMEA_CheckGSAMsg(const char header[])
{
  ...
  //   is_gsamsg = 0;
  // }
  is_gsamsg = NMEA_CheckTalkerIdHeader(header, "GSA");
  /*^^^ END OF PATCH STMC-474 ^^^*/
...
static int32_t NMEA_CheckGSVMsg(const char header[])
{
  ...
  //   is_gsvmsg = 0;
  // }
  is_gsvmsg = NMEA_CheckTalkerIdHeader(header, "GSV");
  /*^^^ END OF PATCH STMC-474 ^^^*/
  ...
  if(NMEA != NULL) {
    ...
    for(int32_t i = 0, j = 0, k = 0; (NMEA[i] != (uint8_t)'\n'); i++)
    {
      ...
      if ((NMEA[i] == (uint8_t)',') || (NMEA[i] == (uint8_t)'*'))
      {
        ...
        if (NMEA_CheckGGAMsg((char *)app[0]) == 0)
        {
        /*^^^ END OF PATCH STMC-474 ^^^*/
          j++;
          k = 0;
    ...
    if (valid_msg == TRUE)
    {
      ...
      /*vvv PATCH STMC-474 vvvvvvvvvv*/
      pGPGGAInfo->valid = strtol((char *)app[6], NULL, BASE);
      /*^^^ END OF PATCH STMC-474 ^^^*/
  ...
  if(NMEA != NULL) {
    ...
    for (int32_t i = 0, j = 0, k = 0; (NMEA[i] != (uint8_t)'\n'); i++)
    {
      ...
      if ((NMEA[i] == (uint8_t)',') || (NMEA[i] == (uint8_t)'*'))
      {
        ...
        if (NMEA_CheckGSTMsg((char *)app[0]) == 0)
        /*^^^ END OF PATCH STMC-474 ^^^*/
        {
          j++;
          k = 0;
  ...
  if(NMEA != NULL)
  {
    ...
    for (int32_t i = 0, j = 0, k = 0; (NMEA[i] != (uint8_t)'\n'); i++)
    {
      ...
      if ((NMEA[i] == (uint8_t)',') || (NMEA[i] == (uint8_t)'*'))
      {
        ...
        // if (strcmp((char *)app[0], "$GPRMC") == 0 || strcmp((char *)app[0], "$GNRMC") == 0)
        if (NMEA_CheckRMCMsg((char *)app[0]) == 0)
        /*^^^ END OF PATCH STMC-474 ^^^*/
        {
          j++;
          k = 0;
...
ParseStatus_t NMEA_ParseGLL(GLL_Info_t *pGLLInfo, uint8_t NMEA[])
{
  int32_t new_field;
  BOOL valid_msg = FALSE;
  ...
  if(NMEA != NULL) {
    ...
    for(int32_t i = 0, j = 0, k = 0; (NMEA[i] != (uint8_t)'\n'); i++)
    {
      ...
      if ((NMEA[i] == (uint8_t)',') || (NMEA[i] == (uint8_t)'*'))
      {
        ...
        if (NMEA_CheckGLLMsg((char *)app[0]) == 0)
        {
          j++;
          k = 0;
          valid_msg = TRUE;
    ...
    if (valid_msg == TRUE)
    {
      pGLLInfo->xy.lat = strtod((char *)app[1], NULL);
      pGLLInfo->xy.ns = *((uint8_t*)app[2]);
      pGLLInfo->xy.lon = strtod((char *)app[3], NULL);
      pGLLInfo->xy.ew = *((uint8_t*)app[4]);
      scan_utc(app[5], &pGLLInfo->utc);
      pGLLInfo->valid = nmea_status(*((uint8_t*)app[6]));
      if (*((uint8_t*)app[8]) != '\0')
      {
        pGLLInfo->mode = *((uint8_t*)app[7]);
        pGLLInfo->checksum = nmea_checksum(app[8]);
      }
      else
      {
        pGLLInfo->mode = 'N';
        pGLLInfo->checksum = nmea_checksum(app[7]);
      }

      /* Unsued fields in coordinates */
      pGLLInfo->xy.alt = NAN;
      pGLLInfo->xy.mis = 'M';
...
ParseStatus_t NMEA_ParseVTG(VTG_Info_t *pVTGInfo, uint8_t NMEA[])
{
  int32_t new_field;
  BOOL valid_msg = FALSE;
  ...
  if(NMEA != NULL) {
    ...
    for(int32_t i = 0, j = 0, k = 0; (NMEA[i] != (uint8_t)'\n'); i++)
    {
      ...
      if ((NMEA[i] == (uint8_t)',') || (NMEA[i] == (uint8_t)'*'))
      {
        ...
        if (NMEA_CheckVTGMsg((char *)app[0]) == 0)
        {
          j++;
          k = 0;
          valid_msg = TRUE;
    ...
    if (valid_msg == TRUE)
    {
      if (*((uint8_t*)app[1]) != '\0')
      {
        pVTGInfo->true_trackgood = strtof((char *)app[1], NULL);
      }
      else
      {
        pVTGInfo->true_trackgood = NAN;
      }
      if (*((uint8_t*)app[3]) != '\0')
      {
        pVTGInfo->magnetic_trackgood = strtof((char *)app[3], NULL);
      }
      else
      {
        pVTGInfo->magnetic_trackgood = NAN;
      }
      pVTGInfo->speed_knots = strtof((char *)app[5], NULL);
      pVTGInfo->speed_kmph = strtof((char *)app[7], NULL);
      if (*((uint8_t*)app[10]) != '\0')
      {
        pVTGInfo->mode = *((uint8_t*)app[9]);
        pVTGInfo->checksum = nmea_checksum(app[10]);
      }
      else
      {
        pVTGInfo->mode = 'N';
        pVTGInfo->checksum = nmea_checksum(app[9]);
      }
...
ParseStatus_t NMEA_ParsePSTMCPU(PSTMCPU_Info_t *pSTMCPU, uint8_t NMEA[])
{
  int32_t new_field;
  BOOL valid_msg = FALSE;
  ...
  if(NMEA != NULL)
  {
    ...
    for(int32_t i = 0, j = 0, k = 0; (NMEA[i] != (uint8_t)'\n'); i++)
    {
      ...
      if ((NMEA[i] == (uint8_t)',') || (NMEA[i] == (uint8_t)'*'))
      {
        ...
        if (strcmp((char *)app[0], "$PSTMCPU") == 0)
        {
          j++;
          k = 0;
          valid_msg = TRUE;
    ...
    if (valid_msg == TRUE)
    {
      pSTMCPU->cpu_load = strtof((char *)app[1], NULL);
      pSTMCPU->cpu_clock = strtoul((char *)app[3], NULL, BASE);
...
ParseStatus_t NMEA_ParsePSTMSetPar(PSTMSETPAR_Info_t *pSTMSETPAR, uint8_t NMEA[])
{
  int32_t new_field;
  BOOL valid_msg = FALSE;
  ...
  if(NMEA != NULL)
  {
    ...
    for(int32_t i = 0, j = 0, k = 0; (NMEA[i] != (uint8_t)'\n'); i++)
    {
      ...
      if ((NMEA[i] == (uint8_t)',') || (NMEA[i] == (uint8_t)'*'))
      {
        ...
        if ((strcmp((char *)app[0], "$PSTMSETPAR") == 0) || (strcmp((char *)app[0], "$PSTMSETPAROK") == 0) || (strcmp((char *)app[0], "$PSTMGETPARERROR") == 0) || (strcmp((char *)app[0], "$PSTMSETPARERROR") == 0))
        {
          j++;
          k = 0;
          valid_msg = TRUE;
    ...
    if (valid_msg == TRUE)
    {
      ...
      {
        uint32_t cdb_full_id =  strtoul((char *)app[1], NULL, BASE);
        if (0u == cdb_full_id)
        {
          return PARSE_FAIL;
        }
        pSTMSETPAR->config_block = cdb_full_id / 1000u;
        pSTMSETPAR->param_id = cdb_full_id % 1000u;
        if (*((uint8_t*)app[4]) != '\0')
        {
          uint32_t i = 0u;
          memset(pSTMSETPAR->value.bytes, 0u, sizeof(pSTMSETPAR->value.bytes));
          while ((*((uint8_t*)app[i+2]) != '\0') && (i < sizeof(pSTMSETPAR->value.bytes)))
          {
            pSTMSETPAR->value.bytes[i] = (uint8_t)strtoul((char *)app[i+2], NULL, BASE);
            i++;
          }
        }
        else if ((app[2][0] != '0') && (app[2][1] != 'x'))
        {
          char * stop_pos;
          pSTMSETPAR->value.f64 = strtod((char *)app[2], &stop_pos);
          if (*stop_pos != '\0')
          {
            memset(pSTMSETPAR->value.str, '\0', sizeof(pSTMSETPAR->value.str));
            memcpy(pSTMSETPAR->value.str, app[2], strlen((char *)app[2]));
          }
        }
        else
        {
          uint8_t * pos = &app[2][2];
          pSTMSETPAR->value.u64 = 0u;
          while (*pos != '\0')
          {
            uint8_t halfbyte_val = char2int(*pos);
            if ((0u == halfbyte_val) && (*pos != '0'))
            {
              break;
            }
            pSTMSETPAR->value.u64 = (pSTMSETPAR->value.u64 << 4) | (uint64_t)halfbyte_val;
            pos++;
          }
          if (*pos != '\0')
          {
        ...
        pSTMSETPAR->result = GNSS_OP_OK;
      }
      else if (strcmp((char *)app[0], "$PSTMSETPAROK") == 0)
      {
        uint32_t cdb_full_id =  strtoul((char *)app[1], NULL, BASE);
        if (0u == cdb_full_id)
        {
      ...
      {
        pSTMSETPAR->config_block = 0u;
        pSTMSETPAR->param_id = 0u;
        pSTMSETPAR->result = GNSS_OP_ERROR;
      }
  ...
  if(NMEA != NULL)
  {
    ...
    for(int32_t i = 0, j = 0, k = 0; (NMEA[i] != (uint8_t)'\n'); i++)
    {
      ...
      if ((NMEA[i] == (uint8_t)',') || (NMEA[i] == (uint8_t)'*'))
      {
        ...
        if ((strcmp((char *)app[0], "$PSTMRESTOREPAROK") == 0) || (strcmp((char *)app[0], "$PSTMRESTOREPARERROR") == 0))
        {
          j++;
          k = 0;
          valid_msg = TRUE;
    ...
    if (valid_msg == TRUE)
    {
      ...
      else if (strcmp((char *)app[0], "$PSTMRESTOREPARERROR") == 0)
      {
        *pResult = GNSS_OP_ERROR;
      }
      else
...
ParseStatus_t NMEA_ParsePSTMGPSSuspend(OpResult_t *pResult, uint8_t NMEA[])
{
  int32_t new_field;
  BOOL valid_msg = FALSE;
  ...
  if(NMEA != NULL)
  {
    ...
    for(int32_t i = 0, j = 0, k = 0; (NMEA[i] != (uint8_t)'\n'); i++)
    {
      ...
      if ((NMEA[i] == (uint8_t)',') || (NMEA[i] == (uint8_t)'*'))
      {
        ...
        if (strcmp((char *)app[0], "$PSTMGPSSUSPENDED") == 0)
        {
          j++;
          k = 0;
          valid_msg = TRUE;
...
ParseStatus_t NMEA_ParsePSTMForceStandby(OpResult_t *pResult, uint8_t NMEA[])
{
  int32_t new_field;
  BOOL valid_msg = FALSE;
  ...
  if(NMEA != NULL)
  {
    ...
    for(int32_t i = 0, j = 0, k = 0; (NMEA[i] != (uint8_t)'\n'); i++)
    {
      ...
      if ((NMEA[i] == (uint8_t)',') || (NMEA[i] == (uint8_t)'*'))
      {
        ...
        if ((strcmp((char *)app[0], "$PSTMFORCESTANDBYOK") == 0) || (strcmp((char *)app[0], "$PSTMFORCESTANDBYERROR") == 0))
        {
          j++;
          k = 0;
          valid_msg = TRUE;
    ...
    if (valid_msg == TRUE)
    {
      if (strcmp((char *)app[0], "$PSTMFORCESTANDBYOK") == 0)
      {
        *pResult = GNSS_OP_OK;
      }
      else if (strcmp((char *)app[0], "$PSTMFORCESTANDBYERROR") == 0)
      {
        *pResult = GNSS_OP_ERROR;
      }
      else
...
ParseStatus_t NMEA_ParsePSTMGetUCode(PSTMVER_Info_t *pPSTMUCODE, uint8_t NMEA[])
{
  ParseStatus_t status = PARSE_FAIL;

  if(NMEA != NULL)
  {
    if (strncmp((char *)NMEA, "$PSTMGETUCODE", strlen("$PSTMGETUCODE")) == 0)
    {
      uint8_t * store_ptr = &pPSTMUCODE->pstmver_string[0];
      uint8_t * read_ptr = &NMEA[strlen("$PSTMGETUCODE") + 1u];
      for (uint32_t i = 0u; i < (sizeof(pPSTMUCODE->pstmver_string) - 1u); i++)
      {
...
ParseStatus_t NMEA_ParseGPTXT(GPTXT_Info_t *pGPTXT, uint8_t NMEA[])
{
  ParseStatus_t status = PARSE_FAIL;

  if(NMEA != NULL)
  {
    if (strncmp((char *)NMEA, "$GPTXT", strlen("$GPTXT")) == 0)
    {
      char * store_ptr = &pGPTXT->log_msg[0];
      uint8_t * read_ptr = &NMEA[strlen("$GPTXT") + 1u];
      for (uint32_t i = 0u; i < (sizeof(pGPTXT->log_msg) - 1u); i++)
      {
        if (('*' == *read_ptr) || (',' == *read_ptr) || ('\r' == *read_ptr) || ('\n' == *read_ptr) || ('\0' == *read_ptr))
        {
          break;
        }
        *store_ptr = (char)(*read_ptr);
        store_ptr++;
        read_ptr++;
      }
      *store_ptr = '\0';
