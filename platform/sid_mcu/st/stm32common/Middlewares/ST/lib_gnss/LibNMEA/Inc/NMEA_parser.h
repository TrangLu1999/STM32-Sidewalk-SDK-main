...
/**
 * @brief Maximum size (including the '\0') of a GPTXT string that can be processed
 */
#define MAX_GPTXT_MSG_LEN (72u)
...
/**
 * @brief Data structure that contains latitude-longitude information
 */
typedef struct {
  Coords_t xy;            /**< Coords data member */
  UTC_Info_t utc;         /**< UTC Time */
  uint8_t valid;          /**< GPS fix status */
  uint8_t mode;           /**< GPS Signal fix quality */
  uint32_t checksum;      /**< Checksum of the message bytes */
} GLL_Info_t;
 ...
 */
typedef struct {
  float32_t true_trackgood;     /**< True course made good (referencing the true north pole) */
  float32_t magnetic_trackgood; /**< Magnetic course made good (referencing the magnetic north pole) */
  float32_t speed_knots;        /**< Speed over ground in knots */
  float32_t speed_kmph;         /**< Speed over ground in km/h */
  uint8_t  mode;                /**< GPS Signal fix quality */
  uint32_t checksum;            /**< Checksum of the message bytes */
} VTG_Info_t;

/**
 * @brief Data structure that contains CPU status information
 */
typedef struct {
  float32_t cpu_load;  /**< CPU load percentage */
  uint16_t  cpu_clock; /**< CPU clock speed in MHz */
} PSTMCPU_Info_t;

/**
 * @brief Data structure that contains CBD parameter value from $PSTMSETPAR response to $SPSTMGETPAR command
 */
typedef struct {
  OpResult_t  result;
  uint8_t     config_block;
  uint16_t    param_id;
  union {
    uint64_t  u64;
    float64_t f64;
...
/**
 * @brief Data structure that contains GPTXT log message
 */
typedef struct {
  char log_msg[MAX_GPTXT_MSG_LEN]; /**< String containing the log message */
} GPTXT_Info_t;
/*^^^ END OF PATCH STMC-474 ^^^*/
...
/*vvv PATCH STMC-474 vvvvvvvvvv*/
/**
 * @brief  Function that makes the parsing of the $--GLL NMEA string.
 * @param  pGLLInfo      Pointer to a GLL_Info_t struct
 * @param  NMEA	         NMEA string read by the Gps expansion.
 * @retval PARSE_SUCC if the parsing process goes ok, PARSE_FAIL if it doesn't
 */
...
/**
 * @brief  Function that makes the parsing of the $--VTG NMEA string.
 * @param  pGLLInfo      Pointer to a VTG_Info_t struct
 * @param  NMEA	         NMEA string read by the Gps expansion.
 * @retval PARSE_SUCC if the parsing process goes ok, PARSE_FAIL if it doesn't
 */
...
/**
 * @brief  This function parses the CPU usage message
 * @param  pSTMCPU             Pointer to data structure to store CPU usage data
 * @param  NMEA                NMEA string read by the Gps expansion.
 * @retval PARSE_SUCC if the parsing process goes ok, PARSE_FAIL if it doesn't
 */
...
/**
 * @brief  This function parses the $PSTMSETPAR response to $PSTMGETPAR command
 * @param  pSTMCPU             Pointer to data structure to store PSTMSETPAR response
 * @param  NMEA                NMEA string read by the Gps expansion.
 * @retval PARSE_SUCC if the parsing process goes ok, PARSE_FAIL if it doesn't
 */
...
/**
 * @brief  This function parses the RestorePar messages
 * @param  pResult             Ack from Teseo
 * @param  NMEA                NMEA string read by the Gps expansion.
 * @retval PARSE_SUCC if the parsing process goes ok, PARSE_FAIL if it doesn't
 */
...
/**
 * @brief  This function parses the PSTMGPSSUSPEND response messages
 * @param  pResult             Ack from Teseo
 * @param  NMEA                NMEA string read by the Gps expansion.
 * @retval PARSE_SUCC if the parsing process goes ok, PARSE_FAIL if it doesn't
 */
...
/**
 * @brief  This function parses the PSTMFORCESTANDBY response messages
 * @param  pResult             Ack from Teseo
 * @param  NMEA                NMEA string read by the Gps expansion.
 * @retval PARSE_SUCC if the parsing process goes ok, PARSE_FAIL if it doesn't
 */
...
/**
 * @brief  This function parses the PSTMGETUCODE response messages
 * @param  pPSTMUCODE          Pointer to Teseo_UCode_t storage
 * @param  NMEA                NMEA string read by the Gps expansion.
 * @retval PARSE_SUCC if the parsing process goes ok, PARSE_FAIL if it doesn't
 */
...
/**
 * @brief  This function parses the $GPTXT NMEA string with log message.
 * @param  pGPTXT              Pointer to data structure to store GPTXT data
 * @param  NMEA                NMEA string read by the Gps expansion.
 * @retval PARSE_SUCC if the parsing process goes ok, PARSE_FAIL if it doesn't
 */
