...
GNSSParser_Status_t GNSS_PARSER_ParseMsg(GNSSParser_Data_t *pGNSSParser_Data, uint8_t msg, uint8_t *pBuffer)
{
  ...
  case GLL:
    status = NMEA_ParseGLL(&pGNSSParser_Data->gll_data, pBuffer);
    break;
  case VTG:
    status = NMEA_ParseVTG(&pGNSSParser_Data->vtg_data, pBuffer);
    break;
  case PSTMCPU:
    status = NMEA_ParsePSTMCPU(&pGNSSParser_Data->pstmcpu_data, pBuffer);
    break;
  case GPTXT:
    status = NMEA_ParseGPTXT(&pGNSSParser_Data->gptxt_data, pBuffer);
    break;
  case PSTMSETPAR:
    status = NMEA_ParsePSTMSetPar(&pGNSSParser_Data->pstmsetpar_data, pBuffer);
    break;
  case PSTMRESTOREPAR:
    status = NMEA_ParsePSTMRestorePar(&pGNSSParser_Data->result, pBuffer);
    break;
  case PSTMGPSSUSPEND:
    status = NMEA_ParsePSTMGPSSuspend(&pGNSSParser_Data->result, pBuffer);
    break;
  case PSTMFORCESTANDBY:
    status = NMEA_ParsePSTMForceStandby(&pGNSSParser_Data->result, pBuffer);
    break;
  case PSTMGETUCODE:
    status = NMEA_ParsePSTMGetUCode(&pGNSSParser_Data->pstmver_data, pBuffer);
    break;
