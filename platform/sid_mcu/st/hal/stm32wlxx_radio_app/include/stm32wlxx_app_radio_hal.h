/**
  ******************************************************************************
  * @file    stm32wlxx_app_radio_hal.h
...
#ifndef __STM32WLXX_APP_RADIO_HAL_H_
#define __STM32WLXX_APP_RADIO_HAL_H_

/* Includes ------------------------------------------------------------------*/

#include "stm32wlxx_app_radio_common_defs.h"
#include <comm_def.h>
 ...
 *
 * @param [in] drv_ctx    Host MCU's radio driver context
 * @param [in] cad_params LoRa CAD parameters to be applied by STM32WLxx
 * @returns Operation status
 */
stm32wlxx_hal_status_t stm32wlxx_hal_radio_set_lora_cad_params(const halo_drv_stm32wlxx_ctx_t * const drv_ctx, const sid_pal_radio_lora_cad_params_t * const cad_params);
 ...
 *
 * @param [in] drv_ctx Host MCU's radio driver context
 * @param [in] cad_params FSK Carrier Sense parameters to be applied during the scan
 * @param [in] exit_mode Desired Carrier Sense exit mode
 * @returns    Operation status
 */
stm32wlxx_hal_status_t stm32wlxx_hal_radio_start_carrier_sense(const halo_drv_stm32wlxx_ctx_t * const drv_ctx, const sid_pal_radio_fsk_cad_params_t * const cad_params, const sid_pal_radio_cad_param_exit_mode_t exit_mode);
...
/**
 * @brief Call PAL layer start continuous preamble tx
 *
 * @param [in] drv_ctx Host MCU's radio driver context
 * @param [in] freq    Radio frequency to use for Tx
 * @returns    Operation status
 */
stm32wlxx_hal_status_t stm32wlxx_hal_start_infinite_preamble_tx(const halo_drv_stm32wlxx_ctx_t * const drv_ctx, const uint32_t freq);
...
/**
 * @brief Call PAL layer start rx duty cycle
 *
 * @param [in] drv_ctx Host MCU's radio driver context
 * @param [in] rx_time Time to spend in Rx state
 * @param [in] sleep_time Sleep time between Rx
 * @returns    Operation status
 */
stm32wlxx_hal_status_t stm32wlxx_hal_start_rx_duty_cycle(const halo_drv_stm32wlxx_ctx_t * const drv_ctx, const uint32_t rx_time, const uint32_t sleep_time);

/**
 * @brief Call PAL layer start continuous rx
 *
 * @param [in]  drv_ctx Host MCU's radio driver context
 * @param [out] out_rssi Captured RSSI value will be stored here
 * @returns     Operation status
 */
stm32wlxx_hal_status_t stm32wlxx_hal_get_live_rssi(halo_drv_stm32wlxx_ctx_t * const drv_ctx, int16_t * const out_rssi);

/**
 * @brief Call PAL layer start continuous rx
 *
 * @param [in]  drv_ctx Host MCU's radio driver context
 * @param [in]  freq Channel center frequency in Hz
 * @param [in]  threshold RSSI threshold to consider channel busy (any RSSI above thethreshold means the channel is occupied)
 * @param [in]  scan_duration_us Measurement duration in microseconds. The channel is continuously sampled during this period. If any sample reports RSSI above threshold the channel is considered busy
 * @param [out] is_channel_free Channel status will be stored here. Set to true if the channel is free
 * @returns     Operation status
 */
stm32wlxx_hal_status_t stm32wlxx_hal_is_channel_free(halo_drv_stm32wlxx_ctx_t * const drv_ctx, uint32_t freq, int16_t threshold, uint32_t scan_duration_us, bool * is_channel_free);

/**
 * @brief Call PAL layer Get Channel Noise
 *
 * @param [in]  drv_ctx Host MCU's radio driver context
 * @param [in]  freq Channel center frequency in Hz
 * @param [out] out_noise Noise level in dBi will be stored here
 * @returns     Operation status
 */
stm32wlxx_hal_status_t stm32wlxx_hal_get_channel_noise(halo_drv_stm32wlxx_ctx_t * const drv_ctx, uint32_t freq, int16_t * const out_noise);

/**
 * @brief Call PAL layer Start LoRa CAD
 *
 * @param [in]  drv_ctx Host MCU's radio driver context
 * @returns     Operation status
 */
stm32wlxx_hal_status_t stm32wlxx_hal_radio_start_lora_cad(const halo_drv_stm32wlxx_ctx_t * const drv_ctx);
...
/**
 * @brief Initializes MCU GPIO pins used by the radio driver
 *
 * @param [in] drv_ctx  Host MCU's radio driver context
 * @returns    Operation status
 */
...
/**
 * @brief De-initializes MCU GPIO pins used by the radio driver
 *
 * @param [in] drv_ctx  Host MCU's radio driver context
 * @returns    Operation status
 */
...
#endif /* __STM32WLXX_APP_RADIO_HAL_H_ */
