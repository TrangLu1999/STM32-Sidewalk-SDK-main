/**
  ******************************************************************************
  * @file    stm32wlxx_app_radio_hal.c
...
#define STM32WLxx_HAL_RADIO_SUBGHZ_GET_LIVE_RSSI_TIMEOUT_MS (5u)   /* Time limit for STM32WLxx to read live RSSI on the current Rx channel */
#define STM32WLxx_HAL_RADIO_SUBGHZ_IS_CH_FREE_TIMEOUT_MS    (25u)  /* Time limit for STM32WLxx to check if a single radio channel is free */
#define STM32WLxx_HAL_RADIO_SUBGHZ_GET_CH_NOISE_TIMEOUT_MS  (50u)  /* Time limit for STM32WLxx to measure signal noise on a single radio channel */
...
#  ifndef STM32WLxx_HAL_STM32WLxx_WAKEUP_TIME_US
#    define STM32WLxx_HAL_STM32WLxx_WAKEUP_TIME_US          (STM32WLxx_HAL_STM32WLxx_DEFAULT_WAKEUP_TIME_US) /* STM32WLxx wakeup delay to be able to accept SPI transfers - HAL will wait at least for this time to allow STM32WLxx to wakeup and restart the AHB/APB clocks. IMPORTANT: this is not a full wakeup, it's just the time required to restart the clocks */
#  endif /* STM32WLxx_HAL_STM32WLxx_WAKEUP_TIME_US */
#endif /* STM32WLxx_RADIO_APP_LPM_SUPPORT */
...
SID_STM32_SPEED_OPTIMIZED static inline stm32wlxx_hal_status_t _spi_transmit_receive_full_frame(const halo_drv_stm32wlxx_ctx_t * const drv_ctx, const uint8_t * const tx_buf, uint8_t * const rx_buf, const size_t data_length)
{
    ...
    do
    {
        ...
        {
            const sid_pal_serial_bus_stm32wbaxx_transaction_config_t * const spi_transaction_cfg = (sid_pal_serial_bus_stm32wbaxx_transaction_config_t *)drv_ctx->config->spi_client_cfg->client_selector_context;
            ...
            case STM32WLxx_APP_IRQ_USER_DATA:
                if ((drv_ctx->app_irq_mask & active_irq) != 0u)
                {
                    ...
                    if (drv_ctx->on_user_data_rx != NULL)
                    {
                        ...
                        {
                            /* Exclude UDT header from the payload size */
                            const uint32_t udt_payload_len = drv_ctx->last_irq_status.followup_payload_len - (sizeof(stm32wlxx_rcp_user_data_irq_followup_t) - SID_STM32_UTIL_STRUCT_MEMBER_SIZE(stm32wlxx_rcp_user_data_irq_followup_t, data));

                            /* Call user-defined callback */
                            drv_ctx->on_user_data_rx(user_data_frame->data, udt_payload_len, drv_ctx->on_user_data_rx_user_arg);
                        }
                    }
                }
                else
...
SID_STM32_SPEED_OPTIMIZED stm32wlxx_hal_status_t stm32wlxx_hal_radio_set_lora_cad_params(const halo_drv_stm32wlxx_ctx_t * const drv_ctx, const sid_pal_radio_lora_cad_params_t * const cad_params)
{
    stm32wlxx_hal_status_t err;
    stm32wlxx_radio_comm_spi_frame_t set_lora_cad_params_req = {
        ...
        .payload = {
            .set_lora_cad_params = {
                .exit_mode   = STM32WLxx_SUBGHZ_ZIP_CAD_EXIT_MODE((stm32wlxx_pal_radio_cad_param_exit_mode_t)cad_params->cad_exit_mode),
                .detect_peak = cad_params->cad_detect_peak,
                .detect_min  = cad_params->cad_detect_min,
                .timeout     = cad_params->cad_timeout,
                .symbol_num  = cad_params->cad_symbol_num,
            },
        },
    };
...
SID_STM32_SPEED_OPTIMIZED stm32wlxx_hal_status_t stm32wlxx_hal_radio_start_carrier_sense(const halo_drv_stm32wlxx_ctx_t * const drv_ctx, const sid_pal_radio_fsk_cad_params_t * const cad_params, const sid_pal_radio_cad_param_exit_mode_t exit_mode)
{
    stm32wlxx_hal_status_t err;
    stm32wlxx_radio_comm_spi_frame_t start_carrier_sense_req = {
        ...
        .payload = {
            .start_fsk_cs_params = {
                .exit_mode         = STM32WLxx_SUBGHZ_ZIP_CAD_EXIT_MODE((stm32wlxx_pal_radio_cad_param_exit_mode_t)exit_mode),
                .min_preamble_det  = cad_params->fsk_cs_min_prm_det,
                .ed_rssi_threshold = cad_params->fsk_ed_rssi_threshold,
                .ed_duration_mus   = cad_params->fsk_ed_duration_mus,
                .lbt_preamble_len  = cad_params->fsk_cs_lbt_preamble_len,
                .lbt_rx_timeout    = cad_params->fsk_cs_lbt_rx_timeout,
                .duration_us       = cad_params->fsk_cs_duration_us,
            },
        },
    };
...
#if HALO_ENABLE_DIAGNOSTICS
SID_STM32_SPEED_OPTIMIZED stm32wlxx_hal_status_t stm32wlxx_hal_start_infinite_preamble_tx(const halo_drv_stm32wlxx_ctx_t * const drv_ctx, const uint32_t freq)
{
    stm32wlxx_hal_status_t err;
    stm32wlxx_radio_comm_spi_frame_t start_inf_pbl_tx_req = {
        .opcode = STM32WLxx_RADIO_COMM_OPCODE_START_INFINITE_PBL_TX,
        .payload = {
            .start_inf_pbl_tx_params = {
                .frequency = freq,
            },
        }
    };

    err = _spi_transmit_data(drv_ctx, (void *)&start_inf_pbl_tx_req, sizeof(start_inf_pbl_tx_req));
    if (err != STM32WLxx_HAL_STATUS_OK)
    {
        STM32WLxx_HAL_LOG_ERROR("Failed to send Start SubGHz Infinite Preamble Tx request. SPI transfer error %u", (uint32_t)err);
    }
...
#if HALO_ENABLE_DIAGNOSTICS
SID_STM32_SPEED_OPTIMIZED stm32wlxx_hal_status_t stm32wlxx_hal_start_rx_duty_cycle(const halo_drv_stm32wlxx_ctx_t * const drv_ctx, const uint32_t rx_time, const uint32_t sleep_time)
{
    stm32wlxx_hal_status_t err;
    stm32wlxx_radio_comm_spi_frame_t start_rx_dc_req = {
        .opcode = STM32WLxx_RADIO_COMM_OPCODE_SET_RX_DUTY_CYCLE,
        .payload = {
            .start_rx_duty_cycle_params = {
                .rx_time    = rx_time,
    ...
    err = _spi_transmit_data(drv_ctx, (void *)&start_rx_dc_req, sizeof(start_rx_dc_req));
    if (err != STM32WLxx_HAL_STATUS_OK)
    {
        STM32WLxx_HAL_LOG_ERROR("Failed to send Start SubGHz Rx Duty Cycle request. SPI transfer error %u", (uint32_t)err);
    }
...
#if HALO_ENABLE_DIAGNOSTICS
SID_STM32_SPEED_OPTIMIZED stm32wlxx_hal_status_t stm32wlxx_hal_get_live_rssi(halo_drv_stm32wlxx_ctx_t * const drv_ctx, int16_t * const out_rssi)
{
    stm32wlxx_hal_status_t err;
    stm32wlxx_radio_comm_spi_frame_t get_live_rssi_req = {
        .opcode = STM32WLxx_RADIO_COMM_OPCODE_GET_LIVE_RSSI,
        .payload = {
            .raw = {0},
        }
    };
    ...
    do
    {
        ...
        err = _send_request_with_ack(drv_ctx, (void *)&get_live_rssi_req, sizeof(get_live_rssi_req), STM32WLxx_HAL_RADIO_SUBGHZ_GET_LIVE_RSSI_TIMEOUT_MS);

        if (err != STM32WLxx_HAL_STATUS_OK)
        {
            if (STM32WLxx_HAL_STATUS_ERROR_TIMEOUT == err)
            {
                STM32WLxx_HAL_LOG_ERROR("Failed to read live SubGHz RSSI - response timeout");
            }
            else
            {
                STM32WLxx_HAL_LOG_ERROR("Failed to read live SubGHz RSSI. HAL error %u", (uint32_t)err);
            }
            break;
        }
        ...
        /* If ACK was received - inspect the received IRQ status */
        const stm32wlxx_req_completed_irq_details_t * const irq_details = &drv_ctx->last_irq_status.req_cmpltd_details;
        if ((drv_ctx->last_irq_status.irq_flags != STM32WLxx_APP_IRQ_SPI_REQ_DONE) || (irq_details->request_id != STM32WLxx_RADIO_COMM_OPCODE_GET_LIVE_RSSI))
        {
            SID_PAL_LOG_ERROR("Failed to read live SubGHz RSSI. IRQ clash detected. Expected 0x%x flag, but got 0x%x, req_id 0x%x",
                              STM32WLxx_RADIO_COMM_OPCODE_GET_LIVE_RSSI, drv_ctx->last_irq_status.irq_flags, irq_details->request_id);
            err = STM32WLxx_HAL_STATUS_ERROR_UNEXPECTED_DATA;
            break;
        }
        ...
        if (irq_details->status != 0u)
        {
            SID_PAL_LOG_ERROR("Failed to read live SubGHz RSSI. STM32WLxx returned error code %u", irq_details->status);
            err = STM32WLxx_HAL_STATUS_ERROR_REJECTED;
            break;
        }

        /* Everything is ok, extract RSSI from the response data */
        *out_rssi = (int16_t)irq_details->additional_info.half_words[0];

        STM32WLxx_HAL_LOG_DEBUG("SubGHz live RSSI: %d", *out_rssi);
        err = STM32WLxx_HAL_STATUS_OK;
    } while (0);
...
#if HALO_ENABLE_DIAGNOSTICS
SID_STM32_SPEED_OPTIMIZED stm32wlxx_hal_status_t stm32wlxx_hal_is_channel_free(halo_drv_stm32wlxx_ctx_t * const drv_ctx, uint32_t freq, int16_t threshold, uint32_t scan_duration_us, bool * is_channel_free)
{
    stm32wlxx_hal_status_t err;
    stm32wlxx_radio_comm_spi_frame_t is_channel_free_req = {
        .opcode = STM32WLxx_RADIO_COMM_OPCODE_CHECK_CHANNEL_FREE,
        .payload = {
            .is_channel_free_params = {
                .frequency      = freq,
                .duration_us    = scan_duration_us,
                .rssi_threshold = (int8_t)threshold,
            },
        }
    };

    SID_PAL_ASSERT(is_channel_free != NULL);
    SID_PAL_ASSERT(threshold >= INT8_MIN);
    SID_PAL_ASSERT(threshold <= INT8_MAX);

    do
    {
        /* Set timeout to the requested scan duration (using math ceiling) plus a fixed safety buffer */
        const uint32_t request_timeout_ms = STM32WLxx_HAL_RADIO_SUBGHZ_IS_CH_FREE_TIMEOUT_MS + ((scan_duration_us + 999u) / 1000u);

        /* Request RSSI reading */
        err = _send_request_with_ack(drv_ctx, (void *)&is_channel_free_req, sizeof(is_channel_free_req), request_timeout_ms);

        if (err != STM32WLxx_HAL_STATUS_OK)
        {
            if (STM32WLxx_HAL_STATUS_ERROR_TIMEOUT == err)
            {
                STM32WLxx_HAL_LOG_ERROR("Failed to check SubGHz channel is free - response timeout");
            }
            else
            {
                STM32WLxx_HAL_LOG_ERROR("Failed to check SubGHz channel is free. HAL error %u", (uint32_t)err);
            }
            break;
        }
        ...
        /* If ACK was received - inspect the received IRQ status */
        const stm32wlxx_req_completed_irq_details_t * const irq_details = &drv_ctx->last_irq_status.req_cmpltd_details;
        if ((drv_ctx->last_irq_status.irq_flags != STM32WLxx_APP_IRQ_SPI_REQ_DONE) || (irq_details->request_id != STM32WLxx_RADIO_COMM_OPCODE_CHECK_CHANNEL_FREE))
        {
            SID_PAL_LOG_ERROR("Failed to check SubGHz channel is free. IRQ clash detected. Expected 0x%x flag, but got 0x%x, req_id 0x%x",
                              STM32WLxx_RADIO_COMM_OPCODE_CHECK_CHANNEL_FREE, drv_ctx->last_irq_status.irq_flags, irq_details->request_id);
            err = STM32WLxx_HAL_STATUS_ERROR_UNEXPECTED_DATA;
            break;
        }
        ...
        if (irq_details->status != 0u)
        {
            SID_PAL_LOG_ERROR("Failed to check SubGHz channel is free. STM32WLxx returned error code %u", irq_details->status);
            err = STM32WLxx_HAL_STATUS_ERROR_REJECTED;
            break;
        }

        /* Everything is ok, extract channel status from the response data */
        *is_channel_free = irq_details->additional_info.bytes[0] != 0u;
...
#if HALO_ENABLE_DIAGNOSTICS
SID_STM32_SPEED_OPTIMIZED stm32wlxx_hal_status_t stm32wlxx_hal_get_channel_noise(halo_drv_stm32wlxx_ctx_t * const drv_ctx, uint32_t freq, int16_t * const out_noise)
{
    stm32wlxx_hal_status_t err;
    stm32wlxx_radio_comm_spi_frame_t get_channel_noise_req = {
        .opcode = STM32WLxx_RADIO_COMM_OPCODE_GET_CHANNEL_NOISE,
        .payload = {
            .get_channel_noise_params = {
                .frequency = freq,
            },
        }
    };

    SID_PAL_ASSERT(out_noise != NULL);

    do
    {
        /* Request channel noise measurement */
        err = _send_request_with_ack(drv_ctx, (void *)&get_channel_noise_req, sizeof(get_channel_noise_req), STM32WLxx_HAL_RADIO_SUBGHZ_GET_CH_NOISE_TIMEOUT_MS);

        if (err != STM32WLxx_HAL_STATUS_OK)
        {
            if (STM32WLxx_HAL_STATUS_ERROR_TIMEOUT == err)
            {
                STM32WLxx_HAL_LOG_ERROR("Failed to measure SubGHz channel noise - response timeout");
            }
            else
            {
                STM32WLxx_HAL_LOG_ERROR("Failed to measure SubGHz channel noise. HAL error %u", (uint32_t)err);
            }
            break;
        }
        ...
        /* If ACK was received - inspect the received IRQ status */
        const stm32wlxx_req_completed_irq_details_t * const irq_details = &drv_ctx->last_irq_status.req_cmpltd_details;
        if ((drv_ctx->last_irq_status.irq_flags != STM32WLxx_APP_IRQ_SPI_REQ_DONE) || (irq_details->request_id != STM32WLxx_RADIO_COMM_OPCODE_GET_CHANNEL_NOISE))
        {
            SID_PAL_LOG_ERROR("Failed to measure SubGHz channel noise. IRQ clash detected. Expected 0x%x flag, but got 0x%x, req_id 0x%x",
                              STM32WLxx_RADIO_COMM_OPCODE_GET_CHANNEL_NOISE, drv_ctx->last_irq_status.irq_flags, irq_details->request_id);
            err = STM32WLxx_HAL_STATUS_ERROR_UNEXPECTED_DATA;
            break;
        }
        ...
        if (irq_details->status != 0u)
        {
            SID_PAL_LOG_ERROR("Failed to measure SubGHz channel noise. STM32WLxx returned error code %u", irq_details->status);
            err = STM32WLxx_HAL_STATUS_ERROR_REJECTED;
            break;
        }

        /* Everything is ok, extract channel noise from the response data */
        *out_noise = (int16_t)irq_details->additional_info.half_words[0];

        STM32WLxx_HAL_LOG_DEBUG("SubGHz channel noise: %d", *out_noise);
        err = STM32WLxx_HAL_STATUS_OK;
    } while (0);
...
#if HALO_ENABLE_DIAGNOSTICS
SID_STM32_SPEED_OPTIMIZED stm32wlxx_hal_status_t stm32wlxx_hal_radio_start_lora_cad(const halo_drv_stm32wlxx_ctx_t * const drv_ctx)
{
    stm32wlxx_hal_status_t err;
    stm32wlxx_radio_comm_spi_frame_t start_lora_cad_req = {
        .opcode = STM32WLxx_RADIO_COMM_OPCODE_START_LORA_CAD,
        .payload = {
            .raw = {0},
        }
    };

    err = _spi_transmit_data(drv_ctx, (void *)&start_lora_cad_req, sizeof(start_lora_cad_req));
    if (err != STM32WLxx_HAL_STATUS_OK)
    {
        STM32WLxx_HAL_LOG_ERROR("Failed to send Start LoRa CAD request. SPI transfer error %u", (uint32_t)err);
    }
...
SID_STM32_SPEED_OPTIMIZED stm32wlxx_hal_status_t stm32wlxx_radio_hal_init_gpio(halo_drv_stm32wlxx_ctx_t * const drv_ctx)
{
    ...
    do
    {
        /* Configure bi-directional IRQ line -----------------------------------------*/
        sid_err = sid_pal_gpio_pull_mode(drv_ctx->config->gpio.radio_irq, SID_PAL_GPIO_PULL_UP);
        if (sid_err != SID_ERROR_NONE)
        {
            STM32WLxx_HAL_LOG_ERROR("sid_pal_gpio_pull_mode() failed with error code %d, pin: %u", (int32_t)sid_err, drv_ctx->config->gpio.radio_irq);
            err = STM32WLxx_HAL_STATUS_ERROR_GPIO;
            break;
        }
        sid_err = sid_pal_gpio_input_mode(drv_ctx->config->gpio.radio_irq, SID_PAL_GPIO_INPUT_CONNECT);
        if (sid_err != SID_ERROR_NONE)
        {
            STM32WLxx_HAL_LOG_ERROR("sid_pal_gpio_input_mode() failed with error code %d, pin: %u", (int32_t)sid_err, drv_ctx->config->gpio.radio_irq);
            err = STM32WLxx_HAL_STATUS_ERROR_GPIO;
            break;
        }
        sid_err = sid_pal_gpio_set_direction(drv_ctx->config->gpio.radio_irq, SID_PAL_GPIO_DIRECTION_INPUT);
        if (sid_err != SID_ERROR_NONE)
        {
            STM32WLxx_HAL_LOG_ERROR("sid_pal_gpio_set_direction() failed with error code %d, pin: %u", (int32_t)sid_err, drv_ctx->config->gpio.radio_irq);
            err = STM32WLxx_HAL_STATUS_ERROR_GPIO;
            break;
        }
#if (CFG_LPM_STDBY_SUPPORTED != 0)
        sid_err = sid_pal_gpio_ext_ifc_standby_retention_enable(drv_ctx->config->gpio.radio_irq);
        if (sid_err != SID_ERROR_NONE)
        {
            STM32WLxx_HAL_LOG_ERROR("sid_pal_gpio_ext_ifc_standby_retention_enable() failed with error code %d, pin: %u", (int32_t)sid_err, drv_ctx->config->gpio.radio_irq);
            err = STM32WLxx_HAL_STATUS_ERROR_GPIO;
            break;
        }
...
SID_STM32_SPEED_OPTIMIZED stm32wlxx_hal_status_t stm32wlxx_radio_hal_deinit_gpio(halo_drv_stm32wlxx_ctx_t * const drv_ctx)
{
    ...
    do
    {
        /* Configure all pins to initial state to ensure everything (e.g., LEDs) is off */
        err = stm32wlxx_radio_hal_init_gpio(drv_ctx);
        if (err != STM32WLxx_HAL_STATUS_OK)
        {
            break;
        }

        /* Disconnect input buffer from the bi-directional IRQ line and enable pull-down - STM32WLxx may drive the line low, so having a pull-up will result in a leakage current */
