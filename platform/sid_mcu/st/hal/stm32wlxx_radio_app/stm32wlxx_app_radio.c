...
#include "stm32wlxx_app_radio.h"
#include "stm32wlxx_app_radio_default_timings.h"
#include "stm32wlxx_app_radio_hal.h"
...
#if defined(SID_APP_VERSION_HEADER)
#  include SID_APP_VERSION_HEADER
#else
#pragma message("SID_APP_VERSION_HEADER is not defined, application version info will remain empty when passed to the compatibility check callback (if defined)")
#endif /* SID_APP_VERSION_HEADER */
...
#  define STM32WLxx_UDT_TASK_FLAG_INIT_DONE          (1u << 0u) /*!< UDT task notification flag to signal that the radio driver is initialized and fully operational */
#  define STM32WLxx_UDT_TASK_FLAG_UDT_ENABLED        (1u << 1u) /*!< UDT task notification flag to signal that UDT transfers are now enabled */
...
static const osThreadAttr_t wlxx_user_data_transfer_thread_attributes = {
    ...
    .priority   = STM32WLxx_RADIO_CFG_UDT_TASK_PRIO,
    .stack_size = STM32WLxx_RADIO_CFG_UDT_TASK_STACK_SIZE,
...
static inline int32_t  radio_stm32wlxx_get_adjusted_rssi(const int32_t raw_rssi);

#if STM32WLXX_EXT_IFC_ENABLE_USER_DATA_TRANSFER
static inline uint32_t _user_data_transfer_check_transfers_allowed(void);
...
SID_STM32_SPEED_OPTIMIZED static int32_t radio_stm32wlxx_do_handshake(void)
{
    ...
    do
    {
        ...
        const stm32wlxx_version_info_t * const remote_protocol_version = &irq_status.handshake_details.protocol_version;
        const stm32wlxx_version_info_t         local_protocol_version  = {
        ...
        /* Call user-defined protocol version check if available in the configuration */
        if (drv_ctx.config->protocol_compatibility_check_cb != NULL)
        {
            err = drv_ctx.config->protocol_compatibility_check_cb(&local_protocol_version, remote_protocol_version);
        }
        else
        {
            /* Do a default check if user configuration does not define its own */
            if ( (remote_protocol_version->major != local_protocol_version.major)
              || (remote_protocol_version->minor != local_protocol_version.minor)
              || (remote_protocol_version->patch != local_protocol_version.patch))
            {
                /* Display a warning but do not block the communication. Version mismatch is possible during OTA update, so inter-MCU communication channel shall be accessible to finalize an OTA */
                SID_PAL_LOG_WARNING("STM32WLxx runs SPI protocol v%u.%u.%u while host has v%u.%u.%u, compatibility issues possible",
                                    remote_protocol_version->major, remote_protocol_version->minor, remote_protocol_version->patch,
                                    local_protocol_version.major, local_protocol_version.minor, local_protocol_version.patch);
                err = RADIO_ERROR_NONE;
            }
            else
            {
                /* Protocol version fully match */
                err = RADIO_ERROR_NONE;
            }
        }
        if (err != RADIO_ERROR_NONE)
        {
            SID_PAL_LOG_ERROR("STM32WLxx implements incompatible SPI protocol v%u.%u.%u (host requires v%u.%u.%u)",
                              remote_protocol_version->major, remote_protocol_version->minor, remote_protocol_version->patch,
                              local_protocol_version.major, local_protocol_version.minor, local_protocol_version.patch);
            break;
        }
        SID_PAL_LOG_INFO("Reported STM32WLxx SPI protocol version: %u.%u.%u", remote_protocol_version->major, remote_protocol_version->minor, remote_protocol_version->patch);

        /* Call user-defined application version compatibility check if defined */
        const stm32wlxx_version_info_t * const remote_app_version = &irq_status.handshake_details.app_version;
        const stm32wlxx_version_info_t         local_app_version  = {
#if defined(SID_APP_VERSION_HEADER)
        ...
        if (drv_ctx.config->app_compatibility_check_cb != NULL)
        {
            err = drv_ctx.config->app_compatibility_check_cb(&local_app_version, remote_app_version);
        }
        else
        {
            /* By default app version check is not performed */
            err = RADIO_ERROR_NONE;
        }
        if (err != RADIO_ERROR_NONE)
        {
            SID_PAL_LOG_ERROR("STM32WLxx implements incompatible application v%u.%u.%u (host is v%u.%u.%u)",
                              remote_app_version->major, remote_app_version->minor, remote_app_version->patch,
                              local_app_version.major, local_app_version.minor, local_app_version.patch);
            break;
        }
        SID_PAL_LOG_INFO("Detected STM32WLxx with application version %u.%u.%u", remote_app_version->major, remote_app_version->minor, remote_app_version->patch);
                                                                           ...
                                                                           stm32wlxx_radio_phy_cfg_t * const out_cfg)
{
    ...
    /* fsk_cad_params */
    /**
     * @attention This CAD section is somewhat obsolete starting from inter-MCU protocol version 1.1.0, but shall be preserved for backward
     *            compatibility. Now CAD/CS parameters are passed with every Start Carrier Sense command
     */
    ...
    /**
     * @attention This CAD section is somewhat obsolete starting from inter-MCU protocol version 1.1.0, but shall be preserved for backward
     *            compatibility. Now CAD parameters are passed with the Set LoRa CAD command
     */
...
#endif
    {
        ...
        switch (drv_ctx.regional_radio_param->param_region)
        {
            ...
            if (drv_ctx.regional_radio_param->ant_dbi > STM32WLxx_ANT_GAIN_CAP_REGION_NA)
            {
                ...
                target_pwr -= ((int32_t)drv_ctx.regional_radio_param->ant_dbi - STM32WLxx_ANT_GAIN_CAP_REGION_NA);
                target_pwr = (target_pwr >= 0) ? ((target_pwr + 50) / 100) : ((target_pwr - 50) / 100);
            }
            break;

        case RADIO_REGION_EU:
            /* EU requirements put the limit on EIRP - PA gain shall be reduced by antenna gain */
...
SID_STM32_SPEED_OPTIMIZED static inline void radio_stm32wlxx_error_manager(const int32_t reported_err, const uint32_t set_radio_sleep)
{
    ...
    if (drv_ctx.error_monitor.drv_err_cntr > (STM32WLxx_RADIO_ERROR_LIMIT * STM32WLxx_RADIO_ERROR_ASYM_INC_FACTOR))
    {
        ...
        do
        {
            ...
            err = stm32wlxx_radio_send_subghz_config(FALSE, FALSE);
            if (err != RADIO_ERROR_NONE)
            {
...
SID_STM32_SPEED_OPTIMIZED static inline int32_t radio_stm32wlxx_get_adjusted_rssi(const int32_t raw_rssi)
{
    const int32_t rssi_adjustment = (drv_ctx.regional_radio_param->ant_dbi >= 0) ? ((drv_ctx.regional_radio_param->ant_dbi + 50) / 100) : ((drv_ctx.regional_radio_param->ant_dbi - 50) / 100);
    const int32_t adjusted_rssi = raw_rssi - rssi_adjustment;
...
#if STM32WLXX_EXT_IFC_ENABLE_USER_DATA_TRANSFER
SID_STM32_SPEED_OPTIMIZED static inline uint32_t _user_data_transfer_check_transfers_allowed(void)
{
    sid_error_t         sid_err;
    struct sid_timespec t_now;

    /* Calculate the available time for UDT transfers */
    ...
    return drv_ctx.udt_ctx.udt_enabled;
}
#endif /* STM32WLXX_EXT_IFC_ENABLE_USER_DATA_TRANSFER */
...
SID_STM32_SPEED_OPTIMIZED static void _user_data_transfer_thread(void * context)
{
    ...
    do
    {
        ...
        while (FALSE == drv_ctx.init_done)
        {
            ...
            flags = osThreadFlagsWait(STM32WLxx_UDT_TASK_FLAG_INIT_DONE, osFlagsWaitAll, osWaitForever);
            SID_PAL_ASSERT((flags & osFlagsError) == 0u); /* Check against systematic failures */
        }
        ...
        if (osOK == os_err)
        {
            if ((msg_desc.data_ptr != NULL) && (msg_desc.data_len > 0u))
            {
                ...
                while (_user_data_transfer_check_transfers_allowed() == FALSE)
                {
                    flags = osThreadFlagsWait(STM32WLxx_UDT_TASK_FLAG_UDT_ENABLED, osFlagsWaitAll, osWaitForever);
                    SID_PAL_ASSERT((flags & osFlagsError) == 0u); /* Check against systematic failures */
                }
...
SID_STM32_SPEED_OPTIMIZED int32_t stm32wlxx_radio_send_subghz_config(const uint32_t use_defaults, const uint32_t need_ack)
{
    uint32_t err = RADIO_ERROR_GENERIC;
    stm32wlxx_hal_status_t hal_err;
...
#if SID_SDK_CONFIG_ENABLE_LINK_TYPE_2
                                                                            (FALSE == use_defaults) ? &drv_ctx.applied_fsk_phy_cfg : radio_stm32wlxx_fsk_get_default_phy_config();
#else
                                                                            NULL;
#endif /* SID_SDK_CONFIG_ENABLE_LINK_TYPE_2 */
...
#if SID_SDK_CONFIG_ENABLE_LINK_TYPE_3
                                                                            (FALSE == use_defaults) ? &drv_ctx.applied_lora_phy_cfg : radio_stm32wlxx_lora_get_default_phy_config();
#else
                                                                            NULL;
#endif /* SID_SDK_CONFIG_ENABLE_LINK_TYPE_3 */

        stm32wlxx_radio_phy_cfg_t subghz_cfg = {0};
...
SID_STM32_SPEED_OPTIMIZED int32_t stm32wlxx_radio_sidewalk_event_process(void)
{
    ...
    do
    {
        ...
        if (STM32WLxx_SUBGHZ_RADIO_MODEM_MODE_FSK == drv_ctx.modem)
        {
            ...
            drv_ctx.radio_rx_packet->fsk_rx_packet_status.rssi_avg  = (int8_t)radio_stm32wlxx_get_adjusted_rssi(drv_ctx.radio_rx_packet->fsk_rx_packet_status.rssi_avg);
            drv_ctx.radio_rx_packet->fsk_rx_packet_status.rssi_sync = (int8_t)radio_stm32wlxx_get_adjusted_rssi(drv_ctx.radio_rx_packet->fsk_rx_packet_status.rssi_sync);
        }
        else if (STM32WLxx_SUBGHZ_RADIO_MODEM_MODE_LORA == drv_ctx.modem)
        {
            ...
            /* Compensate for antenna gain to get unified RSSI readings */
            drv_ctx.radio_rx_packet->lora_rx_packet_status.rssi        = (int8_t)radio_stm32wlxx_get_adjusted_rssi(drv_ctx.radio_rx_packet->lora_rx_packet_status.rssi);
            drv_ctx.radio_rx_packet->lora_rx_packet_status.signal_rssi = (int8_t)radio_stm32wlxx_get_adjusted_rssi(drv_ctx.radio_rx_packet->lora_rx_packet_status.signal_rssi);
        }
        else
        {
...
SID_STM32_SPEED_OPTIMIZED int32_t sid_pal_radio_set_tx_power(int8_t power)
{
    ...
    do
    {
        ...
        {
            /* New PA config is stored to drv_ctx.pa_cfg directly by radio_stm32wlxx_set_tx_power_config() */
        }
         ...
         *  and we need to ensure RF timings consistency. To achieve this, it's better to always send out the PA config and account for this SPI transfer delay
         *  on every Tx, rather than dealing with the jitter caused by occasional PA config update frames.
         */
...
#  endif /* CFG_LPM_STDBY_SUPPORTED */
        ...
        if (0u == sleep_us)
        {
            ...
            /* Notify the UDT task about the transfers are (re-)enabled now */
            const uint32_t flags = osThreadFlagsSet(drv_ctx.udt_ctx.task, STM32WLxx_UDT_TASK_FLAG_UDT_ENABLED);
            SID_PAL_ASSERT((flags & osFlagsError) == 0u); /* Check against systematic failures */
        }
        else if (sleep_us > STM32WLxx_RADIO_CFG_UDT_CUTOFF_TIME_US)
        {
            ...
            sid_us_to_timespec((sleep_us - STM32WLxx_RADIO_CFG_UDT_CUTOFF_TIME_US), &t_sleep);
...
SID_STM32_SPEED_OPTIMIZED int32_t sid_pal_radio_start_tx(uint32_t timeout)
{
    ...
    do
    {
        ...
        if (STM32WLxx_SUBGHZ_RADIO_MODEM_MODE_FSK == drv_ctx.modem)
        {
            /* Prohibit Stop mode until Tx is done as we need fast response on Tx Done IRQ for switching into ACK Rx mode */
...
#  if (CFG_LPM_STDBY_SUPPORTED != 0)
        /* Standby LPM is not allowed during active Tx for all radio links */
...
#if HALO_ENABLE_DIAGNOSTICS
SID_STM32_SPEED_OPTIMIZED int32_t sid_pal_radio_set_tx_continuous_preamble(uint32_t freq, int8_t power)
{
    int32_t err = RADIO_ERROR_GENERIC;
    stm32wlxx_hal_status_t hal_err;

    STM32WLxx_LOG_INFO("sid_pal_radio_set_tx_continuous_preamble... ");
    ...
    do
    {
        ...
        hal_err = stm32wlxx_hal_start_infinite_preamble_tx(&drv_ctx, freq);
        if (hal_err != STM32WLxx_HAL_STATUS_OK)
        {
            SID_PAL_LOG_ERROR("Failed to start STM32WLxx SubGHz Infinite Preamble Tx. HAL error %u", (uint32_t)hal_err);
            err = RADIO_ERROR_HARDWARE_ERROR;
            break;
        }
...
#  if (CFG_LPM_STDBY_SUPPORTED != 0)
        /* Standby LPM is not allowed during active Rx for all radio links */
        UTIL_LPM_SetOffMode((1u << CFG_LPM_SIDEWALK), UTIL_LPM_DISABLE);
#  endif /* CFG_LPM_STDBY_SUPPORTED */
#endif /* (CFG_LPM_LEVEL != 0) */
...
SID_STM32_SPEED_OPTIMIZED int32_t sid_pal_radio_start_carrier_sense(const sid_pal_radio_fsk_cad_params_t * cad_params, sid_pal_radio_cad_param_exit_mode_t exit_mode)
{
    ...
    do
    {
        ...
        /* Initiate CS */
        hal_err = stm32wlxx_hal_radio_start_carrier_sense(&drv_ctx, cad_params, exit_mode);
        if (hal_err != STM32WLxx_HAL_STATUS_OK)
        {
        ...
        SID_STM32_UTIL_fast_memcpy(&drv_ctx.applied_fsk_phy_cfg.fsk_cad_params, cad_params, sizeof(drv_ctx.applied_fsk_phy_cfg.fsk_cad_params));
...
SID_STM32_SPEED_OPTIMIZED int32_t sid_pal_radio_set_rx_duty_cycle(uint32_t rx_time, uint32_t sleep_time)
{
    ...
    STM32WLxx_LOG_DEBUG("sid_pal_radio_set_rx_duty_cycle... ");
    ...
    do
    {
        ...
        hal_err = stm32wlxx_hal_start_rx_duty_cycle(&drv_ctx, rx_time, sleep_time);
        if (hal_err != STM32WLxx_HAL_STATUS_OK)
        {
            SID_PAL_LOG_ERROR("Failed to start STM32WLxx SubGHz Rx Duty Cycle. HAL error %u", (uint32_t)hal_err);
            err = RADIO_ERROR_HARDWARE_ERROR;
            break;
        }
...
{
#if HALO_ENABLE_DIAGNOSTICS && SID_SDK_CONFIG_ENABLE_LINK_TYPE_3
    int32_t err = RADIO_ERROR_GENERIC;
    stm32wlxx_hal_status_t hal_err;

    STM32WLxx_LOG_DEBUG("sid_pal_radio_lora_start_cad... ");

    do
    {
        /* Initiate CAD */
        hal_err = stm32wlxx_hal_radio_start_lora_cad(&drv_ctx);
        if (hal_err != STM32WLxx_HAL_STATUS_OK)
        {
            SID_PAL_LOG_ERROR("Failed to start STM32WLxx SubGHz LoRa CAD. HAL error %u", (uint32_t)hal_err);
            err = RADIO_ERROR_HARDWARE_ERROR;
            break;
        }
        ...
        drv_ctx.cad_exit_mode = drv_ctx.applied_lora_phy_cfg.lora_cad_params.cad_exit_mode;
        __COMPILER_BARRIER();

#  if (CFG_LPM_LEVEL != 0)
        /* Prohibit Stop mode until CAD is done as we need fast response on the radio IRQs */
        UTIL_LPM_SetStopMode((1u << CFG_LPM_SIDEWALK), UTIL_LPM_DISABLE);
#    if (CFG_LPM_STDBY_SUPPORTED != 0)
        UTIL_LPM_SetOffMode((1u << CFG_LPM_SIDEWALK), UTIL_LPM_DISABLE);
#    endif /* CFG_LPM_STDBY_SUPPORTED */
...
#  if STM32WLxx_RADIO_CFG_USE_STATUS_LED
        /* Drive the status LEDs after the actual start of CS to avoid impact on timings */
        if ((SID_PAL_RADIO_CAD_EXIT_MODE_CS_LBT == drv_ctx.cad_exit_mode) || (SID_PAL_RADIO_CAD_EXIT_MODE_ED_LBT == drv_ctx.cad_exit_mode))
        {
            /* This is carrier sense for FSK Tx */
            stm32wlxx_radio_hal_tx_led_on(&drv_ctx);
        }
...
#else
    return RADIO_ERROR_NOT_SUPPORTED;
#endif /* HALO_ENABLE_DIAGNOSTICS && SID_SDK_CONFIG_ENABLE_LINK_TYPE_3 */
}
...
{
#if HALO_ENABLE_DIAGNOSTICS
    int16_t rssi = INT16_MAX;
    stm32wlxx_hal_status_t hal_err;

    STM32WLxx_LOG_INFO("sid_pal_radio_rssi... ");

    do
    {
        hal_err = stm32wlxx_hal_get_live_rssi(&drv_ctx, &rssi);
        if (hal_err != STM32WLxx_HAL_STATUS_OK)
        {
            SID_PAL_LOG_ERROR("Failed to read instant STM32WLxx SubGHz RSSI. HAL error %u", (uint32_t)hal_err);
            break;
        }

        rssi = (int16_t)radio_stm32wlxx_get_adjusted_rssi(rssi);
    } while (0);
...
SID_STM32_SPEED_OPTIMIZED int32_t sid_pal_radio_is_channel_free(uint32_t freq, int16_t threshold, uint32_t scan_duration_us, bool * is_channel_free)
{
#if HALO_ENABLE_DIAGNOSTICS
    int32_t err;
    stm32wlxx_hal_status_t hal_err;

    STM32WLxx_LOG_DEBUG("sid_pal_radio_is_channel_free... ");
...
#  endif /* STM32WLxx_RADIO_CFG_USE_STATUS_LED */
    ...
    do
    {
        /* Adjust threshold by Rx path gain/loss */
        const int32_t rssi_adjustment = (drv_ctx.regional_radio_param->ant_dbi >= 0) ? ((drv_ctx.regional_radio_param->ant_dbi + 50) / 100) : ((drv_ctx.regional_radio_param->ant_dbi - 50) / 100);
        const int16_t adjusted_threshold = (int16_t)((int32_t)threshold + rssi_adjustment);

        hal_err = stm32wlxx_hal_is_channel_free(&drv_ctx, freq, adjusted_threshold, scan_duration_us, is_channel_free);
        if (hal_err != STM32WLxx_HAL_STATUS_OK)
        {
            SID_PAL_LOG_ERROR("Failed to check if %uHz channel is free. HAL error %u", freq, (uint32_t)hal_err);
            err = RADIO_ERROR_HARDWARE_ERROR;
            break;
        }
...
SID_STM32_SPEED_OPTIMIZED int32_t sid_pal_radio_get_chan_noise(uint32_t freq, int16_t *noise)
{
    ...
    STM32WLxx_LOG_DEBUG("sid_pal_radio_get_chan_noise... ");
...
#  endif /* STM32WLxx_RADIO_CFG_USE_STATUS_LED */
    ...
    do
    {
        hal_err = stm32wlxx_hal_get_channel_noise(&drv_ctx, freq, noise);
        if (hal_err != STM32WLxx_HAL_STATUS_OK)
        {
            SID_PAL_LOG_ERROR("Failed to capture %uHz channel noise. HAL error %u", freq, (uint32_t)hal_err);
            err = RADIO_ERROR_HARDWARE_ERROR;
            break;
        }

        /* Adjust RSSI */
        *noise = (int16_t)radio_stm32wlxx_get_adjusted_rssi(*noise);
...
SID_STM32_SPEED_OPTIMIZED int32_t sid_pal_radio_get_radio_state_transition_delays(sid_pal_radio_state_transition_timings_t * state_delay)
{
    ...
    /* While the TCXO is controlled by STM32WLxx, this value shall be aligned with STM32WLxx operation to accommodate the delays caused by TCXO startup */
    state_delay->tcxo_delay_us = STM32WLxx_SUBGHZ_TICKS_TO_US(STM32WLxx_RADIO_COMM_TCXO_TIMEOUT_DURATION_TUS);

    /* Set default wakeup delay if user config does not explicitly specify it */
    if (STM32WLxx_RADIO_CFG_USE_DEFAULT_TIMINGS == state_delay->sleep_to_full_power_us)
    {
        state_delay->sleep_to_full_power_us = STM32WLxx_RADIO_SLEEP_TO_FULL_POWER_DEFAULT_DELAY_US;
    }
...
SID_STM32_SPEED_OPTIMIZED int32_t sid_pal_radio_init(sid_pal_radio_event_notify_t notify, sid_pal_radio_irq_handler_t dio_irq_handler, sid_pal_radio_rx_packet_t *rx_packet)
{
    ...
    do
    {
        ...
        {
            drv_ctx.udt_ctx.outbound_msg_queue = osMessageQueueNew(STM32WLxx_RADIO_CFG_UDT_OB_MSG_QUEUE_LEN, sizeof(stm32wlxx_ext_ifc_out_msg_desc_t), NULL);
            if (NULL == drv_ctx.udt_ctx.outbound_msg_queue)
            {
        ...
        /* Send out SubGHz radio configuration via SPI to STM32WLxx ------------------*/
        err = stm32wlxx_radio_send_subghz_config(TRUE, TRUE);
        if (err != RADIO_ERROR_NONE)
        {
            /* Logs are provided by stm32wlxx_radio_send_subghz_config() */
            break;
...
#if STM32WLXX_EXT_IFC_ENABLE_USER_DATA_TRANSFER
        /* Ensure drv_ctx.init_done is set to TRUE before interacting with the UDT task */
        __COMPILER_BARRIER();

        /* Notify the UDT task about the driver readiness */
        const uint32_t flags = osThreadFlagsSet(drv_ctx.udt_ctx.task, STM32WLxx_UDT_TASK_FLAG_INIT_DONE);
        SID_PAL_ASSERT((flags & osFlagsError) == 0u); /* Check against systematic failures */
#endif /* STM32WLXX_EXT_IFC_ENABLE_USER_DATA_TRANSFER */
...
SID_STM32_SPEED_OPTIMIZED int32_t sid_pal_radio_deinit(void)
{
    ...
    do
    {
        ...
        /**
         * Important note: not killing the UDT task and message queue here to allow retention of any unprocessed UDT messages.
         * These messages will be delivered to the STM32WLxx side upon the next time the radio driver is initialized.Termnating
         * the UDT task may result in the loss of a single message that was already popped out of the queue but not sent yet.
         * Destroying the message queue will result in the total loss of all non-delivered messages.
         */
#endif /* STM32WLXX_EXT_IFC_ENABLE_USER_DATA_TRANSFER */
...
SID_STM32_SPEED_OPTIMIZED void sid_pal_radio_notify_host_standby_exit(void)
{
    stm32wlxx_hal_status_t hal_err;

    (void)hal_err;

    /* Important note: this method is typically called in the context of system wakeup from Standby LPM, so available stack size is quite limited. Try to avoid printing logs from here or calling any stack-heavy functions */

    do
    {
        if (NULL == drv_ctx.config)
        {
            /* Radio is not initialized yet, GPIO config is not available */
            break;
        }
        ...
        if (drv_ctx.init_done != FALSE)
        {
            ...
            hal_err = stm32wlxx_radio_hal_init_gpio(&drv_ctx);
            SID_PAL_ASSERT(STM32WLxx_HAL_STATUS_OK == hal_err);

            /* SPI bus re-initialization is performed by SPI driver independently. No actions required here */
        }
        else
        {
            /* Radio is not used, ensure GPIO pins are configured to keep SX126x off */
            hal_err = stm32wlxx_radio_hal_deinit_gpio(&drv_ctx);
            SID_PAL_ASSERT(STM32WLxx_HAL_STATUS_OK == hal_err);
        }
    } while (0);
