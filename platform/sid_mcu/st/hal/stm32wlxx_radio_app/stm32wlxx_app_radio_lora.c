/**
  ******************************************************************************
  * @file  stm32wlxx_app_radio_lora.c
...
const sid_pal_radio_lora_phy_settings_t lora_phy_default_cfg = {
    .sync_word = SID_PAL_RADIO_LORA_PRIVATE_NETWORK_SYNC_WORD,
    .lora_ldr_long_interleaved_enable = SID_PAL_RADIO_LORA_LDR_LONG_INTERLEAVER_OFF,
    .lora_modulation_params = {
        .spreading_factor             = SID_PAL_RADIO_LORA_SF11,
    ...
    .lora_cad_params = {
        ...
        .cad_detect_peak              = 21u,
        .cad_detect_min               = 10u,
        .cad_exit_mode                = SID_PAL_RADIO_CAD_EXIT_MODE_CS_ONLY,
        .cad_timeout                  = 0u,
    },
};
...
SID_STM32_SPEED_OPTIMIZED int32_t sid_pal_radio_set_lora_cad_params(const sid_pal_radio_lora_cad_params_t *cad_params)
{
    ...
    do
    {
        static_assert(SID_PAL_RADIO_LORA_CAD_EXIT_MODE_CAD_ONLY == STM32WLxx_RADIO_CAD_EXIT_MODE_CS_ONLY);
        static_assert(SID_PAL_RADIO_LORA_CAD_EXIT_MODE_CAD_RX   == STM32WLxx_RADIO_CAD_EXIT_MODE_CS_RX);
        static_assert(SID_PAL_RADIO_LORA_CAD_EXIT_MODE_CAD_LBT  == STM32WLxx_RADIO_CAD_EXIT_MODE_CS_LBT);
        if ((cad_params->cad_exit_mode != SID_PAL_RADIO_LORA_CAD_EXIT_MODE_CAD_ONLY)
         && (cad_params->cad_exit_mode != SID_PAL_RADIO_LORA_CAD_EXIT_MODE_CAD_RX)
         && (cad_params->cad_exit_mode != SID_PAL_RADIO_LORA_CAD_EXIT_MODE_CAD_LBT))
        {
            /* Not a known CAD mode */
            err = RADIO_ERROR_NOT_SUPPORTED;
            break;
        }
        drv_ctx->cad_exit_mode = cad_params->cad_exit_mode;

        if (SID_STM32_UTIL_fast_memcmp(&drv_ctx->applied_lora_phy_cfg.lora_cad_params, cad_params, sizeof(drv_ctx->applied_lora_phy_cfg.lora_cad_params)) == 0u)
        {
            /* No change in CAD parameters, skipping the send out */
            err = RADIO_ERROR_NONE;
            break;
        }

        /* Send out new LoRa CAD parameters */
        hal_err = stm32wlxx_hal_radio_set_lora_cad_params(drv_ctx, cad_params);
        if (hal_err != STM32WLxx_HAL_STATUS_OK)
        {
            /* Logs are provided by stm32wlxx_hal_radio_set_lora_cad_params() */
            err = RADIO_ERROR_HARDWARE_ERROR;
            break;
        }

        /* Store the new parameters */
        SID_STM32_UTIL_fast_memcpy(&drv_ctx->applied_lora_phy_cfg.lora_cad_params, cad_params, sizeof(drv_ctx->applied_lora_phy_cfg.lora_cad_params));
...
SID_STM32_SPEED_OPTIMIZED uint32_t sid_pal_radio_lora_time_on_air(const sid_pal_radio_lora_modulation_params_t * mod_params, const sid_pal_radio_lora_packet_params_t * packet_params, uint8_t packet_len)
{
    ...
    else
    {
        ...
        time_on_air = stm32wlxx_phy_get_lora_time_on_air_in_ms(&lora_packet_params, &lora_mod_params) + STM32WLxx_RADIO_CFG_IRQ_PROCESS_GUARD_TIME_MS;
    }
...
#if SID_SDK_CONFIG_ENABLE_LINK_TYPE_3
SID_STM32_SPEED_OPTIMIZED const sid_pal_radio_lora_phy_settings_t * radio_stm32wlxx_lora_get_default_phy_config(void)
{
    return &lora_phy_default_cfg;
}
#endif /* SID_SDK_CONFIG_ENABLE_LINK_TYPE_3 */
