...
#define S2LP_RADIO_HAL_RSSI_THRESHOLD_DBM                  (-80)      /*!< RSSI threshold in dBm for various adjustments (e.g. AFC, CSMA, packet filtering, etc.) - used to avoid unwanted reactions on noise signals */
...
SID_STM32_SPEED_OPTIMIZED static inline void _compute_synth_pll_steps(const halo_drv_s2_lp_ctx_t * const drv_ctx, const uint32_t freq_hz, s2_lp_ic_reg_synt_t * const synt_regs)
{
    ...
    if (FALSE == S2_LP_IC_IS_FREQ_BAND_HIGH(freq_hz))
    {
        ...
        base_divider_pow2 += (SID_STM32_UTIL_ROUNDUP_TO_POW_2_EXP(S2_LP_IC_MIDDLE_BAND_FACTOR) - 1u);
        synt_regs->synt3.BS = 1u;
    }
    else
    {
        /* Translate B/2 into pow2 exponent */
        base_divider_pow2 += (SID_STM32_UTIL_ROUNDUP_TO_POW_2_EXP(S2_LP_IC_HIGH_BAND_FACTOR) - 1u);
...
s2_lp_hal_status_t s2_lp_radio_hal_init_gpio(const halo_drv_s2_lp_ctx_t * const drv_ctx, const uint32_t restore_from_standby)
{
    s2_lp_hal_status_t err = S2_LP_RADIO_HAL_STATUS_ERROR_GENERIC;
    sid_error_t        sid_err;
    ...
    do
    {
...
#endif /* CFG_LPM_STDBY_SUPPORTED */
        {
            ...
            sid_err = sid_pal_gpio_pull_mode(drv_ctx->config->gpio.radio_shutdown, SID_PAL_GPIO_PULL_NONE);
            if (sid_err != SID_ERROR_NONE)
            {
...
#if (CFG_LPM_STDBY_SUPPORTED != 0)
            sid_err = sid_pal_gpio_ext_ifc_standby_retention_enable(drv_ctx->config->gpio.radio_shutdown);
            if (sid_err != SID_ERROR_NONE)
            {
                S2LP_RADIO_HAL_LOG_ERROR("Failed to enable S2-LP reset GPIO standby retention");
                err = S2_LP_RADIO_HAL_STATUS_ERROR_GPIO;
                break;
            }
#endif /* CFG_LPM_STDBY_SUPPORTED */
        ...
        {
            sid_err = sid_pal_gpio_ext_ifc_restore_output_after_standby(drv_ctx->config->gpio.radio_shutdown, SID_PAL_GPIO_OUTPUT_PUSH_PULL, SID_PAL_GPIO_PULL_NONE);
            if (sid_err != SID_ERROR_NONE)
            {
                S2LP_RADIO_HAL_LOG_ERROR("Failed to restore S2-LP reset GPIO");
                err = S2_LP_RADIO_HAL_STATUS_ERROR_GPIO;
                break;
            }
        }
...
s2_lp_hal_status_t s2_lp_radio_hal_deinit_gpio(const halo_drv_s2_lp_ctx_t * const drv_ctx)
{
    ...
    /* Set all GPIO pins to the initial state that keeps S2-LP in reset and FEM off */
    err = s2_lp_radio_hal_init_gpio(drv_ctx, FALSE);
