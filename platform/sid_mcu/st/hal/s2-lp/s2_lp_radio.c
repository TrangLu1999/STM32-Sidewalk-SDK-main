...
SID_STM32_SPEED_OPTIMIZED static inline int32_t _s2lp_radio_platform_init(void)
{
    ...
    do
    {
        ...
        hal_err = s2_lp_radio_hal_init_gpio(&drv_ctx, FALSE);
...
SID_STM32_SPEED_OPTIMIZED void sid_pal_radio_notify_host_standby_exit(void)
{
    ...
    do
    {
        ...
        if (drv_ctx.init_done != FALSE)
        {
            ...
            hal_err = s2_lp_radio_hal_init_gpio(&drv_ctx, TRUE);
            SID_PAL_ASSERT(S2_LP_RADIO_HAL_STATUS_OK == hal_err);

            /* SPI bus re-initialization is performed by SPI driver independently. No actions required here */

            /* Restore radio IRQ config */
            hal_err = s2_lp_radio_hal_arm_irq(&drv_ctx);
            SID_PAL_ASSERT(S2_LP_RADIO_HAL_STATUS_OK == hal_err);
        }
        else
        {
            /* Radio is not used, ensure GPIO pins are configured to keep S2-LP off */
            hal_err = s2_lp_radio_hal_deinit_gpio(&drv_ctx);
            SID_PAL_ASSERT(S2_LP_RADIO_HAL_STATUS_OK == hal_err);
        }
    } while (0);
...
SID_STM32_SPEED_OPTIMIZED int32_t sid_pal_radio_get_radio_state_transition_delays(sid_pal_radio_state_transition_timings_t * state_delay)
{
    ...
    if (S2LP_RADIO_CFG_USE_DEFAULT_TIMINGS == state_delay->sleep_to_full_power_us)
    {
        state_delay->sleep_to_full_power_us = S2LP_RADIO_SLEEP_TO_FULL_POWER_DEFAULT_DELAY_US;
    }
