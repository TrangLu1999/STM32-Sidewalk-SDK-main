...
/**
  * @brief  De-initialize resources to handle deep sleep entry/exit
  * @param  None
  * @retval LL_SYS status
  */
ll_sys_status_t ll_sys_dp_slp_deinit(void)
{
  ll_sys_status_t return_status = LL_SYS_ERROR;
  int32_t err;

  /* Disable radio interrupt as LL is being de-initialized */
  LINKLAYER_PLAT_DisableRadioIT();

  /* Run the below actions in a critical section */
  LINKLAYER_PLAT_DisableIRQ();

  /* Force HCI into standby mode */
  (void)ll_sys_dp_slp_enter(LL_DP_SLP_NO_WAKEUP);

  /* Delete link layer timer for handling IP DEEP SLEEP mode */
  if (radio_dp_slp_tmr_id != NULL)
  {
    ...
    err = os_timer_free(radio_dp_slp_tmr_id);
  }
  else
  {
    err = 0u;
...
ll_sys_status_t ll_sys_dp_slp_exit(void){
  ...
  else
  {
    ...
    /*vvv PATCH STMC-485 vvvvvvvvvv*/
    // if(os_get_tmr_state(radio_dp_slp_tmr_id) != (os_timer_state)osTimerStopped)
    if((radio_dp_slp_tmr_id != NULL) && (os_get_tmr_state(radio_dp_slp_tmr_id) != (os_timer_state)osTimerStopped))
    /*^^^ END OF PATCH STMC-485 ^^^*/
    {
      os_timer_stop(radio_dp_slp_tmr_id);
    }
