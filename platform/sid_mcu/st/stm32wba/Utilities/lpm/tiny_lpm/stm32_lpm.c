...
static uint32_t legacy_off_state_disable = 0u;
...
static uint32_t legacy_stop_state_disable = 0u;
/*^^^ END OF PATCH STMC-485 ^^^*/
...
void UTIL_LPM_SetMaxMode(UTIL_LPM_bm_t id_bm, uint8_t lp_mode)
{
  ...
  if (lp_mode < UTIL_LPM_Driver_num)
  {
    ...
    switch (lp_mode)
    {
      case UTIL_LPM_SLEEPMODE:
        legacy_stop_state_disable |= id_bm;
        legacy_off_state_disable  |= id_bm;
        break;

      case UTIL_LPM_STOPMODE:
        legacy_stop_state_disable &= (~id_bm);
        legacy_off_state_disable  |= id_bm;
        break;

      case UTIL_LPM_OFFMODE:
        legacy_stop_state_disable &= (~id_bm);
        legacy_off_state_disable  &= (~id_bm);
        break;
...
void UTIL_LPM_SetOffMode(UTIL_LPM_bm_t lpm_id_bm, UTIL_LPM_State_t state)
{
  ...
  if (UTIL_LPM_ENABLE == state)
  {
    legacy_off_state_disable &= (~lpm_id_bm);
  }
  else
  {
    legacy_off_state_disable |= lpm_id_bm;
  ...
  UTIL_LPM_ControlTable[UTIL_LPM_OFFMODE]  = (~legacy_off_state_disable) & (~legacy_stop_state_disable);
  UTIL_LPM_ControlTable[UTIL_LPM_STOPMODE] = legacy_off_state_disable & (~legacy_stop_state_disable);
  UTIL_LPM_ControlTable[UTIL_LPM_SLEEPMODE] = legacy_stop_state_disable;
...
void UTIL_LPM_SetStopMode(UTIL_LPM_bm_t lpm_id_bm, UTIL_LPM_State_t state)   
{
  ...
  if (UTIL_LPM_ENABLE == state)
  {
    legacy_stop_state_disable &= (~lpm_id_bm);
  }
  else
  {
    legacy_stop_state_disable |= lpm_id_bm;
