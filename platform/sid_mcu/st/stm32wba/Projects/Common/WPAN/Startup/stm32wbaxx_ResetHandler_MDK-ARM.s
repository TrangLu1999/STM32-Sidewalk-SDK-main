...
Reset_Handler    PROC
                 ...
                 IMPORT  rcc_reset_flags            ; application-defined storage for the reset flags in RCC_CSR register captured on early startup

     EXPORT  CPUcontextSave
                 EXPORT  backup_system_register
                 EXPORT  restore_system_register
; Check RCC->CSR register
        LDR     R1, =0x46020CF4 ; RCC->CSR register address
        LDR     R1, [R1]        ; Read RCC->CSR value
        CMP     R1, #0
        BEQ     StoreResetFlags ; CSR is zero - use the CSR value stored in r0 by the bootloader (if used)
        MOV     R0, R1          ; CSR is non-zero - use its value as is
StoreResetFlags:
        LDR     R1, =rcc_reset_flags
        STR     R0, [R1]
