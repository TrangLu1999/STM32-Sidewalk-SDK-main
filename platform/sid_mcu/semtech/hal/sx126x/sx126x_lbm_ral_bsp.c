...
SID_STM32_SPEED_OPTIMIZED void ral_sx126x_bsp_get_xosc_cfg(const void * context, ral_xosc_cfg_t * xosc_cfg, sx126x_tcxo_ctrl_voltages_t * supply_voltage, uint32_t * startup_time_in_tick)
{
    ...
    *startup_time_in_tick = SX126X_US_TO_TUS((SX126X_RADIO_CFG_USE_DEFAULT_TIMINGS == drv_ctx->config->tcxo_config.timeout_us) ? SX126X_RADIO_TCXO_SUBGHZ_DEFAULT_START_TIME_US : drv_ctx->config->tcxo_config.timeout_us);
}
