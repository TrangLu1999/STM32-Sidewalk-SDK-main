...
SID_STM32_SPEED_OPTIMIZED sx126x_hal_status_t sx126x_hal_init_gpio(const void * const context, const uint32_t restore_from_standby)
{
    ...
    do
    {
...
#endif /* CFG_LPM_STDBY_SUPPORTED */
        {
            ...
            if (sid_err != SID_ERROR_NONE)
            {
                SX126X_RADIO_HAL_LOG_ERROR("Failed to configure SX126x reset GPIO pull");
                err = SX126X_HAL_STATUS_ERROR;
                break;
            }
#if (CFG_LPM_STDBY_SUPPORTED != 0)
            ...
            if (sid_err != SID_ERROR_NONE)
            {
                SX126X_RADIO_HAL_LOG_ERROR("Failed to enable SX126x reset GPIO standby retention");
                err = SX126X_HAL_STATUS_ERROR;
                break;
            }
#endif /* CFG_LPM_STDBY_SUPPORTED */
        ...
        else
        {
            ...
            if (sid_err != SID_ERROR_NONE)
            {
                SX126X_RADIO_HAL_LOG_ERROR("Failed to restore SX126x reset GPIO");
                err = SX126X_HAL_STATUS_ERROR;
                break;
            }
        }
...
{
    sx126x_hal_status_t                  err;
    const halo_drv_semtech_ctx_t * const drv_ctx = (const halo_drv_semtech_ctx_t *)ctx;

    SID_PAL_ASSERT(drv_ctx != NULL);
    SID_PAL_ASSERT(drv_ctx->config != NULL);

    /* Set all GPIO pins to the initial state that keeps SX126x in reset and FEM off */
    err = sx126x_hal_init_gpio(drv_ctx, FALSE);
