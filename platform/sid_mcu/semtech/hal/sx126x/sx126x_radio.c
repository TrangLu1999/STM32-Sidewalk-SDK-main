...
SID_STM32_SPEED_OPTIMIZED static int32_t radio_sx126x_configure_clocks(void)
{
    ...
    do
    {
        ...
        if (drv_ctx.config->tcxo_config.ctrl != SX126X_TCXO_CTRL_NONE)
        {
            ...
            {
                if (SX126X_RADIO_CFG_USE_DEFAULT_TIMINGS == drv_ctx.config->tcxo_config.timeout_us)
                {
                    tcxo_start_timeout = SX126X_US_TO_TUS(SX126X_RADIO_TCXO_SUBGHZ_DEFAULT_START_TIME_US);
                }
                else
                {
...
SID_STM32_SPEED_OPTIMIZED static int32_t radio_sx126x_platform_init(void)
{
    ...
    do
    {
        ...
        /* Configure GPIO pins */
        hal_err = sx126x_hal_init_gpio(&drv_ctx, FALSE);
        if (hal_err != SX126X_HAL_STATUS_OK)
        {
...
#  endif /* SX126X_RADIO_CFG_USE_STATUS_LED */
                ...
                else if ((SID_PAL_RADIO_CAD_EXIT_MODE_CS_RX == drv_ctx.cad_exit_mode) && (SID_PAL_RADIO_EVENT_CAD_DONE == radio_event))
                {
...
#if (CFG_LPM_LEVEL != 0)
        /* Allow host MCU low power modes after the radio was put into Sleep */
        UTIL_LPM_SetStopMode((1u << CFG_LPM_SIDEWALK), UTIL_LPM_ENABLE);
#  if (CFG_LPM_STDBY_SUPPORTED != 0)
        UTIL_LPM_SetOffMode((1u << CFG_LPM_SIDEWALK), UTIL_LPM_ENABLE);
#  endif /* CFG_LPM_STDBY_SUPPORTED */
...
#if SID_SDK_CONFIG_RF_TIMINGS_MEASUREMENT
        /* Indicate SX126x wakeup start point */
        __COMPILER_BARRIER();
        if (SID_PAL_RADIO_SLEEP == drv_ctx.radio_state)
        {
            SIDEWALK_RF_TIMINGS_RXTX_START_GPIO_Port->BRR = SIDEWALK_RF_TIMINGS_RXTX_START_Pin;
...
#if SID_SDK_CONFIG_RF_TIMINGS_MEASUREMENT
            /* Indicate SX126x wakeup end point */
...
SID_STM32_SPEED_OPTIMIZED int32_t sid_pal_radio_set_tx_continuous_preamble(uint32_t freq, int8_t power)
{
    ...
    do
    {
        ...
        /* Initiate Tx */
        sys_err = sx126x_set_tx_infinite_preamble(&drv_ctx);
        if (sys_err != SX126X_STATUS_OK)
        {
            SX126X_RADIO_LOG_ERROR("Failed to initiate continuous preamble Tx. Error %d", (int32_t)sys_err);
            drv_ctx.radio_state = SID_PAL_RADIO_UNKNOWN;
            err = RADIO_ERROR_HARDWARE_ERROR;
            break;
        }
...
SID_STM32_SPEED_OPTIMIZED int32_t sid_pal_radio_get_radio_state_transition_delays(sid_pal_radio_state_transition_timings_t * state_delay)
{
    ...
    if (SX126X_TCXO_CTRL_DIO3 == drv_ctx.config->tcxo_config.ctrl)
    {
        ...
        if (SX126X_RADIO_CFG_USE_DEFAULT_TIMINGS == drv_ctx.config->tcxo_config.timeout_us)
        {
            state_delay->tcxo_delay_us = SX126X_RADIO_TCXO_SUBGHZ_DEFAULT_START_TIME_US;
        }
        else
        {
    ...
    if (SX126X_RADIO_CFG_USE_DEFAULT_TIMINGS == state_delay->sleep_to_full_power_us)
    {
        state_delay->sleep_to_full_power_us = SX126X_RADIO_SLEEP_TO_FULL_POWER_DEFAULT_DELAY_US;
    }
...
SID_STM32_SPEED_OPTIMIZED void sid_pal_radio_notify_host_standby_exit(void)
{
    ...
    do
    {
        ...
        if (drv_ctx.init_done != FALSE)
        {
            ...
            hal_err = sx126x_hal_init_gpio(&drv_ctx, TRUE);
            SID_PAL_ASSERT(SX126X_HAL_STATUS_OK == hal_err);
