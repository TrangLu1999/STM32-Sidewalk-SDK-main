...
#define LR11XX_LORA_CAD_IRQ_MASK              ( LR11XX_DEFAULT_LORA_IRQ_MASK   \
...
SID_STM32_SPEED_OPTIMIZED static int32_t radio_lr11xx_configure_clocks(void)
{
    ...
    do
    {
        ...
        if (drv_ctx.config->tcxo_config.ctrl != LR11XX_TCXO_CTRL_NONE)
        {
            ...
            {
                if (LR11XX_RADIO_CFG_USE_DEFAULT_TIMINGS == drv_ctx.config->tcxo_config.timeout_us)
                {
                    tcxo_start_timeout = LR11XX_US_TO_TUS(LR11XX_RADIO_TCXO_SUBGHZ_DEFAULT_START_TIME_US);
                }
                else
                {
...
SID_STM32_SPEED_OPTIMIZED static int32_t radio_lr11xx_platform_init(void)
{
    ...
    do
    {
        ...
        /* Configure GPIO pins */
        hal_err = lr11xx_hal_init_gpio(&drv_ctx, FALSE);
...
SID_STM32_SPEED_OPTIMIZED static int32_t radio_lr11xx_platform_deinit(void)
{
    ...
    do
    {
        ...
        /* Release GPIO pins */
        hal_err = lr11xx_hal_deinit_gpio(&drv_ctx);
        if (hal_err != LR11XX_HAL_STATUS_OK)
        {
...
SID_STM32_SPEED_OPTIMIZED static inline const lr11xx_freq_band_t * lr11xx_get_freq_band(const uint32_t freq)
{
    ...
    for (uint32_t i = 0u; i < SID_STM32_UTIL_ARRAY_SIZE(bands); i++)
    {
        ...
        else if ((i > 0u) && (freq < bands[i].start) && (freq > bands[i - 1u].stop))
        {
            /* Special case - the frequency is between the two known bands - select the closest one */
            if ((bands[i].start - freq) < (freq - bands[i - 1u].stop))
            {
                /* The frequency is closer to the current band */
                freq_band = &bands[i];
                break;
            }
            else
            {
                /* The frequency is closer to the previous band */
                freq_band = &bands[i - 1u];
                break;
            }
        }
        else
...
#if (CFG_LPM_LEVEL != 0)
        /* Allow host MCU low power modes after Rx/Tx timeout */
...
SID_STM32_SPEED_OPTIMIZED int32_t sid_pal_radio_irq_process(void)
{
    ...
    do
    {
        ...
        if ((reported_irqs & LR11XX_SYSTEM_IRQ_ERROR) != LR11XX_SYSTEM_IRQ_NONE)
        {
            ...
            /* Terminate if no more active IRQs are left, otherwise proceed with processing */
        ...
        if ((reported_irqs & LR11XX_SYSTEM_IRQ_CAD_DONE) != LR11XX_SYSTEM_IRQ_NONE)
        {
            ...
            {
                err = RADIO_ERROR_NONE;
#if SID_SDK_CONFIG_ENABLE_LINK_TYPE_3 && HALO_ENABLE_DIAGNOSTICS
                ...
                if (err != RADIO_ERROR_NONE)
                {
                    LR11XX_RADIO_LOG_ERROR("Failed to restore default IRQ mask for LoRa");
                }
                ...
                {
                    /* LoRa CAD discovered no activities - radio channel is free */
                    radio_event = SID_PAL_RADIO_EVENT_UNKNOWN;

                    if (SID_PAL_RADIO_CAD_EXIT_MODE_CS_LBT == drv_ctx.cad_exit_mode)
                    {
                        /* Start Tx if CAD-LBT mode is used */
                        err = sid_pal_radio_start_tx(LR11XX_CAD_DEFAULT_TX_TIMEOUT);
                        if (err != RADIO_ERROR_NONE)
                        {
                            /* Logs provided by sid_pal_radio_start_tx() */
...
#  if LR11XX_RADIO_CFG_USE_STATUS_LED
                        /* Turn off Rx LED as CAD is finished */
                        lr11xx_radio_hal_rx_led_off(&drv_ctx);
#  endif /* LR11XX_RADIO_CFG_USE_STATUS_LED */
                    }
                    else
    ...
    if (RADIO_ERROR_NONE == err)
    {
        ...
        if ((SID_PAL_RADIO_EVENT_RX_TIMEOUT == radio_event) || (SID_PAL_RADIO_EVENT_RX_DONE == radio_event) || (SID_PAL_RADIO_EVENT_TX_DONE == radio_event) || (SID_PAL_RADIO_EVENT_TX_TIMEOUT == radio_event)
          ...
          || (SID_PAL_RADIO_EVENT_CAD_TIMEOUT == radio_event))
        {
            /* Assume STDBY_RC state if STDB_XOSC is not indicated explicitly by any IRQ handler */
            if (drv_ctx.radio_state != SID_PAL_RADIO_STANDBY_XOSC)
            {
                drv_ctx.radio_state = SID_PAL_RADIO_STANDBY;
            }
#if (CFG_LPM_LEVEL != 0)
            /* Allow host MCU low power modes after Rx/Tx is finished */
            UTIL_LPM_SetStopMode((1u << CFG_LPM_SIDEWALK), UTIL_LPM_ENABLE);
#  if (CFG_LPM_STDBY_SUPPORTED != 0)
            UTIL_LPM_SetOffMode((1u << CFG_LPM_SIDEWALK), UTIL_LPM_ENABLE);
#  endif /* CFG_LPM_STDBY_SUPPORTED */
...
#if SID_SDK_CONFIG_RF_TIMINGS_MEASUREMENT
        /* Indicate LR11xx wakeup start point */
        __COMPILER_BARRIER();
        if (SID_PAL_RADIO_SLEEP == drv_ctx.radio_state)
        {
            SIDEWALK_RF_TIMINGS_RXTX_START_GPIO_Port->BRR = SIDEWALK_RF_TIMINGS_RXTX_START_Pin;
...
#if SID_SDK_CONFIG_RF_TIMINGS_MEASUREMENT
            /* Indicate LR11xx wakeup end point */
...
#  endif /* LR11XX_RADIO_CFG_LBM_CONCURRENCY_SUPPORT */
    ...
    do
    {
        ...
        /* Initiate Tx */
        sys_err = lr11xx_radio_set_tx_infinite_preamble(&drv_ctx);
        if (sys_err != LR11XX_STATUS_OK)
        {
            LR11XX_RADIO_LOG_ERROR("Failed to initiate continuous preamble Tx. Error %d", (int32_t)sys_err);
            drv_ctx.radio_state = SID_PAL_RADIO_UNKNOWN;
            err = RADIO_ERROR_HARDWARE_ERROR;
            break;
        }
...
#  endif /* LR11XX_RADIO_CFG_LBM_CONCURRENCY_SUPPORT */
        ...
        if (SID_PAL_RADIO_MODEM_MODE_FSK == drv_ctx.modem)
        {
            /* Prohibit Stop mode until Rx is done as we need very fast response on radio IRQs because some adjustments have to be made on the fly after the packet header is received */
            UTIL_LPM_SetStopMode((1u << CFG_LPM_SIDEWALK), UTIL_LPM_DISABLE);
        }
        else
        {
...
#  if (CFG_LPM_LEVEL != 0)
        /* Prohibit Stop and Standby LPM modes until CS is done as we need fast response on the radio IRQs */
        UTIL_LPM_SetStopMode((1u << CFG_LPM_SIDEWALK), UTIL_LPM_DISABLE);
#    if (CFG_LPM_STDBY_SUPPORTED != 0)
        UTIL_LPM_SetOffMode((1u << CFG_LPM_SIDEWALK), UTIL_LPM_DISABLE);
#    endif /* CFG_LPM_STDBY_SUPPORTED */
...
#  endif /* LR11XX_RADIO_CFG_SMTC_LBM_INTEGRATION */
        ...
        err = radio_set_irq_mask(LR11XX_LORA_CAD_IRQ_MASK);
        if (err != RADIO_ERROR_NONE)
        {
            LR11XX_RADIO_LOG_ERROR("Failed to set IRQ mask for LoRa CAD mode");
            break;
        }
...
SID_STM32_SPEED_OPTIMIZED int32_t sid_pal_radio_get_radio_state_transition_delays(sid_pal_radio_state_transition_timings_t * state_delay)
{
    ...
    if (LR11XX_TCXO_CTRL_VTCXO == drv_ctx.config->tcxo_config.ctrl)
    {
        ...
        if (LR11XX_RADIO_CFG_USE_DEFAULT_TIMINGS == drv_ctx.config->tcxo_config.timeout_us)
        {
            state_delay->tcxo_delay_us = LR11XX_RADIO_TCXO_SUBGHZ_DEFAULT_START_TIME_US;
        }
        else
        {
    ...
    if (LR11XX_RADIO_CFG_USE_DEFAULT_TIMINGS == state_delay->sleep_to_full_power_us)
    {
        state_delay->sleep_to_full_power_us = LR11XX_RADIO_SLEEP_TO_FULL_POWER_DEFAULT_DELAY_US;
    }
...
SID_STM32_SPEED_OPTIMIZED void sid_pal_radio_notify_host_standby_exit(void)
{
    ...
    do
    {
        ...
        if (drv_ctx.init_done != FALSE)
        {
            ...
            hal_err = lr11xx_hal_init_gpio(&drv_ctx, TRUE);
            SID_PAL_ASSERT(LR11XX_HAL_STATUS_OK == hal_err);

            /* SPI bus re-initialization is performed by SPI driver independently. No actions required here */
        }
        else
        {
            /* Radio is not used, ensure GPIO pins are configured to keep LR11xx off */
            hal_err = lr11xx_hal_deinit_gpio(&drv_ctx);
            SID_PAL_ASSERT(LR11XX_HAL_STATUS_OK == hal_err);
        }
    } while (0);
...
#if LR11XX_RADIO_CFG_SMTC_LBM_INTEGRATION
SID_STM32_SPEED_OPTIMIZED int32_t sid_pal_radio_lr11xx_start_lbm_bridge_mode(const radio_lr11xx_lbm_bridge_state_t desired_mode)
{
    int32_t err;
...
#  endif /* LR11XX_RADIO_CFG_LBM_CONCURRENCY_SUPPORT */
    SID_PAL_LOG_INFO("LR11xx LBM Bridge activated in %s mode", (RADIO_LR11XX_LBM_BRIDGE_STATE_EXCLUSIVE_MODE == desired_mode) ? "exclusive" : "Sidewalk blanking");
        err = RADIO_ERROR_NONE;
    } while (0);
...
SID_STM32_SPEED_OPTIMIZED int32_t sid_pal_radio_lr11xx_stop_lbm_bridge_mode(void)
{
    ...
    do
    {
        ...
        if ((drv_ctx.radio_state != SID_PAL_RADIO_SLEEP) && (FALSE == drv_ctx.lbm.radio_sleep_pending))
        {
             ...
             *       logic flow
             */
            SID_PAL_LOG_ERROR("LBM bridge mode exit error - radio is not in sleep state (%u)", drv_ctx.radio_state);
            err = RADIO_ERROR_BUSY;
            break;
        }
...
#  if SID_SDK_CONFIG_ENABLE_LINK_TYPE_3
        if (SID_PAL_RADIO_MODEM_MODE_LORA == drv_ctx.modem)
        {
            /* Restore LoRa packet and modulation parameters from the last known state (if available) */
            if ( (drv_ctx.settings_cache.lora_mod_params.bandwidth != 0u) /* Using modulation parameters that can't be zero to check config cache validity */
              && (drv_ctx.settings_cache.lora_mod_params.coding_rate != 0u)
              && (drv_ctx.settings_cache.lora_mod_params.spreading_factor != 0u)
            )
            {
...
#  if SID_SDK_CONFIG_ENABLE_LINK_TYPE_2
        if (SID_PAL_RADIO_MODEM_MODE_FSK == drv_ctx.modem)
        {
            /* Restore FSK packet and modulation parameters from the last known state (if available) */
            if ( (drv_ctx.settings_cache.fsk_mod_params.bandwidth != 0u) /* Using modulation parameters that can't be zero to check config cache validity */
              && (drv_ctx.settings_cache.fsk_mod_params.bit_rate != 0u)
              && (drv_ctx.settings_cache.fsk_mod_params.freq_dev != 0u)
            )
...
#  endif /* SID_SDK_CONFIG_ENABLE_LINK_TYPE_2 */
        ...
        if ((err != RADIO_ERROR_NONE) && (err != RADIO_ERROR_INVALID_PARAMS)) /* RADIO_ERROR_INVALID_PARAMS may be reported if LBM bridge is stopped before Sidewalk sets frequency for the first time, it's ok */
        {
            SID_PAL_LOG_ERROR("LBM bridge mode exit failed to restore RF frequency (%uHz). Error %d", drv_ctx.radio_freq_hz, err);
            err = RADIO_ERROR_HARDWARE_ERROR;
            break;
        }
        ...
        SID_PAL_LOG_INFO("LR11xx LBM Bridge deactivated");
        err = RADIO_ERROR_NONE;
    } while (0);
