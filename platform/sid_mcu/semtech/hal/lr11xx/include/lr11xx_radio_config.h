...
#ifndef SID_RADIO_PLATFORM_LR11XX
...
#  define LR11XX_RADIO_CFG_GEOLOCATION_SUPPORT      (LR11XX_RADIO_CFG_SMTC_LBM_INTEGRATION) /*!< Include additional code to support LR11xx geolocation features. LR1xx geolocation shall be controlled separately from SID_SDK_CONFIG_ENABLE_LOCATION because Sidewalk BLE location may be enabled without using LR11xx location features */
...
#if LR11XX_RADIO_CFG_SMTC_LBM_INTEGRATION && !defined(LR11XX_RADIO_CFG_SID_BRIDGE_LBM_STACK_ID)
#  define LR11XX_RADIO_CFG_SID_BRIDGE_LBM_STACK_ID  (0) /*!< Stack ID for the LoRa Basics Modem library. By default LBM is built for a single stack, so this value is 0 in most of the cases */
#endif /* LR11XX_RADIO_CFG_SMTC_LBM_INTEGRATION && !defined(LR11XX_RADIO_CFG_SID_BRIDGE_LBM_STACK_ID) */
...
#define LR11XX_SIDEWALK_FSK_CS_DURATION_US          (1200u)   /*!< Carrier Sense timeout for FSK link as defined by Sidewalk specification (1200us) */
#define LR11XX_SIDEWALK_FSK_CS_RSSI_THRESHOLD       (-80)     /*!< Carrier Sense RSSI threshold in dBm - any signal above that level means the radio channel is busy */

#define LR11XX_PA_CFG_DB_MULT                       (100)     /*!< Multiplier to conduct integer calculations with more precision */

/*----------------------------------------------------------------------------*/
/* Defaults for Sidewalk Location Services configuration which may be missing from sid_sdk_config.h */
#ifndef SID_SDK_CONFIG_ENABLE_WIFI
#  define SID_SDK_CONFIG_ENABLE_WIFI                (SID_SDK_CONFIG_ENABLE_LOCATION && LR11XX_RADIO_CFG_GEOLOCATION_SUPPORT)
#endif /* SID_SDK_CONFIG_ENABLE_WIFI */

#ifndef SID_SDK_CONFIG_ENABLE_GNSS
#  define SID_SDK_CONFIG_ENABLE_GNSS                (SID_SDK_CONFIG_ENABLE_LOCATION && LR11XX_RADIO_CFG_GEOLOCATION_SUPPORT)
...
#if LR11XX_RADIO_CFG_GEOLOCATION_SUPPORT && (SID_SDK_CONFIG_ENABLE_GNSS || SID_SDK_CONFIG_ENABLE_WIFI)
typedef void (*radio_lr11xx_lbm_event_notify_cb_t)(void);
#endif /* LR11XX_RADIO_CFG_GEOLOCATION_SUPPORT && (SID_SDK_CONFIG_ENABLE_GNSS || SID_SDK_CONFIG_ENABLE_WIFI) */
...
#  if  SID_SDK_CONFIG_ENABLE_GNSS
        struct {
            smtc_modem_gnss_constellation_t  active_constellations; /*!< Constellations to use in GNSS scan. Can be SMTC_MODEM_GNSS_CONSTELLATION_GPS for GPS-only, SMTC_MODEM_GNSS_CONSTELLATION_BEIDOU for BeiDou, or SMTC_MODEM_GNSS_CONSTELLATION_GPS_BEIDOU for both */
            smtc_modem_gnss_mode_t           scan_mode;             /*!< GNSS scan mode. SMTC_MODEM_GNSS_MODE_MOBILE is faster and suits better for moving devices, SMTC_MODEM_GNSS_MODE_STATIC is slower due to additional wait time for sky diversity and suits better for static objects */
            uint16_t                         retry_interval_s;      /*!< Delay between GNSS rescans (in seconds). The scan will be relaunched after this delay */
            uint8_t                          retry_limit;           /*!< Maximum number of retries if GNSS scan fails or does not discover enough space vehicles. 0 means just a single scan, no retries */
        } gnss;
#  endif /* SID_SDK_CONFIG_ENABLE_GNSS */
#  if  SID_SDK_CONFIG_ENABLE_WIFI
        struct {
            uint8_t                          min_results;           /*!< Minimum quantity of discovered WiFi networks to estimate location. Most of location resolving services require at least two networks for privacy reasons */
            uint8_t                          retry_limit;           /*!< Maximum number of retries if WiFi scan fails or does not discover enough WiFi networks nearby. 0 means just a single scan, no retries */
            uint16_t                         retry_interval_s;      /*!< Delay between WiFi rescans (in seconds). The scan will be relaunched after this delay */
        } wifi;
#  endif /* SID_SDK_CONFIG_ENABLE_WIFI */
    } location;
#endif /* LR11XX_RADIO_CFG_GEOLOCATION_SUPPORT && (SID_SDK_CONFIG_ENABLE_GNSS || SID_SDK_CONFIG_ENABLE_WIFI) */
