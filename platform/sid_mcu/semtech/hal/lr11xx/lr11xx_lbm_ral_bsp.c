...
SID_STM32_SPEED_OPTIMIZED void ral_lr11xx_bsp_get_xosc_cfg(const void * context, ral_xosc_cfg_t * xosc_cfg, lr11xx_system_tcxo_supply_voltage_t * supply_voltage, uint32_t * startup_time_in_tick)
{
    ...
    *startup_time_in_tick = LR11XX_US_TO_TUS((LR11XX_RADIO_CFG_USE_DEFAULT_TIMINGS == drv_ctx->config->tcxo_config.timeout_us) ? LR11XX_RADIO_TCXO_SUBGHZ_DEFAULT_START_TIME_US : drv_ctx->config->tcxo_config.timeout_us);
}
...
SID_STM32_SPEED_OPTIMIZED void ral_lr11xx_bsp_get_crc_state(const void * context, bool * crc_is_activated)
{
    ...
    SID_PAL_ASSERT(drv_ctx != NULL);
    SID_PAL_ASSERT(drv_ctx->config != NULL);
    SID_PAL_ASSERT(drv_ctx->config->bus_selector.client_selector_context != NULL);

    /* Pop SPI CRC selection from the SPI bus config */
    const sid_pal_serial_bus_stm32wbaxx_transaction_config_t * const radio_spi_transaction_config = (const sid_pal_serial_bus_stm32wbaxx_transaction_config_t *)drv_ctx->config->bus_selector.client_selector_context;
...
#  endif /* LR11XX_RADIO_CFG_USE_STATUS_LED */
        ...
        {
            if (LR11XX_RADIO_CFG_USE_DEFAULT_TIMINGS == drv_ctx->config->tcxo_config.gnss_timeout_us)
            {
                tcxo_start_timeout = LR11XX_US_TO_TUS(LR11XX_RADIO_TCXO_GNSS_DEFAULT_START_TIME_US);
            }
            else
            {
...
SID_STM32_SPEED_OPTIMIZED void geolocation_bsp_gnss_postscan_actions(void)
{
    ...
    do
    {
        ...
        if (LR11XX_TCXO_CTRL_VTCXO == drv_ctx->config->tcxo_config.ctrl)
        {
            tcxo_start_timeout = LR11XX_US_TO_TUS((LR11XX_RADIO_CFG_USE_DEFAULT_TIMINGS == drv_ctx->config->tcxo_config.timeout_us) ? LR11XX_RADIO_TCXO_SUBGHZ_DEFAULT_START_TIME_US : drv_ctx->config->tcxo_config.timeout_us);
        }
        else /* if (LR11XX_TCXO_CTRL_VDD == drv_ctx.config->tcxo_config.ctrl) */
        {
            /**
