...
SID_STM32_SPEED_OPTIMIZED lr11xx_hal_status_t lr11xx_hal_init_gpio(const halo_drv_semtech_ctx_t * const drv_ctx, const uint32_t restore_from_standby)
{
    ...
    do
    {
...
#endif /* CFG_LPM_STDBY_SUPPORTED */
        {
            ...
            sid_err = sid_pal_gpio_pull_mode(drv_ctx->config->gpio.radio_reset, SID_PAL_GPIO_PULL_NONE);
            if (sid_err != SID_ERROR_NONE)
            {
                LR11XX_RADIO_HAL_LOG_ERROR("Failed to configure LR11xx reset GPIO pull");
                err = LR11XX_HAL_STATUS_ERROR;
                break;
            }
#if (CFG_LPM_STDBY_SUPPORTED != 0)
            sid_err = sid_pal_gpio_ext_ifc_standby_retention_enable(drv_ctx->config->gpio.radio_reset);
            if (sid_err != SID_ERROR_NONE)
            {
                LR11XX_RADIO_HAL_LOG_ERROR("Failed to enable LR11xx reset GPIO standby retention");
                err = LR11XX_HAL_STATUS_ERROR;
                break;
            }
#endif /* CFG_LPM_STDBY_SUPPORTED */
        ...
        {
            sid_err = sid_pal_gpio_ext_ifc_restore_output_after_standby(drv_ctx->config->gpio.radio_reset, SID_PAL_GPIO_OUTPUT_PUSH_PULL, SID_PAL_GPIO_PULL_NONE);
            if (sid_err != SID_ERROR_NONE)
            {
                LR11XX_RADIO_HAL_LOG_ERROR("Failed to restore LR11xx reset GPIO");
                err = LR11XX_HAL_STATUS_ERROR;
                break;
            }
        }
        ...
        /* Ensure any sub-GHz FEM-related pins are set to keep FEM off */
...
#if LR11XX_RADIO_CFG_GEOLOCATION_SUPPORT
        /* Ensure any GNSS FEM-related pins are set to keep FEM off */
        (void)radio_lr11xx_set_gnss_fem_mode(LR11XX_GNSS_FRONTEND_MODE_OFF);
...
SID_STM32_SPEED_OPTIMIZED lr11xx_hal_status_t lr11xx_hal_deinit_gpio(const halo_drv_semtech_ctx_t * const drv_ctx)
{
    lr11xx_hal_status_t err;
    ...
    /* Set all GPIO pins to the initial state that keeps LR11xx in reset and all FEMs off */
    err = lr11xx_hal_init_gpio(drv_ctx, FALSE);
