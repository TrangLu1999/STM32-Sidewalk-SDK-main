...
#define SID_STM32WBA_SYS_TIMER_RTC_MAX_PPM_TO_COMPENSATE (SID_STM32WBA_SYS_TIMER_RTC_MAX_PPM)
#define SID_STM32WBA_SYS_TIMER_RTC_PPM_DENOMINATOR_SQRT  (1000u)
#define SID_STM32WBA_SYS_TIMER_RTC_PPM_DENOMINATOR       (SID_STM32WBA_SYS_TIMER_RTC_PPM_DENOMINATOR_SQRT * SID_STM32WBA_SYS_TIMER_RTC_PPM_DENOMINATOR_SQRT)
...
} sid_stm32wba_sys_timer_drift_t;

/*----------------------------------------------------------------------------*/

static sid_stm32wba_sys_timer_drift_t uptime_drift = {
        .pos     = SID_TIME_ZERO,
        .neg     = SID_TIME_ZERO,
...
static inline void        time_add(struct sid_timespec * const tm1, const struct sid_timespec * const tm2);
static inline void        time_sub(struct sid_timespec * const tm1, const struct sid_timespec * const tm2);
static inline sid_error_t sid_stm32wba_sys_timer_rtc_get(struct sid_timespec * const result, sid_stm32wba_sys_timer_drift_t * const drift_info);
static inline int16_t     sid_stm32wba_sys_timer_get_time_ppm(void);
static inline int8_t      sid_stm32wba_sys_timer_calculate_drift(struct sid_timespec * const uptime, const struct sid_timespec * const old_uptime, const int16_t ppm, struct sid_timespec * const drift);
static inline void        sid_stm32wba_sys_timer_update_global_drift(const int16_t ppm);
...
SID_STM32_SPEED_OPTIMIZED static sid_error_t sid_stm32wba_sys_timer_rtc_get(struct sid_timespec * const result, sid_stm32wba_sys_timer_drift_t * const drift_info)
{
...
SID_STM32_SPEED_OPTIMIZED static inline int8_t sid_stm32wba_sys_timer_calculate_drift(struct sid_timespec * const uptime, const struct sid_timespec * const old_uptime, const int16_t ppm, struct sid_timespec * const drift)
{
    ...
    struct sid_timespec elapsed_ts = *uptime;
    uint32_t            sec_drift  = 0u;
    uint32_t            usec_drift = 0u;
    uint32_t            nsec_drift = 0u;
    uint32_t            abs_ppm    = (uint32_t)abs(ppm);

    /* Calculate the elapsed time since last drift update */
    time_sub(&elapsed_ts, old_uptime);

    /* Non-overflow calculation without 64-bit operations */
    /* 1. Calculate the drift in step of sec */
    sec_drift = (elapsed_ts.tv_sec / SID_STM32WBA_SYS_TIMER_RTC_PPM_DENOMINATOR) * abs_ppm;

    /* 2. The remainder drift in usec */
    usec_drift = (elapsed_ts.tv_sec % SID_STM32WBA_SYS_TIMER_RTC_PPM_DENOMINATOR) * abs_ppm;

    /* 3. Calculate the drift of nanosec part. Use two-step denominator division to minimize integer math losses */
    nsec_drift = ((elapsed_ts.tv_nsec / SID_STM32WBA_SYS_TIMER_RTC_PPM_DENOMINATOR_SQRT) * abs_ppm) / SID_STM32WBA_SYS_TIMER_RTC_PPM_DENOMINATOR_SQRT;

    /* Last, normalize usec_drift into sec_drift and nsec_drift */
    drift->tv_sec  = sec_drift  +  (usec_drift / SID_TIME_USEC_PER_SEC);
    drift->tv_nsec = nsec_drift + ((usec_drift % SID_TIME_USEC_PER_SEC) * SID_TIME_NSEC_PER_USEC);
...
SID_STM32_SPEED_OPTIMIZED static inline void sid_stm32wba_sys_timer_update_global_drift(const int16_t ppm)
{
    sid_error_t           err;
...
{
    sid_stm32wba_sys_timer_drift_t drift_info;
    struct sid_timespec            drift = SID_TIME_ZERO;
    ...
    err = sid_stm32wba_sys_timer_rtc_get(result, &drift_info);
    if (err != SID_ERROR_NONE)
    {
        return err;
    }
...
SID_STM32_SPEED_OPTIMIZED void sid_stm32wba_sys_timer_set_xtal_ppm(int16_t ppm)
{
    ...
    /* Boundary checks */
    if (ppm > SID_STM32WBA_SYS_TIMER_RTC_MAX_PPM_TO_COMPENSATE)
    {
        /* Trim to upper boundary */
        ppm = SID_STM32WBA_SYS_TIMER_RTC_MAX_PPM_TO_COMPENSATE;
    }
    else if (ppm < (-SID_STM32WBA_SYS_TIMER_RTC_MAX_PPM_TO_COMPENSATE))
    {
