...
SID_STM32_SPEED_OPTIMIZED static inline spi_client_context_t * _find_spi_client_ctx(const serial_bus_spi_ctx * const bus_ctx, const struct sid_pal_serial_bus_client * const client)
{
    ...
    {
        /* Iterate over the list of client contexts and try to find a matching one */
        tListNode * iterator;
...
SID_STM32_SPEED_OPTIMIZED static inline sid_error_t _create_spi_client_ctx(const serial_bus_spi_ctx * const bus_ctx, const struct sid_pal_serial_bus_client * const client, spi_client_context_t ** const out_client_ctx)
{
    ...
    do
    {
        ...
        if (client->speed_hz < spi_peripheral_clock)
        {
            ...
            spi_baud_rate_prescaler = SID_STM32_UTIL_ROUNDUP_TO_POW_2_EXP((spi_peripheral_clock + (client->speed_hz - 1u))/ client->speed_hz);
        ...
        if (new_client_ctx->peripherals_cfg.nss_gpio_port != NULL)
        {
            ...
            err = sid_pal_gpio_output_mode(client->client_selector, LL_GPIO_OUTPUT_PUSHPULL);
            SID_PAL_ASSERT(SID_ERROR_NONE == err);

            err = sid_pal_gpio_pull_mode(client->client_selector, SID_PAL_GPIO_PULL_NONE);
            SID_PAL_ASSERT(SID_ERROR_NONE == err);

            err = sid_pal_gpio_set_direction(client->client_selector, SID_PAL_GPIO_DIRECTION_OUTPUT);
            SID_PAL_ASSERT(SID_ERROR_NONE == err);

#if (CFG_LPM_STDBY_SUPPORTED != 0)
            /* Enable retention in Standby LPM mode */
            err = sid_pal_gpio_ext_ifc_standby_retention_enable(client->client_selector);
            SID_PAL_ASSERT(SID_ERROR_NONE == err);
            ...
            uint32_t lptim_prescaler = SID_STM32_UTIL_ROUNDUP_TO_POW_2_EXP((nss_lptim_base_clock_hz + (client->speed_hz - 1u)) / client->speed_hz);
        ...
        if (client->client_selector_context != NULL)
        {
            const sid_pal_serial_bus_stm32wbaxx_transaction_config_t * const xfer_cfg = (sid_pal_serial_bus_stm32wbaxx_transaction_config_t *)client->client_selector_context;
...
static sid_error_t _sid_pal_serial_bus_spi_stm32wbaxx_destroy(const struct sid_pal_serial_bus_iface * iface)
{
    ...
    do
    {
        ...
        if (bus_ctx == &bus)
        {
            ...
            if (bus_ctx->factory_config->hlptim != NULL)
            {
            ...
            /* Bring down LPTIM */
            if ((bus_ctx->factory_config->hlptim != NULL) && (bus_ctx->factory_config->hlptim->State != HAL_LPTIM_STATE_RESET))
            {
...
#if (CFG_LPM_STDBY_SUPPORTED != 0)
SID_STM32_SPEED_OPTIMIZED void sid_pal_serial_bus_ext_ifc_notify_standby_exit(void)
{
    sid_error_t err;
    ...
    do
    {
        serial_bus_spi_ctx * bus_ctx = &bus;

        if ((NULL == bus_ctx->factory_config) || (LST_is_empty((tListNode *)&bus_ctx->client_contexts) != FALSE))
        {
            /* SPI is not initialized or is not used by any client - no need to restore configuration after leaving Standby LPM */
            break;
        }

        /* Restore NSS pin states for all clients */
        tListNode * iterator;
        LST_get_next_node((tListNode *)&bus_ctx->client_contexts, &iterator);

        /* Iterate over the list of client contexts */
        while (iterator != &(bus_ctx->client_contexts))
        {
            spi_client_context_t * const client_ctx = containerof(iterator, spi_client_context_t, node);
            ...
            /* Proceed with GPIO configuration if NSS is used for this client */
            if (client_ctx->client_selector != GPIO_PORT_PIN_NOT_CONNECTED)
            {
                err = sid_pal_gpio_ext_ifc_restore_output_after_standby(client_ctx->client_selector, SID_PAL_GPIO_OUTPUT_PUSH_PULL, SID_PAL_GPIO_PULL_NONE);
                SID_PAL_ASSERT(SID_ERROR_NONE == err);
            }
        }

        /* Re-initialize LPTIM that will be used for NSS pin timings */
        if (bus_ctx->factory_config->hlptim != NULL)
        {
            /* Clear HAL instances of any related DMA channels */
            for (uint32_t i = 0u; i < SID_STM32_UTIL_ARRAY_SIZE(bus_ctx->factory_config->hlptim->hdma); i++)
            {
                if (bus_ctx->factory_config->hlptim->hdma[i] != NULL)
                {
                    SID_STM32_UTIL_fast_memset(bus_ctx->factory_config->hlptim->hdma[i], 0u, sizeof(*(bus_ctx->factory_config->hlptim->hdma[i])));
                }
            }

            /* Clear HAL instance of the LPTIM itself */
            SID_STM32_UTIL_fast_memset(bus_ctx->factory_config->hlptim, 0u, sizeof(*(bus_ctx->factory_config->hlptim)));

            /* Re-initialize LPTIM and any underlying DMA channels */
            err = _nss_lptim_init(bus_ctx);
            SID_PAL_ASSERT(SID_ERROR_NONE == err);
        }

        /* Clear SPI DMA Rx HAL instance */
        if (bus_ctx->factory_config->hspi->hdmarx != NULL)
        {
            SID_STM32_UTIL_fast_memset(bus_ctx->factory_config->hspi->hdmarx, 0u, sizeof(*(bus_ctx->factory_config->hspi->hdmarx)));
        }

        /* Clear SPI DMA Tx HAL instance */
        if (bus_ctx->factory_config->hspi->hdmatx != NULL)
        {
            SID_STM32_UTIL_fast_memset(bus_ctx->factory_config->hspi->hdmatx, 0u, sizeof(*(bus_ctx->factory_config->hspi->hdmatx)));
        }

        /* Clear HAL instance of the SPI itself */
        SID_STM32_UTIL_fast_memset(bus_ctx->factory_config->hspi, 0u, sizeof(*(bus_ctx->factory_config->hspi)));

        /* Re-initialize SPI and any underlying DMA channels */
        err = _spi_peripheral_init(bus_ctx);
        SID_PAL_ASSERT(SID_ERROR_NONE == err);
    } while (0);
}
#endif /* CFG_LPM_STDBY_SUPPORTED */
