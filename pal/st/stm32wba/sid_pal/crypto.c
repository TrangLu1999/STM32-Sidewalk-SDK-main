...
/* ECC working buffer */
SID_STM32_ALIGN_4BYTES(static uint8_t working_buffer[BUFF_SIZE]); /* CMOX buffer hast to be aligned to 4-byte boundary due to the use of alignment-sensitive assembly instructions */
...
static inline sid_error_t        stm32wba_crypto_obtain_hw_control(void);
...
static        cmox_hash_retval_t stm32wba_crypto_hash(sid_pal_hash_params_t * const params);
...
SID_STM32_SPEED_OPTIMIZED static inline sid_error_t stm32wba_crypto_obtain_hw_control(void)
{
    sid_error_t err;

    do
    {
        /* Lock CRC to avoid concurrent access by multiple users */
        const CRCCTRL_Cmd_Status_t crc_lock_status = CRCCTRL_MutexTake();
        if (crc_lock_status != CRCCTRL_OK)
        {
            SID_PAL_LOG_ERROR("Unable to lock CRC access: %d", (int32_t)crc_lock_status);
        ...
        /* Configure CRC for CMOX library */
...
SID_STM32_SPEED_OPTIMIZED static inline void stm32wba_crypto_release_hw_control(void)
{
    ...
    /* Restore the rest of the registers - this is required to keep compatibility with CRC_CTRL module */
...
SID_STM32_SPEED_OPTIMIZED static cmox_hash_retval_t stm32wba_crypto_hash(sid_pal_hash_params_t * const params)
{
    ...
    sid_err = stm32wba_crypto_obtain_hw_control();
        ...
        case SID_PAL_CRYPTO_ENCRYPT:
            {
                ...
                SID_STM32_UTIL_fast_memcpy(params->out, working_buffer, expected_out_size);
                params->out_size = expected_out_size;

                if (params->mac != NULL)
                {
                    SID_STM32_UTIL_fast_memcpy(params->mac, &(working_buffer[expected_out_size]), expected_mac_size);
        ...
        case SID_PAL_CRYPTO_DECRYPT:
            {
                ...
                SID_STM32_UTIL_fast_memcpy(working_buffer, params->in, params->in_size);
                SID_STM32_UTIL_fast_memcpy(&(working_buffer[params->in_size]), params->mac, params->mac_size);
                                                                           ...
                                                                           const size_t hash_size)
{
    ...
    if (in_params->algo == SID_PAL_ECDSA_SECP256R1)
    {
        ...
        ret = stm32wba_crypto_hash(&hash_params);
    ...
    switch (params->algo)
    {
        case SID_PAL_EDDSA_ED25519:
            ...
            else if (CMOX_ECC_ED25519_PRIVKEY_LEN == params->key_size)
            {
                ...
                SID_STM32_UTIL_fast_memcpy(eddsa_priv_key, params->key, CMOX_ECC_ED25519_PRIVKEY_LEN);
                                                                   ...
                                                                   size_t * const            P_pPubKeyLen)
{
    ...
    SID_STM32_UTIL_fast_memcpy(P_pPrivKey, P_pRandom, P_RandomLen);
    P_pPrivKey[0] &= 0xF8u;
        ...
        case SID_PAL_EDDSA_ED25519:
            {
                ...
                do
                {
                    sid_err = stm32wba_crypto_rand(random_data, sizeof(random_data));
                    if (sid_err != SID_ERROR_NONE)
                    {
...
SID_STM32_SPEED_OPTIMIZED static sid_error_t stm32wba_crypto_ecc_dsa(sid_pal_dsa_params_t * const params)
{
    ...
    switch (params->algo)
    {
        case SID_PAL_EDDSA_ED25519:
            ...
            {
                status = stm32wba_crypto_obtain_hw_control();
        ...
        case SID_PAL_ECDH_SECP256R1:
            {
                ...
                /* Get xclusive access to CRC peripheral and configure it properly */
                status = stm32wba_crypto_obtain_hw_control();
                if (status != SID_ERROR_NONE)
                {
                    return status;
                ...
                else
                {
                    ...
                    SID_STM32_UTIL_fast_memcpy(params->shared_secret, computed_secret, useful_secret_bytes);
...
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_pal_crypto_hash(sid_pal_hash_params_t* params)
{
    ...
    retval = stm32wba_crypto_hash(params);
