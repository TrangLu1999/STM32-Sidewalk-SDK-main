...
/* SID PAL interface functions require to pass one number for pin and port. This macro converts GPIO pin and port to a single number */
#define GPIO_PORT_PIN_TO_NUM(port, pin) ((ST_GPIO_PORT_TO_NUM(port) << (NUM_GPIO_PORT_OFFSET)) | ((pin) & NUM_GPIO_PIN_MASK))
 ...
 * @param[in] gpio_irq_handler The callback function to be called on wakeup interrupt.
 * @param[in] callback_arg     User-defined callback argument.
 *
 * @retval SID_ERROR_NONE in case of success.
 */
sid_error_t sid_pal_gpio_ext_ifc_wakeup_enable(const uint32_t gpio_number, const sid_pal_gpio_irq_trigger_t edge, const sid_pal_gpio_irq_handler_t callback, void * const callback_arg);

/**
 * @brief Disable GPIO pin as a wakeup source for the Standby LPM
 *
 * @param[in] gpio_number The logical GPIO number to operate on.
 *
 * @retval SID_ERROR_NONE in case of success.
 */
sid_error_t sid_pal_gpio_ext_ifc_wakeup_disable(const uint32_t gpio_number);

/**
 * @brief Enable retention of a GPIO pin in the Standby LPM
 * @note GPIO retention on STM32WBA platform is achieved through internal pull-up and pull-down resistors for all types of GPIO pins.
 *       If the pin is an output driving a low-impedance load or sinking current, the retained logical level cannot be guaranteed.
 *
 * @param[in] gpio_number The logical GPIO number to operate on.
 *
 * @retval SID_ERROR_NONE in case of success.
 */
sid_error_t sid_pal_gpio_ext_ifc_standby_retention_enable(const uint32_t gpio_number);

/**
 * @brief Disable retention of a GPIO pin in the Standby LPM
 *
 * @param[in] gpio_number The logical GPIO number to operate on.
 *
 * @retval SID_ERROR_NONE in case of success.
 */
sid_error_t sid_pal_gpio_ext_ifc_standby_retention_disable(const uint32_t gpio_number);

/**
 * @brief Restore GPIO pin output mode and state based on the the state retained throughout the standby low power mode
 *
 * @param[in] gpio_number The logical GPIO number to operate on.
 * @param[in] output_mode The output mode to set.
 * @param[in] pull        The pull mode to set.
 *
 * @retval SID_ERROR_NONE in case of success.
 */
sid_error_t sid_pal_gpio_ext_ifc_restore_output_after_standby(const uint32_t gpio_number, const sid_pal_gpio_output_t output_mode, const sid_pal_gpio_pull_t pull);
