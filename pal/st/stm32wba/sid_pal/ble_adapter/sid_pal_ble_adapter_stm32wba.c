...
#define SID_STM32_BLE_INTERNAL_DB_VALUE_TO_HCI_COMPENSATION(_X_)    ((_X_) > 0 ? (int16_t)(((_X_) + 5) / 10) : (int16_t)(((_X_) - 5) / 10)) /*!< Convert internal fixed-point representation of power level (dB) to HCI fixed-point format for Tx/RX path compensation */
...
SID_STM32_SPEED_OPTIMIZED static sid_error_t _gap_and_gatt_params_init(const sid_pal_ble_prv_operating_mode_t init_type)
{
    ...
    SID_BLE_GENERIC_LOG_DEBUG("==>> Start gap_and_gatt_params_init function");
    ...
    do
    {
        ...
        if (NULL == sid_ble_drv_ctx.cfg)
        {
            SID_PAL_LOG_ERROR("Cannot initialize BLE GAP and GATT params. BLE config is NULL");
            err = SID_ERROR_INVALID_ARGS;
            break;
        }
        ...
        if ((init_type != SPBP_OPERATING_MODE_SIDEWALK) && (init_type != SPBP_OPERATING_MODE_USER))
...
#endif /* SID_STM32_BLE_COEXISTENCE_MODE */
        {
            SID_PAL_LOG_ERROR("Mismatch between the BLE driver cfg and requested init type. Cfg mode: %u, init type: %u", SID_STM32_BLE_COEXISTENCE_MODE, (uint32_t)init_type);
        ...
        const uint64_t hci_le_event_mask = (
                            ...
                            /* SID_BLE_HCI_LE_EVENT_MASK_CIS_ESTABLISHED_V2 - not used */
                            /* SID_BLE_HCI_LE_EVENT_MASK_READ_ALL_REMOTE_FEATURES_COMPLETE - not used */
                            /* SID_BLE_HCI_LE_EVENT_MASK_CS_READ_REMOTE_SUPPORTED_CAPABILITIES_COMPLETE - not used */
                            /* SID_BLE_HCI_LE_EVENT_MASK_CS_READ_REMOTE_FAE_TABLE_COMPLETE - not used */
                            /* SID_BLE_HCI_LE_EVENT_MASK_CS_SECURITY_ENABLE_COMPLETE - not used */
                            /* SID_BLE_HCI_LE_EVENT_MASK_CS_CONFIG_COMPLETE - not used */
                            /* SID_BLE_HCI_LE_EVENT_MASK_CS_PROCEDURE_ENABLE_COMPLETE - not used */
                            /* SID_BLE_HCI_LE_EVENT_MASK_CS_SUBEVENT_RESULT - not used */
                            /* SID_BLE_HCI_LE_EVENT_MASK_CS_SUBEVENT_RESULT_CONTINUE - not used */
                            /* SID_BLE_HCI_LE_EVENT_MASK_CS_TEST_END_COMPLETE - not used */
                            /* SID_BLE_HCI_LE_EVENT_MASK_MONITORED_ADVERTISERS_REPORT - not used */
                            /* SID_BLE_HCI_LE_EVENT_MASK_FRAME_SPACE_UPDATE_COMPLETE - not used */
        );
        ...
        SID_BLE_GENERIC_LOG_DEBUG("  Success: hci_le_set_event_mask command");
        ...
        SID_BLE_GENERIC_LOG_DEBUG("  Success: rotate_random_mac_address");
        ...
        SID_BLE_GENERIC_LOG_DEBUG("  Success: aci_hal_write_config_data command - CONFIG_DATA_IR_OFFSET");
        ...
        SID_BLE_GENERIC_LOG_DEBUG("  Success: aci_hal_write_config_data command - CONFIG_DATA_ER_OFFSET");

        /* Compute Tx and Rx paths loss/gain */
        const int16_t tx_path_adjustment = SID_STM32_BLE_INTERNAL_DB_VALUE_TO_HCI_COMPENSATION(
                                    sid_ble_drv_ctx.cfg->pa_config.ant_gain
#if SID_STM32_BLE_CFG_USE_EXTERNAL_PA
                                    + sid_ble_drv_ctx.cfg->pa_config.ext_pa_gain
#endif /* SID_STM32_BLE_CFG_USE_EXTERNAL_PA */
                                    - sid_ble_drv_ctx.cfg->pa_config.tx_insertion_loss
                                );

        const int16_t rx_path_adjustment = SID_STM32_BLE_INTERNAL_DB_VALUE_TO_HCI_COMPENSATION(
                                    -sid_ble_drv_ctx.cfg->pa_config.ant_gain
#if SID_STM32_BLE_CFG_USE_EXTERNAL_LNA
                                    - sid_ble_drv_ctx.cfg->pa_config.lna_gain
#endif /* SID_STM32_BLE_CFG_USE_EXTERNAL_LNA */
                                    + sid_ble_drv_ctx.cfg->pa_config.rx_insertion_loss
                                );

        /* Apply the calculated corrections to the radio - after this point no further adjustement of the Tx power or RSSI values is needed in the driver, compensation is handled by the Link Layer internally, you are ok to use desired values in dB(m) as is */
        ret = hci_le_write_rf_path_compensation((uint16_t)tx_path_adjustment, (uint16_t)rx_path_adjustment);
        if (ret != BLE_STATUS_SUCCESS)
        {
            SID_PAL_LOG_ERROR("hci_le_write_rf_path_compensation 0x%02X", ret);
        }
        SID_BLE_GENERIC_LOG_DEBUG("  Success: hci_le_write_rf_path_compensation command. Tx offset: %d, Rx offset: %d", tx_path_adjustment, rx_path_adjustment);

        /* Configure Tx power */
        int8_t applied_tx_power;
        err = sid_stm32wba_ble_adapter_prv_generic_set_tx_power(sid_ble_drv_ctx.cfg->pa_config.max_tx_power_dbm, &applied_tx_power); /* No need to use BLE_WRAP_PREPROC/BLE_WRAP_POSTPROC here since LL mutex is locked already during the initialization */
        if (err != SID_ERROR_NONE)
        {
            /* Logs are provided by sid_stm32wba_ble_adapter_prv_generic_set_tx_power() */
            break;
        }

        /* Printout a warning message if the actual Tx EIRP differs from the desired Tx power */
        if (applied_tx_power != sid_ble_drv_ctx.cfg->pa_config.max_tx_power_dbm)
        {
            SID_PAL_LOG_WARNING("Global BLE Tx power limit trimmed to %s%ddBm from %s%ddBm due to the HW limits",
                                applied_tx_power < 0 ? "" : "+", applied_tx_power,    /* %+d format may not be supported by tiny_vsnprintf() */
                                sid_ble_drv_ctx.cfg->pa_config.max_tx_power_dbm < 0 ? "" : "+", sid_ble_drv_ctx.cfg->pa_config.max_tx_power_dbm); /* %+d format may not be supported by tiny_vsnprintf() */
        }

        /* Printout the applied Tx power level */
        SID_PAL_LOG_INFO("Global BLE Tx power set to %s%ddBm", applied_tx_power < 0 ? "" : "+", applied_tx_power);
        ...
        SID_BLE_GENERIC_LOG_DEBUG("  Success: aci_gatt_init command");
        ...
        SID_BLE_GENERIC_LOG_DEBUG("  Success: aci_gap_init command");
        ...
        if (gap_device_name_len > 0u)
        {
            ...
            SID_BLE_GENERIC_LOG_DEBUG("  Success: aci_gatt_update_char_value - Device Name");
        }
        ...
        SID_BLE_GENERIC_LOG_DEBUG("  Success: aci_gatt_update_char_value - Appearance");
        ...
        SID_BLE_GENERIC_LOG_DEBUG("  Success: hci_le_set_default_phy command");
        ...
        SID_BLE_GENERIC_LOG_DEBUG("  Success: hci_le_write_suggested_default_data_length command");
        ...
        SID_BLE_GENERIC_LOG_DEBUG("  Success: aci_gap_set_io_capability command");
        ...
        SID_BLE_GENERIC_LOG_DEBUG("  Success: aci_gap_set_authentication_requirement command");
        ...
        SID_BLE_GENERIC_LOG_DEBUG("  Success: hci_le_set_address_resolution_enable command");
        ...
        SID_BLE_GENERIC_LOG_DEBUG("  Success: hci_le_set_resolvable_private_address_timeout command");
        ...
        SID_BLE_GENERIC_LOG_DEBUG("  Success: aci_gap_set_event_mask command");
        ...
        SID_BLE_GENERIC_LOG_DEBUG("  Success: aci_gatt_set_event_mask command");

        err = SID_ERROR_NONE;
    } while (0);

    SID_BLE_GENERIC_LOG_DEBUG("==>> End gap_and_gatt_params_init function");
...
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_stm32wba_ble_adapter_prv_rotate_random_mac_address(sid_pal_ble_prv_bt_addr_buffer_t * const out_addr, const sid_ble_cfg_mac_address_type_t addr_type)
{
    ...
    do
    {
        ...
        SID_BLE_GENERIC_LOG_DEBUG("  Success: aci_hal_write_config_data command - CONFIG_DATA_RANDOM_ADDRESS_OFFSET");
        ...
        SID_BLE_GENERIC_LOG_DEBUG("  Success: hci_le_set_random_address command");
...
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_stm32wba_ble_adapter_prv_generic_init(const sid_pal_ble_prv_operating_mode_t init_type, const sid_ble_config_t * const sid_ble_cfg)
{
    ...
    SID_BLE_GENERIC_LOG_DEBUG("==>> Start ble_adapter_generic_init function");
    ...
    do
    {
        ...
        /* Store the targeted driver operating mode */
...
#if (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED != 0)
        /* Enable radio RAM retention in Standby mode */
        HAL_PWREx_EnableRadioSRAMClockStandbyRetention(PWR_RADIOSRAM_FULL_STANDBY_RETENTION);
#endif /* (CFG_LPM_STDBY_SUPPORTED != 0) && (CFG_LPM_LEVEL != 0) */
    ...
    SID_BLE_GENERIC_LOG_DEBUG("==>> End ble_adapter_generic_init function");
...
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_stm32wba_ble_adapter_prv_generic_deinit(void)
{
    ...
    SID_BLE_GENERIC_LOG_DEBUG("==>> Start ble_adapter_generic_deinit function");
...
#if (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED != 0)
        /* Disable radio RAM retention in Standby mode */
        HAL_PWREx_DisableRadioSRAMClockStandbyRetention();
#endif /* (CFG_LPM_STDBY_SUPPORTED != 0) && (CFG_LPM_LEVEL != 0) */

        /* Enable RFTS Bypass for flash operation - since LL is terminated */
    ...
    SID_BLE_GENERIC_LOG_DEBUG("==>> End ble_adapter_generic_deinit function");
...
#if SID_STM32_BLE_EXTENDED_ADV_SUPPORTED
{
    ...
    do
    {
        ...
        SID_BLE_GENERIC_LOG_DEBUG("==>> hci_le_set_extended_advertising_enable - Success, set: 0x%02X", adv_set->Advertising_Handle);
...
#else
{
    ...
    do
    {
        ...
        SID_BLE_GENERIC_LOG_DEBUG("==>> hci_le_set_advertising_enable - Success");
...
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_stm32wba_ble_adapter_prv_generic_stop_advertisement(const uint32_t adv_set_handle)
{
    ...
    do
    {
        ...
        SID_BLE_GENERIC_LOG_DEBUG("==>> hci_le_set_extended_advertising_disable - Success, set: 0x%02X", adv_set_handle);
        ...
        SID_BLE_GENERIC_LOG_DEBUG("==>> hci_le_set_advertising_disable - Success");
        if (err != SID_ERROR_NONE)
        {
            /* Terminate from here */
            break;
...
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_stm32wba_ble_adapter_prv_generic_set_tx_power(const int8_t desired_tx_power_dbm, int8_t * const out_actual_tx_power)
{
    sid_error_t err;
    tBleStatus ble_status;
    int8_t power_to_set, out_tx_pwr;
    int8_t tx_power_min, tx_power_max;

    /**
     * @attention This is an internal method and it's not supposed to be inovked directly. Calls to this method shall be wrapped with
     *            BLE_WRAP_PREPROC() and BLE_WRAP_POSTPROC() to ensure correct operation. Failing to do so may result in undefined
     *            behavior.
     */

    do
    {
        /* List the available PHYs for which we need to configure the Tx power */
        const uint8_t phy_types[] = {
            SID_BLE_HCI_TX_POWER_PHY_LE_1M,
            SID_BLE_HCI_TX_POWER_PHY_LE_2M,
            SID_BLE_HCI_TX_POWER_PHY_LE_CODED_S8,
            SID_BLE_HCI_TX_POWER_PHY_LE_CODED_S2,
        };

        uint8_t  connection_sm_states[LINK_STATUS_SIZE]; /* Array that stores the state of each internal connection state machine */
        uint16_t connection_sm_ids[LINK_STATUS_SIZE];    /* Array that stores the connection id (differs from HCI connection handle) associated with each internal state machine */

        /* Protection from systematic failures - ensure local buffers can fit all SM states and to stack overflow will happen */
        const uint32_t ll_sm_num = ll_sys_get_concurrent_state_machines_num() + 2u; /* Two SMs are reserved for non-connectable Observer and Broadcaster roles */
        SID_PAL_ASSERT(LINK_STATUS_SIZE >= ll_sm_num);

        /* Protection from systematic failures - ensure configuration is available */
        SID_PAL_ASSERT(sid_ble_drv_ctx.cfg != NULL);

        /* Ensure the request does not violate the software config limits */
        const int8_t global_power_limit = (sid_ble_drv_ctx.cfg->pa_config.max_tx_power_dbm != SID_STM32_BLE_TX_POWER_NO_PREFERENCE) ? sid_ble_drv_ctx.cfg->pa_config.max_tx_power_dbm : SID_STM32_BLE_DEFAULT_TX_POWER_DBM;
        power_to_set = desired_tx_power_dbm;
        if (power_to_set > global_power_limit)
        {
            /* Over the global power limit - trim to the user-defined limit */
            power_to_set = global_power_limit;
        }
        if ((SPBP_OPERATING_MODE_SIDEWALK == sid_ble_drv_ctx.operating_mode) || (SPBP_OPERATING_MODE_CONCURRENT == sid_ble_drv_ctx.operating_mode))
        {
            if ((sid_ble_sidewalk_ctx.ble_cfg != NULL) && (power_to_set > sid_ble_sidewalk_ctx.ble_cfg->max_tx_power_in_dbm))
            {
                /* Over the global power limit - trim to the user-defined limit */
                power_to_set = sid_ble_sidewalk_ctx.ble_cfg->max_tx_power_in_dbm;
            }
        }

        /* Explicitly switch to LPA table to ensure coherent state of the LL and this driver */
        ble_status = ll_intf_cmn_select_tx_power_table(SID_STM32_BLE_TX_POWER_TABLE_ID_LPA);
        if (ble_status != BLE_STATUS_SUCCESS)
        {
            SID_PAL_LOG_ERROR("  Fail   : ll_intf_cmn_select_tx_power_table command, result: 0x%02X", ble_status);
            err = SID_ERROR_IO_ERROR;
            break;
        }
        SID_BLE_GENERIC_LOG_DEBUG("  Success: ll_intf_cmn_select_tx_power_table command");

        /* Read current Tx power limits for the LPA */
        ble_status = ll_intf_le_read_tx_pwr(&tx_power_min, &tx_power_max);
        if (ble_status != BLE_STATUS_SUCCESS)
        {
            SID_PAL_LOG_ERROR("  Fail   : ll_intf_le_read_tx_pwr command, result: 0x%02X", ble_status);
            err = SID_ERROR_IO_ERROR;
            break;
        }
        SID_BLE_GENERIC_LOG_DEBUG("  Success: ll_intf_le_read_tx_pwr command");

        /* Check if we need to switch to HPA power table */
        if (power_to_set > tx_power_max)
        {
            /* Switch to HPA */
            ble_status = ll_intf_cmn_select_tx_power_table(SID_STM32_BLE_TX_POWER_TABLE_ID_HPA);
            if (ble_status != BLE_STATUS_SUCCESS)
            {
                SID_PAL_LOG_ERROR("  Fail   : ll_intf_cmn_select_tx_power_table command, result: 0x%02X", ble_status);
                err = SID_ERROR_IO_ERROR;
            ...
            /* Ensure upper boundary is not crossed */
            if (power_to_set > tx_power_max)
            {
                /* Requested Tx power is still too high, cap at the maximum output level for HPA */
                power_to_set = tx_power_max;
            }
            else
            {
                /* Desired Tx power is achievable with HPA */
            }
        }
        else
        {
            /* It's ok to use LPA, but we need to ensure the requested Tx power is not below the lower limit for LPA */
            if (power_to_set < tx_power_min)
            {
                /* Requested Tx power is too low, cap at the minimum output level for LPA */
                power_to_set = tx_power_min;
            }
            else
            {
                /* Desired Tx power is achievable with LPA */
            }
        }

        /* Set global TX power limit. This commands adjusts PA power supply and thus applies to all PHYs all together (1Mb, 2Mb, LE-coded S2, and S8) */
        ble_status = ll_intf_le_write_tx_pwr(power_to_set);
        if (ble_status != BLE_STATUS_SUCCESS)
        {
            SID_BLE_GENERIC_LOG_ERROR("  Fail   : ll_intf_le_write_tx_pwr command, result: 0x%02X, target power: %d", ble_status, power_to_set);
            err = SID_ERROR_IO_ERROR;
            break;
        }
        SID_BLE_GENERIC_LOG_DEBUG("  Success: ll_intf_le_write_tx_pwr command");

        /* Get the states of the internal SMs */
        ble_status = ll_intf_get_link_status(connection_sm_states, (uint8_t *)(void *)connection_sm_ids);
        if (ble_status != BLE_STATUS_SUCCESS)
        {
            SID_BLE_GENERIC_LOG_ERROR("  Fail   : ll_intf_get_link_status command, result: 0x%02X", ble_status);
            err = SID_ERROR_IO_ERROR;
            break;
        }
        SID_BLE_GENERIC_LOG_DEBUG("  Success: ll_intf_get_link_status command");

        /* Build connection configuration template */
        le_set_conn_tx_pwr_lvl_cmd_st tx_pwr_cmd_params = {
            .tx_power  = power_to_set,
        };

        /* Assume no error */
        err = SID_ERROR_NONE;

        /* Update power setting selection for all active connections to match new PA configuration set by ll_intf_le_write_tx_pwr(). Skipping this step will render the connections unusable */
        for (uint32_t i = 0u; i < ll_sm_num; i++)
        {
            /* Processes only the SMs with active connections */
            if ((SM_STATUS_PERI_CONN == connection_sm_states[i]) || (SM_STATUS_CENT_CONN == connection_sm_states[i]))
            {
                /* Set state machine ID */
                tx_pwr_cmd_params.conn_handle_id = connection_sm_ids[i];

                uint32_t conn_tx_pwr_cfg_errors = 0u;
                /* Loop through the list of PHYs and update the Tx power for every PHY */
                for (uint32_t j = 0u; j < SID_STM32_UTIL_ARRAY_SIZE(phy_types); j++)
                {
                    /* Update the selected PHY type in parameters */
                    tx_pwr_cmd_params.phy = phy_types[j];

                    /* Update Tx power level for the selected state machine and associated PHY */
                    ble_status = ll_intf_set_conn_tx_power_level(&tx_pwr_cmd_params, &out_tx_pwr);
                    if (ble_status != BLE_STATUS_SUCCESS)
                    {
                        SID_PAL_LOG_ERROR("Failed to update BLE Tx power for handle 0x%04X, phy 0x%02X. Error 0x%02X",
                            (connection_sm_ids[i] + 1u), /* Map the LL's internal state machine index to the HCI connection handle */
                            phy_types[j],
                            ble_status);
                        err = SID_ERROR_IO_ERROR;

                        /* Proceed with updating the other PHYs anyway */
                        continue;
                    }

                    /* Check the power level was updated correctly */
                    if ((SID_STM32_BLE_LL_POWER_VALUE_INVALID == out_tx_pwr) || (out_tx_pwr != power_to_set))
                    {
                        /* Increment error counter. Facing SID_STM32_BLE_LL_POWER_VALUE_INVALID is pretty normal for one or several PHYs (e.g., respective PHY is down), so the only issue is when all PHYs report an issue */
                        conn_tx_pwr_cfg_errors++;
                    }
                }

                /* Check if all the PHYs rejected the power change */
                if (conn_tx_pwr_cfg_errors >= SID_STM32_UTIL_ARRAY_SIZE(phy_types))
                {
                    /* Failed to apply the new power setting to any PHY */
                    SID_PAL_LOG_ERROR("Failed to update BLE Tx power for handle 0x%04X. All PHYs rejected the change",
                            (connection_sm_ids[i] + 1u)); /* Map the LL's internal state machine index to the HCI connection handle */
                    err = SID_ERROR_IO_ERROR;

                    /* Proceed with updating the other connections anyway */
                    continue;
                }
            }
        }

        /* Propagate the error from the for() loop */
        if (err != SID_ERROR_NONE)
        {
            break;
        }
        ...
        /* Done */
        if (out_actual_tx_power != NULL)
        {
            *out_actual_tx_power = power_to_set;
        }
        err = SID_ERROR_NONE;
    } while (0);
