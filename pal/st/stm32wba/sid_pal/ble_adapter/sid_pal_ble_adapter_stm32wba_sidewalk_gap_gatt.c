...
static        sid_error_t           _ble_adapter_sidewalk_get_tx_pwr(int16_t * tx_power);
...
static        sid_error_t           _ble_adapter_sidewalk_set_tx_pwr(int16_t tx_power);
static        sid_error_t           _ble_adapter_sidewalk_disconnect(void);
static        sid_error_t           _ble_adapter_sidewalk_deinit(void);
static        sid_error_t           _ble_adapter_sidewalk_get_mac_addr(uint8_t * addr);

#if SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG
static inline sid_error_t           _ble_adapter_sidewalk_handle_adv_user_config(sid_ble_user_config_t * cfg);
static inline sid_error_t           _ble_adapter_sidewalk_handle_conn_user_config(sid_ble_user_config_t * cfg);
static        sid_error_t           _ble_adapter_sidewalk_handle_user_config(sid_ble_user_config_t * cfg);
#endif /* SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG */
...
static const struct sid_pal_ble_adapter_interface sidwalk_ble_ifc = {
...
#if SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG
    .user_config          = _ble_adapter_sidewalk_handle_user_config,
#else
    .user_config          = NULL,
#endif /* SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG */
    ...
    .received_data_result = NULL,
    .disconnect           = _ble_adapter_sidewalk_disconnect,
    .deinit               = _ble_adapter_sidewalk_deinit,
    .notify_ama_state     = NULL,
    .get_mac_addr         = _ble_adapter_sidewalk_get_mac_addr,
};
...
SID_STM32_SPEED_OPTIMIZED static inline SVCCTL_EvtAckStatus_t _ble_adapter_sidewalk_process_gap_char_read_req(const uint16_t att_handle)
{
    ...
    do
    {
        ...
        {
#  if SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG
            /* Use critical section to prevent dynamic connection parameters updates */
            sid_pal_enter_critical_region();
#endif /* SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG */

            const sid_ble_cfg_conn_param_t * const conn_param =
#  if SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG
                &sid_ble_sidewalk_ctx.current_conn_param;
#  else
                &sid_ble_sidewalk_ctx.ble_cfg->conn_param;
#endif /* SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG */
...
SID_STM32_SPEED_OPTIMIZED static sid_error_t _ble_adapter_sidewalk_service_init(sid_ble_adapter_sid_gatt_svc_ctx_t * const out_svc_ctx, const sid_ble_adapter_sid_gatt_svc_init_t * const ble_svc_init)
{
    ...
    do
    {
        ...
        if (ble_status != BLE_STATUS_SUCCESS)
        {
            ...
            err = (BLE_STATUS_OUT_OF_MEMORY == ble_status) ? SID_ERROR_OOM : SID_ERROR_IO_ERROR;
            break;
        }
        else
        {
...
SID_STM32_SPEED_OPTIMIZED static sid_error_t _ble_adapter_sidewalk_init(const sid_ble_config_t * cfg)
{
    ...
    do
    {
        ...
        /* Printout the MTU size specific to Sidewalk connections */
...
#if SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG
    /* Store active Sidewalk advertisement and connection parameters to RAM context as these can be modified dynamically by the application */
    sid_ble_sidewalk_ctx.current_adv_param.type          = AMA_SERVICE;
    sid_ble_sidewalk_ctx.current_adv_param.fast_enabled  = sid_ble_sidewalk_ctx.ble_cfg->adv_param.fast_enabled;
    sid_ble_sidewalk_ctx.current_adv_param.slow_enabled  = sid_ble_sidewalk_ctx.ble_cfg->adv_param.slow_enabled;
    sid_ble_sidewalk_ctx.current_adv_param.fast_interval = sid_ble_sidewalk_ctx.ble_cfg->adv_param.fast_interval;
    sid_ble_sidewalk_ctx.current_adv_param.fast_timeout  = sid_ble_sidewalk_ctx.ble_cfg->adv_param.fast_timeout;
    sid_ble_sidewalk_ctx.current_adv_param.slow_interval = sid_ble_sidewalk_ctx.ble_cfg->adv_param.slow_interval;
    sid_ble_sidewalk_ctx.current_adv_param.slow_timeout  = sid_ble_sidewalk_ctx.ble_cfg->adv_param.slow_timeout;

    sid_ble_sidewalk_ctx.current_conn_param = sid_ble_sidewalk_ctx.ble_cfg->conn_param;
#endif /* SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG */
...
SID_STM32_SPEED_OPTIMIZED static inline sid_error_t _ble_adapter_sidewalk_set_adv_params(const uint32_t adv_interval)
{
    ...
    do
    {
        ...
        ble_status = hci_le_set_extended_advertising_parameters(
                        ...
                        SID_STM32_BLE_ADV_TX_POWER_NO_PREFERENCE,               /* Tx power preference - request the same Tx power as for the Sidewalk connection */
...
#if SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG
        /* Sidewalk advertisement config is dynamically adjustable by the application - use current (most recent) values */
        sid_pal_enter_critical_region(); /* Run in a critical region to avoid unexpected changes of sid_ble_sidewalk_ctx.current_adv_param */
        if (sid_ble_sidewalk_ctx.current_adv_param.fast_enabled != FALSE)
        {
            new_adv_state                         = BADVS_ADVERTISEMENT_FAST;
            adv_interval                          = sid_ble_sidewalk_ctx.current_adv_param.fast_interval;
            sid_ble_sidewalk_ctx.adv_set.Duration = sid_ble_sidewalk_ctx.current_adv_param.fast_timeout;
        }
        else if (sid_ble_sidewalk_ctx.current_adv_param.slow_enabled != FALSE)
        {
            new_adv_state                         = BADVS_ADVERTISEMENT_SLOW;
            adv_interval                          = sid_ble_sidewalk_ctx.current_adv_param.slow_interval;
            sid_ble_sidewalk_ctx.adv_set.Duration = sid_ble_sidewalk_ctx.current_adv_param.slow_timeout;
        }
#else
        /* Sidewalk advertisement config is static - use the configuration stored on the flash */
...
SID_STM32_SPEED_OPTIMIZED static sid_error_t _ble_adapter_sidewalk_stop_advertisement(void)
{
    ...
    do
    {
        ...
        if ((SID_STM32_UTIL_IS_IRQ() == FALSE)
          ...
          && (osThreadGetId() != sid_ble_drv_ctx.advertising_cmd_task)
#endif /* SID_STM32_BLE_EXTENDED_ADV_SUPPORTED */
        )
        {
...
SID_STM32_SPEED_OPTIMIZED static sid_error_t _ble_adapter_sidewalk_get_rssi(int8_t * rssi)
{
    ...
    do
    {
        ...
        /* Process raw value */
        *rssi = raw_rssi; /* If raw RSSI is valid it's already in dBm units and adjusted by the Link Layer for Rx path gain/loss - can be returned as is */
        err = SID_ERROR_NONE;
    } while (0);
...
SID_STM32_SPEED_OPTIMIZED static sid_error_t _ble_adapter_sidewalk_get_tx_pwr(int16_t * tx_power)
{
    sid_error_t err = SID_ERROR_GENERIC;
    tBleStatus ble_status;
    int8_t current_pwr;
    ...
    do
    {
        ...
        ble_status = hci_read_transmit_power_level(conn_handle, SID_BLE_HCI_TX_POWER_LEVEL_TYPE_CURRENT, (uint8_t *)&current_pwr);
        if (ble_status != BLE_STATUS_SUCCESS)
        {
            SID_PAL_LOG_ERROR("Failed to read current Sidewalk Tx power, result: 0x%02X", ble_status);
            err = SID_ERROR_IO_ERROR;
            break;
        }
        SID_BLE_SIDEWALK_LOG_DEBUG("Current Sidewalk BLE Tx power: %s%d", current_pwr < 0 ? "" : "+", current_pwr); /* %+d format may not be supported by tiny_vsnprintf() */

        /* The Tx power reported by the controller is already adjusted for Tx path gain/loss and can be provided as is */
...
SID_STM32_SPEED_OPTIMIZED static sid_error_t _ble_adapter_sidewalk_set_tx_pwr(int16_t target_tx_power)
{
    sid_error_t err = SID_ERROR_GENERIC;
    int8_t      actual_tx_power;

    static int16_t previous_tx_power = INT16_MIN; /* Use static storage avoid dummy updates when the supplied target_tx_power is continuously over the hardware limits */

    BLE_WRAP_PREPROC();

    do
    {
        /* Ensure Sidewalk BLE is initialized */
        if (FALSE == sid_ble_sidewalk_ctx.init_done)
        {
            err = SID_ERROR_UNINITIALIZED;
            break;
        }

        /* Ensure driver config is available */
        if (NULL == sid_ble_sidewalk_ctx.ble_cfg)
        {
            err = SID_ERROR_INVALID_STATE;
            break;
        }

        /* Skip updates if there's no change in the desired Tx power level */
        if (target_tx_power == previous_tx_power)
        {
            err = SID_ERROR_NONE;
            break;
        }

        /* Check the requested power level does not exceed the maximum for Sidewalk */
        if (target_tx_power > sid_ble_sidewalk_ctx.ble_cfg->max_tx_power_in_dbm)
        {
            actual_tx_power = sid_ble_sidewalk_ctx.ble_cfg->max_tx_power_in_dbm;
        }
        else
        {
            actual_tx_power = (int8_t)target_tx_power;
        }

        err = sid_stm32wba_ble_adapter_prv_generic_set_tx_power(actual_tx_power, &actual_tx_power);
        if (err != SID_ERROR_NONE)
        {
        ...
        if (target_tx_power != actual_tx_power)
        {
            SID_PAL_LOG_WARNING("Global BLE Tx power limit trimmed to %s%ddBm from %s%ddBm due to the HW limits",
                                actual_tx_power < 0 ? "" : "+", actual_tx_power,    /* %+d format may not be supported by tiny_vsnprintf() */
                                target_tx_power < 0 ? "" : "+", target_tx_power); /* %+d format may not be supported by tiny_vsnprintf() */
        }

        /* Store new Tx power level */
        previous_tx_power = target_tx_power;

        /* Done */
        SID_PAL_LOG_INFO("Global BLE Tx power set to %s%ddB", actual_tx_power < 0 ? "" : "+", actual_tx_power); /* tiny_vsnprintf() may not support %+d */
        err = SID_ERROR_NONE;
    } while (0);
...
SID_STM32_SPEED_OPTIMIZED static sid_error_t _ble_adapter_sidewalk_deinit(void)
{
    sid_error_t err;
...
SID_STM32_SPEED_OPTIMIZED static sid_error_t _ble_adapter_sidewalk_get_mac_addr(uint8_t * addr)
{
    sid_error_t err;
    sid_pal_ble_prv_bt_addr_buffer_t ia_addr;
    ...
    do
    {
...
#if SID_STM32_BLE_USE_RANDOM_IDENTITY_ADDRESS
        /* Static random address is used as BLE Identity Address */
        err = sid_stm32wba_ble_adapter_util_get_host_static_random_addr(&ia_addr);
#else
        /* Public address is used as BLE Identity Address */
        err = sid_stm32wba_ble_adapter_util_get_host_public_addr(&ia_addr);
#endif /* SID_STM32_BLE_USE_RANDOM_IDENTITY_ADDRESS */
        if (err != SID_ERROR_NONE)
        {
        ...
        SID_STM32_UTIL_fast_memcpy(addr, ia_addr.bytes, sizeof(ia_addr.bytes));
        err = SID_ERROR_NONE;
    } while (0);
...
#if SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG
SID_STM32_SPEED_OPTIMIZED static inline sid_error_t _ble_adapter_sidewalk_handle_adv_user_config(sid_ble_user_config_t * cfg)
{
    sid_error_t err;
    ...
    if (false == cfg->is_set)
    {
        ...
        sid_pal_enter_critical_region();
        cfg->adv_param = sid_ble_sidewalk_ctx.current_adv_param;
        sid_pal_exit_critical_region();
        err = SID_ERROR_NONE;
    }
    else
    {
        do
        {
            /* Use critical section to avoid race conditions on advertising parameters storage and advertisement state */
            sid_pal_enter_critical_region();

            /* Check if anything has changed */
            if (SID_STM32_UTIL_fast_memcmp(&cfg->adv_param, &sid_ble_sidewalk_ctx.current_adv_param, sizeof(sid_ble_sidewalk_ctx.current_adv_param)) == 0u)
            {
                /* No updates to the advertisement configuration, terminate from here */
                err = SID_ERROR_NONE;
                sid_pal_exit_critical_region();
                break;
            }

            /* Capture current advertisement state */
            const sid_pal_ble_prv_adv_state_t current_advertising_state = sid_ble_sidewalk_ctx.advertising_state;

            /* If Sidewalk advertisement is active we have to stop first because advertising parameters cannot be modified when advertisement is running */
            if (current_advertising_state != BADVS_ADVERTISEMENT_OFF)
            {
                /* Stop advertisement directly from the critical section */
                err = _ble_adapter_sidewalk_stop_advertisement();
                if (err != SID_ERROR_NONE)
                {
                    sid_pal_exit_critical_region();
                    break;
            ...
            sid_ble_sidewalk_ctx.current_adv_param = cfg->adv_param;

            /* Finished with the sensitive actions */
            sid_pal_exit_critical_region();

            /* Restart Sidewalk advertisement with the new settings if it was running on entry */
            if (current_advertising_state != BADVS_ADVERTISEMENT_OFF)
            {
                err = _ble_adapter_sidewalk_start_advertisement();
                if (err != SID_ERROR_NONE)
                {
                    break;
                }
            ...
            /* Done */
            SID_PAL_LOG_INFO("Sidewalk BLE adv params updated, fast enabled: %u, interval: %ums, timeout: %ums; slow enabled: %u, interval: %ums, timeout: %ums",
                             false == cfg->adv_param.fast_enabled ? 0u : 1u,
                             (uint32_t)SID_STM32_BLE_ADV_INTERVAL_UNITS_TO_MS(cfg->adv_param.fast_interval),
                             (uint32_t)SID_STM32_BLE_ADV_DURATION_UNITS_TO_MS(cfg->adv_param.fast_timeout),
                             false == cfg->adv_param.slow_enabled ? 0u : 1u,
                             (uint32_t)SID_STM32_BLE_ADV_INTERVAL_UNITS_TO_MS(cfg->adv_param.slow_interval),
                             (uint32_t)SID_STM32_BLE_ADV_DURATION_UNITS_TO_MS(cfg->adv_param.slow_timeout)
            );
            err = SID_ERROR_NONE;
        } while (0);
    }
...
#if SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG
SID_STM32_SPEED_OPTIMIZED static inline sid_error_t _ble_adapter_sidewalk_handle_conn_user_config(sid_ble_user_config_t * cfg)
{
    sid_error_t err;
    tBleStatus  ble_status;
    ...
    if (false == cfg->is_set)
    {
        /* Get current user config */
        sid_pal_enter_critical_region();
        cfg->conn_param = sid_ble_sidewalk_ctx.current_conn_param;
        sid_pal_exit_critical_region();
        err = SID_ERROR_NONE;
    }
    else
    {
        ...
        do
        {
            sid_ble_ext_proposed_conn_params_t proposed_params = {
                ...
                .latency_max  = cfg->conn_param.slave_latency,
            ...
            if ((err != SID_ERROR_NONE) || (ble_status != BLE_STATUS_SUCCESS))
            {
                SID_PAL_LOG_ERROR("Unacceptable Sidewalk BLE connection user parameters provided - declined");
                break;
            }
             ...
             || (proposed_params.interval_max != accepted_params.interval_max) || (proposed_params.latency_max != accepted_params.latency_max))
            {
                ...
                SID_PAL_LOG_WARNING("Out-of-spec user-defined Sidewalk BLE connection parameters requested - adjustments will apply");
            }

            /* Request parameter update for the Sidewalk connection if it established */
            if (sid_ble_sidewalk_ctx.conn_ctx != NULL)
            {
            ...
            /* Store new user-defined connection settings */
            sid_ble_sidewalk_ctx.current_conn_param.min_conn_interval = accepted_params.interval_min;
            sid_ble_sidewalk_ctx.current_conn_param.max_conn_interval = accepted_params.interval_max;
            sid_ble_sidewalk_ctx.current_conn_param.slave_latency     = accepted_params.latency_max;
            sid_ble_sidewalk_ctx.current_conn_param.conn_sup_timeout  = accepted_params.timeout;
...
#if SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG
SID_STM32_SPEED_OPTIMIZED static sid_error_t _ble_adapter_sidewalk_handle_user_config(sid_ble_user_config_t * cfg)
{
    sid_error_t err;
    ...
    do
    {
        ...
        /* Validate Sidewalk driver state */
        if (FALSE == sid_ble_sidewalk_ctx.init_done)
        {
            err = SID_ERROR_UNINITIALIZED;
            break;
        ...
        switch (cfg->cfg_type)
        {
            case SID_BLE_USER_CFG_ADV:
                /* Process Sidewalk advertisement parameters only */
                err = _ble_adapter_sidewalk_handle_adv_user_config(cfg);
                break;

            case SID_BLE_USER_CFG_CONN:
                /* Process Sidewalk connection parameters only */
                err = _ble_adapter_sidewalk_handle_conn_user_config(cfg);
                break;

            case SID_BLE_USER_CFG_ADV_AND_CONN:
                /* Process both Sidewalk advertisement and connection parameters */
                err = _ble_adapter_sidewalk_handle_adv_user_config(cfg);
                if (err != SID_ERROR_NONE)
                {
                    /* Failed to process advertisement parameters, terminate from here */
                    break;
                }
                err = _ble_adapter_sidewalk_handle_conn_user_config(cfg);
                break;

            case SID_BLE_USER_CFG_INACTIVITY_TIMEOUT: /* Inactivity timeout is handled by the Sidewalk stack itself and normally shall never appear here */
            default:
...
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_stm32wba_ble_adapter_sidewalk_on_adv_timeout(void)
{
    ...
    do
    {
...
#if SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG
        /* Sidewalk advertisement config is dynamically adjustable by the application - use current (most recent) values */
        if ((BADVS_ADVERTISEMENT_FAST == sid_ble_sidewalk_ctx.advertising_state) && (sid_ble_sidewalk_ctx.current_adv_param.slow_enabled != FALSE))
#else
        /* Sidewalk advertisement config is static - use the configuration stored on the flash */
...
#endif /* SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG */
        {
            ...
            sid_ble_sidewalk_ctx.adv_set.Duration =
#if SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG
                sid_ble_sidewalk_ctx.current_adv_param.slow_timeout;
#else
                sid_ble_sidewalk_ctx.ble_cfg->adv_param.slow_timeout;
#endif /* SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG */

            /* Apply advertisement parameters for slow advertisement */
            err = _ble_adapter_sidewalk_set_adv_params(
#if SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG
                sid_ble_sidewalk_ctx.current_adv_param.slow_interval
#else
                sid_ble_sidewalk_ctx.ble_cfg->adv_param.slow_interval
#endif /* SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG */
            );
            if (err != SID_ERROR_NONE)
            {
...
#  if SID_SDK_CONFIG_ENABLE_BLE_USER_CONFIG
            req_conn_interval_min = sid_ble_sidewalk_ctx.current_conn_param.min_conn_interval;
            req_conn_interval_max = sid_ble_sidewalk_ctx.current_conn_param.max_conn_interval;
            req_conn_latency      = sid_ble_sidewalk_ctx.current_conn_param.slave_latency;
            req_conn_timeout      = sid_ble_sidewalk_ctx.current_conn_param.conn_sup_timeout;
#  else
        ...
        else
        {
            ...
            {
                /* We are fine with the updated params */
                conn_params_ok = TRUE;
            }
        }
