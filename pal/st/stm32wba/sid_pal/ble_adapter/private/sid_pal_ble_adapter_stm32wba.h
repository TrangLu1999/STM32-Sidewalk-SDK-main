 ...
 *
 * This method performs the necessary actions to start BLE advertisement regardless if extended advertising is used or not
 *
 * @warning This is a private API and it shall not be used by the user app directly under any circumstances. Failing to comply will result in undefined behavior
 *
 ...
 *
 * This method performs the necessary actions to stop BLE advertisement regardless if extended advertising is used or not
 *
 * @warning This is a private API and it shall not be used by the user app directly under any circumstances. Failing to comply will result in undefined behavior
 *
...
/**
 * @brief Set BLE Tx power (EIRP) to the specified value
 *
 * This method performs similar operation to the aci_hal_set_tx_power_level() with the difference that it accepts the desired
 * EIRP power level in dBm instead of the internal PA index and it automatically selects the PA power table to use for the
 * desired Tx power level. Using the desired EIRP as an input means that you don't have to think about antenna gain, Tx
 * path loss or external PA gain - all these factors are addressed by the Link Layer internally.
 *
 * @warning This is a private API and it shall not be used by the user app directly under any circumstances. Failing to comply will result in undefined behavior
 *
 * @param [in]  desired_tx_power_dbm Desired Tx power (EIRP) in dBm. Since this is a EIRP value, it shall not be adjusted by antenna gain, Tx path gain/loss, etc. All these adjustments are handled by the driver internally
 * @param [out] out_actual_tx_power Actually applied EIRP level, which can differ from the requested one (e.g., too high or too low )
 * @return SID_ERROR_NONE on success, error code otherwise
 */
sid_error_t sid_stm32wba_ble_adapter_prv_generic_set_tx_power(const int8_t desired_tx_power_dbm, int8_t * const out_actual_tx_power);
