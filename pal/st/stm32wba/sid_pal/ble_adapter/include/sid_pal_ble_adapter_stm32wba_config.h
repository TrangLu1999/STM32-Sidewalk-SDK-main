...
#ifndef SID_STM32_BLE_CFG_USE_EXTERNAL_PA
#  define SID_STM32_BLE_CFG_USE_EXTERNAL_PA            (1u)    /*!< Set this to 1 if your setup has an external power amplifier for the 2.4GHz radio */
#endif /* SID_STM32_BLE_CFG_USE_EXTERNAL_PA */

#ifndef SID_STM32_BLE_CFG_USE_EXTERNAL_LNA
#  define SID_STM32_BLE_CFG_USE_EXTERNAL_LNA           (1u)    /*!< Set this to 1 if your setup has an external low noise amplifier for the 2.4GHz radio */
#endif /* SID_STM32_BLE_CFG_USE_EXTERNAL_LNA */

#define SID_STM32_BLE_DBM_TO_INT_MULT                  (100.f) /*!< Multiplier to convert physical dB units into internal integer representation */

#define SID_STM32_BLE_TX_POWER_NO_PREFERENCE           (127)   /*!< Special value to indicate there's no preference in power level selection. Any value different from this one will be treaded as an actual power level expressed in dBm */
#ifndef SID_STM32_BLE_DEFAULT_TX_POWER_DBM
#  define SID_STM32_BLE_DEFAULT_TX_POWER_DBM           (0)     /*!< Default Tx power level (in dBm) to be used if none of the BLE driver users explicitly define the desired power level */
#endif /* SID_STM32_BLE_DEFAULT_TX_POWER_DBM */

/* Exported macro ------------------------------------------------------------*/

#define SID_STM32_BLE_POWER_DBM_TO_FIXED(__DBM__)        ((int16_t)((float)(__DBM__) * (SID_STM32_BLE_DBM_TO_INT_MULT))) /*!< Converts floating point dBm value to an internal fixed-point representation by multiplying by 100 (e.g., 2.15dBm = 215) */
#define SID_STM32_BLE_POWER_INT_TO_DBM(__INT__)        ((float)(__INT__) / (SID_STM32_BLE_DBM_TO_INT_MULT))              /*!< Convert internal integer representation of dBm into physical dBm value (floating point) */
#define SID_STM32_BLE_POWER_INT_TO_DBM_INT(__INT__)    ((int8_t)((__INT__) > 0 ?                                         /*!< Convert internal integer representation of dBm into physical integer value with rounding */ \
                                                       (((__INT__) + ((int32_t)(SID_STM32_BLE_DBM_TO_INT_MULT) / 2)) / (int32_t)(SID_STM32_BLE_DBM_TO_INT_MULT)) : \
                                                       (((__INT__) - ((int32_t)(SID_STM32_BLE_DBM_TO_INT_MULT) / 2)) / (int32_t)(SID_STM32_BLE_DBM_TO_INT_MULT))))
