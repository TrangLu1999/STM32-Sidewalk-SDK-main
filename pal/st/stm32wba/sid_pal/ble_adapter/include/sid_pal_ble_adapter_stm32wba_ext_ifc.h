...
#define SID_STM32_BLE_RADIO_MAX_TX_POWER_EU                (10)                                            /*!< Maximum allowed BLE Tx power to meet ETSI regulations. While ETSI EN 300 328 generally permits up to +20dBm EIRP, there are a lot of additional limitations when going beyond +10dBm */
...
/**
 * @brief Definition of the Alexa Mobile Accessory (AMA) BLE GATT service
 */
extern const sid_ble_cfg_service_t sid_ble_ama_service;

/**
 * @brief Definition of the characteristics of the AMA GATT service
 */
extern const sid_ble_cfg_characteristics_t sid_ble_ama_svc_chars[];

/**
 * @brief Default GATT profile for the Sidewalk-over-BLE link. The array contains list of GATT services associated with the profile
 *
 * @attention Normally you should use this default profile as is. Do not modify or override it unless you absolutely know what you
 *            are doing and the required alterations are unavoidable for your applications
 */
extern const sid_ble_cfg_gatt_profile_t sid_ble_default_profile[];
...
typedef struct {
    struct {
        int8_t                               max_tx_power_dbm;     /*!< Maximum EIRP in dBm. This value should indicate the desired radiated power and thus shall not include any adjustments for antenna gain, RF path loss, etc. */
        int16_t                              ant_gain;             /*!< Antenna gain expressed in cdBi (dBi * 100) */
        int16_t                              tx_insertion_loss;    /*!< Tx path insertion loss (e.g., RF switch, matching network, etc.) expressed in cdB (dB * 100) */
#if SID_STM32_BLE_CFG_USE_EXTERNAL_PA /*!< This part is relevant only when an external RF Power Amplifier (PA) is mounted on PCB */
        int16_t                              ext_pa_gain;          /*!< Tx gain of the external PA, expressed in cdB (dB * 100) */
#endif /* SID_STM32_BLE_CFG_USE_EXTERNAL_PA */
        int16_t                              rx_insertion_loss;    /*!< Rx path insertion loss expressed in cdB (dB * 100) */
        ...
        int16_t                              lna_gain;             /*!< Rx gain of the external LNA, expressed in cdB (dB * 100) */
...
typedef struct {
    /**
     * @brief Initialize the BLE stack to run user-defined GATT profiles
     *
    ...
    /**
     * @brief De-initialize the user-defined portion of BLE stack
     *
     * @note In SID_STM32_BLE_COEXISTENCE_MODE_CONCURRENCY this method can be used independently of Sidewalk state,
    ...
    /**
     * @brief Do a full factory reset of the BLE
     *
     ...
     *
     * @note This affects both Sidewalk and user-mode BLE since internal PA gain is controlled by the VDDRF supply voltage and applies to all connections uniformly
     *
     * @param [in] tx_power_dbm Desired Tx power level in dBm. This parameter specifies EIRP level, antenna gain and external PA gain (if used) are adjusted for by the Link Layer automatically
     *
     * @return SID_ERROR_NONE on success, error code otherwise
     */
    sid_error_t (*set_tx_power)(const int8_t tx_power_dbm);
