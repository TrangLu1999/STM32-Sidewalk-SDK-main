...
static        sid_error_t                          _ble_adapter_user_set_tx_power(const int8_t tx_power_dbm);
...
static const sid_pal_ble_adapter_extended_interface_t user_ble_ifc = {
    ...
    .set_tx_power                   = _ble_adapter_user_set_tx_power,
...
SID_STM32_SPEED_OPTIMIZED static sid_error_t _ble_adapter_user_set_tx_power(const int8_t tx_power_dbm)
{
    sid_error_t err = SID_ERROR_GENERIC;
    ...
    do
    {
        /* Ensure user BLE is initialized */
        if (sid_ble_user_mode_ctx.state != SPBUM_STATE_INITIALIZED)
        {
...
#if (SID_STM32_BLE_COEXISTENCE_MODE == SID_STM32_BLE_COEXISTENCE_MODE_CONCURRENCY)
        /* Respect Sidewalk limits when operating in concurrency mode and Sidewalk is initialized */
        if ((sid_ble_sidewalk_ctx.init_done != FALSE) && (sid_ble_sidewalk_ctx.ble_cfg != NULL))
        {
            /* Check the requested power level does not exceed the maximum for Sidewalk */
            if (tx_power_dbm > sid_ble_sidewalk_ctx.ble_cfg->max_tx_power_in_dbm)
            {
                actual_tx_power = sid_ble_sidewalk_ctx.ble_cfg->max_tx_power_in_dbm;
            }
            else
            {
                actual_tx_power = tx_power_dbm;
            }
        }
#endif /* SID_STM32_BLE_COEXISTENCE_MODE_CONCURRENCY */
        ...
        if (tx_power_dbm != actual_tx_power)
        {
            SID_PAL_LOG_WARNING("Global BLE Tx power limit trimmed to %s%ddBm from %s%ddBm due to the HW limits",
                                ...
                                tx_power_dbm < 0 ? "" : "+", tx_power_dbm); /* %+d format may not be supported by tiny_vsnprintf() */
        }
...
SID_STM32_SPEED_OPTIMIZED static sid_error_t _ble_adapter_user_virt_dev_update_char(const sid_ble_ext_virtual_device_id_t virt_dev_id, const sid_ble_cfg_uuid_info_t * const char_uuid, const uint8_t * const data, const uint32_t data_length, uint8_t notify)
{
    ...
    do
    {
        ...
        else
        {
            ...
            SID_BLE_USER_LOG_DEBUG("==>> aci_gatt_update_char_value_ext - sending out   long data (%u bytes)", data_length);
            ...
            while (remaining_data_size > SID_STM32_BLE_ACI_CHARACTERISTIC_VALUE_UPDATE_SIZE_LIMIT)
            {
                ...
                {
                    SID_BLE_USER_LOG_ERROR("==>> aci_gatt_update_char_value_ext - fail on putting partial data, start offset: %u, total length: %u, result: 0x%02X", current_offset, data_length, ble_ret);
                    break;
                }
            }
        ...
        if (BLE_STATUS_SUCCESS == ble_ret)
        {
            SID_BLE_USER_LOG_DEBUG("==>> aci_gatt_update_char_value_ext - success, %u bytes written", data_length);
            err = SID_ERROR_NONE;
        }
...
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_stm32wba_ble_adapter_user_on_ble_connected_to_central(const sid_ble_ext_virtual_device_id_t virt_dev_id, sid_pal_ble_prv_connection_ctx_t * const conn_ctx)
{
    ...
    do
    {
        ...
        /* Enable data length extensions (DLE) for user BLE link */
