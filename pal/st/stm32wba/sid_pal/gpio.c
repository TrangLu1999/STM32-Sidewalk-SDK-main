...
#ifndef SID_PAL_GPIO_STM32_RESERVE_RTC_WAKEUP_LINE
#  define SID_PAL_GPIO_STM32_RESERVE_RTC_WAKEUP_LINE (CFG_LPM_STDBY_SUPPORTED != 0u) /*!< True: Reserve the PWR Wakeup line that is shared with RTC and do not allow GPIO pins to use this line as a wakeup source; False - GPIO pins can use the shared Wakeup line */
#endif /* SID_PAL_GPIO_STM32_RESERVE_RTC_WAKEUP_LINE */
...
#  define SID_PAL_GPIO_STM32_LOG_ERROR(...)          SID_PAL_LOG_ERROR(__VA_ARGS__)
#  define SID_PAL_GPIO_STM32_LOG_WARNING(...)        SID_PAL_LOG_WARNING(__VA_ARGS__)
#  define SID_PAL_GPIO_STM32_LOG_INFO(...)           SID_PAL_LOG_INFO(__VA_ARGS__)
#  define SID_PAL_GPIO_STM32_LOG_DEBUG(...)          SID_PAL_LOG_DEBUG(__VA_ARGS__)
#  define SID_PAL_GPIO_STM32_LOG_TRACE(...)          SID_PAL_LOG_TRACE(__VA_ARGS__)
#else
#  define SID_PAL_GPIO_STM32_LOG_ERROR(...)          ((void)0u)
#  define SID_PAL_GPIO_STM32_LOG_WARNING(...)        ((void)0u)
#  define SID_PAL_GPIO_STM32_LOG_INFO(...)           ((void)0u)
#  define SID_PAL_GPIO_STM32_LOG_DEBUG(...)          ((void)0u)
#  define SID_PAL_GPIO_STM32_LOG_TRACE(...)          ((void)0u)
#endif
...
#  define GET_LL_EXTI_CONFIG_PIN(pin_num)            ( ( ( (GET_GPIO_PIN_POSITION(pin_num) & 0x3u) << 3) << LL_EXTI_REGISTER_PINPOS_SHFT) | (GET_GPIO_PIN_POSITION(pin_num) >> 2) )

/* RTC wakeup line definition */
#  if defined (__ARM_FEATURE_CMSE) && (__ARM_FEATURE_CMSE == 3U)
#    define RTC_PWR_WAKEUP_FLAG                       (PWR_WAKEUP_FLAG6) /*!< RTC: Wakeup pin 6 (high polarity) */
#  else
#    define RTC_PWR_WAKEUP_FLAG                       (PWR_WAKEUP_FLAG7) /*!< RTC: Wakeup pin 7 (high polarity) */
#endif /* (__ARM_FEATURE_CMSE) && (__ARM_FEATURE_CMSE == 3U) */

/* GPIO wakeup line definitions */
#define GPIO_WAKEUP_LINES                            (8u)
...
#  endif /* STM32WBAxx - GPIO port definitions */
#else
#  error "This implementation applies to the STM32WBAxx MCU family only"
#endif /* STM32WBA */
...
static gpio_irq_callback_cfg_t sid_pal_gpio_irq_callbacks[GPIO_PINS_PER_PORT] = {0};
#if (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED != 0)
static gpio_irq_callback_cfg_t sid_pal_gpio_wakeup_callbacks[GPIO_WAKEUP_LINES] = {0};
...
static inline uint32_t    get_wakeup_source_index_from_flag(const uint32_t wakeup_flag);
static inline uint32_t    get_wakeup_source_index_from_pin(const uint32_t wakeup_pin);
#endif /* (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED != 0) */
...
SID_STM32_SPEED_OPTIMIZED static inline void register_gpio_irq_callback(const uint32_t gpio_number, const sid_pal_gpio_irq_trigger_t irq_trigger, const sid_pal_gpio_irq_handler_t callback, void * const callback_arg)
{
    ...
    SID_PAL_ASSERT(pin_index < SID_STM32_UTIL_ARRAY_SIZE(sid_pal_gpio_irq_callbacks));
    ...
    sid_pal_gpio_irq_callbacks[pin_index].callback     = callback;
    sid_pal_gpio_irq_callbacks[pin_index].callback_arg = callback_arg;
    sid_pal_gpio_irq_callbacks[pin_index].gpio_number  = gpio_number;
    sid_pal_gpio_irq_callbacks[pin_index].trigger      = irq_trigger;
...
SID_STM32_SPEED_OPTIMIZED static inline void deregister_gpio_irq_callback(const uint32_t gpio_number)
{
    ...
    SID_STM32_UTIL_fast_memset(&sid_pal_gpio_irq_callbacks[pin_index], 0x00u, sizeof(sid_pal_gpio_irq_callbacks[pin_index]));
...
#if (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED != 0)
SID_STM32_SPEED_OPTIMIZED static inline uint32_t get_wakeup_source_index_from_flag(const uint32_t wakeup_flag)
{
    uint32_t wakeup_source_index;

    switch (wakeup_flag)
    {
#  ifdef PWR_WAKEUP_FLAG1
        case PWR_WAKEUP_FLAG1:
            wakeup_source_index = 0u;
            break;
#  endif /* PWR_WAKEUP_FLAG1 */

#  ifdef PWR_WAKEUP_FLAG2
        case PWR_WAKEUP_FLAG2:
            wakeup_source_index = 1u;
            break;
#  endif /* PWR_WAKEUP_FLAG2 */

#  ifdef PWR_WAKEUP_FLAG3
        case PWR_WAKEUP_FLAG3:
            wakeup_source_index = 2u;
            break;
#  endif /* PWR_WAKEUP_FLAG3 */

#  ifdef PWR_WAKEUP_FLAG4
        case PWR_WAKEUP_FLAG4:
            wakeup_source_index = 3u;
            break;
#  endif /* PWR_WAKEUP_FLAG4 */

#  ifdef PWR_WAKEUP_FLAG5
        case PWR_WAKEUP_FLAG5:
            wakeup_source_index = 4u;
            break;
#  endif /* PWR_WAKEUP_FLAG5 */

#  ifdef PWR_WAKEUP_FLAG6
        case PWR_WAKEUP_FLAG6:
            wakeup_source_index = 5u;
            break;
#  endif /* PWR_WAKEUP_FLAG6 */

#  ifdef PWR_WAKEUP_FLAG7
        case PWR_WAKEUP_FLAG7:
            wakeup_source_index = 6u;
            break;
#  endif /* PWR_WAKEUP_FLAG7 */

#  ifdef PWR_WAKEUP_FLAG8
        case PWR_WAKEUP_FLAG8:
            wakeup_source_index = 7u;
            break;
#  endif /* PWR_WAKEUP_FLAG8 */

        default:
        wakeup_source_index = UINT32_MAX;
        break;
    }

    SID_PAL_ASSERT(wakeup_source_index != UINT32_MAX);

    return wakeup_source_index;
}
#endif /* (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED != 0) */
/*----------------------------------------------------------------------------*/

#if (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED != 0)
SID_STM32_SPEED_OPTIMIZED static inline uint32_t get_wakeup_source_index_from_pin(const uint32_t wakeup_pin)
{
    uint32_t wakeup_source_index;
...
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_pal_gpio_set_direction(uint32_t gpio_number, sid_pal_gpio_direction_t direction)
{
    ...
    {
        SID_PAL_GPIO_STM32_LOG_DEBUG("Set P%s%d direction to Input", NUM_TO_GPIO_PORT_STR(gpio_number), GET_GPIO_PIN_POSITION(NUM_TO_GPIO_PIN(gpio_number)));
        pin_mode = LL_GPIO_MODE_INPUT;
    }
    else
    {
        SID_PAL_GPIO_STM32_LOG_DEBUG("Set P%s%d direction to Output", NUM_TO_GPIO_PORT_STR(gpio_number), GET_GPIO_PIN_POSITION(NUM_TO_GPIO_PIN(gpio_number)));
...
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_pal_gpio_input_mode(uint32_t gpio_number, sid_pal_gpio_input_t mode)
{
    ...
    {
        SID_PAL_GPIO_STM32_LOG_DEBUG("Set P%s%d input mode to Input", NUM_TO_GPIO_PORT_STR(gpio_number), GET_GPIO_PIN_POSITION(NUM_TO_GPIO_PIN(gpio_number)));
        pin_mode = LL_GPIO_MODE_INPUT;
    }
    else
    {
        SID_PAL_GPIO_STM32_LOG_DEBUG("Set P%s%d input mode to Analog", NUM_TO_GPIO_PORT_STR(gpio_number), GET_GPIO_PIN_POSITION(NUM_TO_GPIO_PIN(gpio_number)));
...
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_pal_gpio_output_mode(uint32_t gpio_number, sid_pal_gpio_output_t mode)
{
    ...
    if (mode == SID_PAL_GPIO_OUTPUT_OPEN_DRAIN)
    {
        SID_PAL_GPIO_STM32_LOG_DEBUG("Set P%s%d output mode to Open Drain", NUM_TO_GPIO_PORT_STR(gpio_number), GET_GPIO_PIN_POSITION(NUM_TO_GPIO_PIN(gpio_number)));
        output_type = LL_GPIO_OUTPUT_OPENDRAIN;
    }
    else
    {
        SID_PAL_GPIO_STM32_LOG_DEBUG("Set P%s%d output mode to Push-Pull", NUM_TO_GPIO_PORT_STR(gpio_number), GET_GPIO_PIN_POSITION(NUM_TO_GPIO_PIN(gpio_number)));
...
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_pal_gpio_pull_mode(uint32_t gpio_number, sid_pal_gpio_pull_t pull)
{
    ...
    switch (pull)
    {
        case SID_PAL_GPIO_PULL_NONE:
            SID_PAL_GPIO_STM32_LOG_DEBUG("Disable pull-up & pull-down on P%s%d", NUM_TO_GPIO_PORT_STR(gpio_number), GET_GPIO_PIN_POSITION(NUM_TO_GPIO_PIN(gpio_number)));
            pin_pull = LL_GPIO_PULL_NO;
            break;

        case SID_PAL_GPIO_PULL_UP:
            SID_PAL_GPIO_STM32_LOG_DEBUG("Enable pull-up on P%s%d", NUM_TO_GPIO_PORT_STR(gpio_number), GET_GPIO_PIN_POSITION(NUM_TO_GPIO_PIN(gpio_number)));
            pin_pull = LL_GPIO_PULL_UP;
            break;

        case SID_PAL_GPIO_PULL_DOWN:
            SID_PAL_GPIO_STM32_LOG_DEBUG("Enable pull-down on P%s%d", NUM_TO_GPIO_PORT_STR(gpio_number), GET_GPIO_PIN_POSITION(NUM_TO_GPIO_PIN(gpio_number)));
...
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_pal_gpio_ext_ifc_set_latch(const uint32_t gpio_number, const sid_pal_gpio_irq_trigger_t edge)
{
    ...
    switch (edge)
    {
        case SID_PAL_GPIO_IRQ_TRIGGER_NONE:
            SID_PAL_GPIO_STM32_LOG_DEBUG("Configure no EXTI trigger on P%s%d", NUM_TO_GPIO_PORT_STR(gpio_number), GET_GPIO_PIN_POSITION(NUM_TO_GPIO_PIN(gpio_number)));
            exti_trigger = LL_EXTI_TRIGGER_NONE;
            break;

        case SID_PAL_GPIO_IRQ_TRIGGER_RISING:
            SID_PAL_GPIO_STM32_LOG_DEBUG("Configure EXTI rising edge trigger on P%s%d", NUM_TO_GPIO_PORT_STR(gpio_number), GET_GPIO_PIN_POSITION(NUM_TO_GPIO_PIN(gpio_number)));
            exti_trigger = LL_EXTI_TRIGGER_RISING;
            break;

        case SID_PAL_GPIO_IRQ_TRIGGER_FALLING:
            SID_PAL_GPIO_STM32_LOG_DEBUG("Configure EXTI falling edge trigger on P%s%d", NUM_TO_GPIO_PORT_STR(gpio_number), GET_GPIO_PIN_POSITION(NUM_TO_GPIO_PIN(gpio_number)));
            exti_trigger = LL_EXTI_TRIGGER_FALLING;
            break;

        case SID_PAL_GPIO_IRQ_TRIGGER_EDGE:
            SID_PAL_GPIO_STM32_LOG_DEBUG("Configure EXTI rising & falling edges trigger on P%s%d", NUM_TO_GPIO_PORT_STR(gpio_number), GET_GPIO_PIN_POSITION(NUM_TO_GPIO_PIN(gpio_number)));
...
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_pal_gpio_irq_enable(uint32_t gpio_number)
{
    ...
    if (sid_pal_gpio_irq_callbacks[pin_index].gpio_number == gpio_number)
    {
        if (((SID_PAL_GPIO_IRQ_TRIGGER_LOW == sid_pal_gpio_irq_callbacks[pin_index].trigger) && ((st_port->IDR & st_pin) == 0x00u))
        || ((SID_PAL_GPIO_IRQ_TRIGGER_HIGH == sid_pal_gpio_irq_callbacks[pin_index].trigger) && ((st_port->IDR & st_pin) != 0x00u)))
        {
    ...
    SID_PAL_GPIO_STM32_LOG_DEBUG("Enable EXTI IRQ for P%s%d", NUM_TO_GPIO_PORT_STR(gpio_number), GET_GPIO_PIN_POSITION(NUM_TO_GPIO_PIN(gpio_number)));
...
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_pal_gpio_irq_disable(uint32_t gpio_number)
{
    ...
    SID_PAL_GPIO_STM32_LOG_DEBUG("Disable EXTI IRQ for P%s%d", NUM_TO_GPIO_PORT_STR(gpio_number), GET_GPIO_PIN_POSITION(NUM_TO_GPIO_PIN(gpio_number)));
...
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_pal_gpio_ext_ifc_set_irq_priority(const uint32_t gpio_number, const uint32_t irq_preempt_prio, const uint32_t irq_sub_prio)
{
    ...
    SID_PAL_GPIO_STM32_LOG_DEBUG("Set EXTI IRQ priority for P%s%d to %u.%u", NUM_TO_GPIO_PORT_STR(gpio_number), GET_GPIO_PIN_POSITION(NUM_TO_GPIO_PIN(gpio_number)), irq_preempt_prio, irq_sub_prio);
...
SID_STM32_SPEED_OPTIMIZED void sid_pal_gpio_exti_irq_handler(const uint32_t st_pin)
{
    ...
    const uint32_t pin_index = GET_GPIO_PIN_POSITION(st_pin);
    SID_PAL_ASSERT(pin_index < GPIO_PINS_PER_PORT);
    GPIO_TypeDef * const st_port = NUM_TO_GPIO_ST_PORT(sid_pal_gpio_irq_callbacks[pin_index].gpio_number);
    const uint32_t irq_num = GET_GPIO_PIN_IRQN(st_pin);

    /* Proceed with user callback invocation if such callback is assigned */
    if (sid_pal_gpio_irq_callbacks[pin_index].callback != NULL)
    {
        /* Call the user-defined callback */
        sid_pal_gpio_irq_callbacks[pin_index].callback(sid_pal_gpio_irq_callbacks[pin_index].gpio_number, sid_pal_gpio_irq_callbacks[pin_index].callback_arg);
    }

    if (
        ( /* Software emulation of the low level-triggered GPIO IRQ */
               (SID_PAL_GPIO_IRQ_TRIGGER_LOW == sid_pal_gpio_irq_callbacks[pin_index].trigger)
        ...
        ( /* Software emulation of the high level-triggered GPIO IRQ */
               (SID_PAL_GPIO_IRQ_TRIGGER_HIGH == sid_pal_gpio_irq_callbacks[pin_index].trigger)
            && ((st_port->IDR & st_pin) != 0x00u) /* Pin remains high */
            && (NVIC_GetEnableIRQ(irq_num)) /* Associated IRQ in NVIC remains enabled */
        )
    )
    {
        /* Generate an SWI if level-based trigger is configured for the pin and the pin remains at the triggering level even after the user callback invocation */
        LL_EXTI_GenerateSWI_0_31(exti_line);
    }
}
...
#if (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED != 0)
SID_STM32_SPEED_OPTIMIZED void sid_pal_gpio_wakeup_irq_handler(void)
{
    const uint32_t wakeup_pins[] = {
#ifdef PWR_WAKEUP_PIN1
        PWR_WAKEUP_PIN1,
#endif /* PWR_WAKEUP_PIN1 */
#ifdef PWR_WAKEUP_PIN2
    ...
    const uint32_t active_wakeup_sources = HAL_PWR_GetClearWakeupSource();

    for (uint32_t i = 0u; i < SID_STM32_UTIL_ARRAY_SIZE(wakeup_pins); i++)
    {
        const uint32_t wakeup_source = (active_wakeup_sources & wakeup_pins[i]);

        if (wakeup_source != 0u)
        {
            /* Get wakeup source index */
            const uint32_t wake_source_index = get_wakeup_source_index_from_pin(wakeup_source);
            SID_PAL_ASSERT(wake_source_index < SID_STM32_UTIL_ARRAY_SIZE(sid_pal_gpio_wakeup_callbacks));

            /* Proceed with user callback invocation if such callback is assigned */
            if (sid_pal_gpio_wakeup_callbacks[wake_source_index].callback != NULL)
            {
                /* Call the user-defined callback */
                sid_pal_gpio_wakeup_callbacks[wake_source_index].callback(sid_pal_gpio_wakeup_callbacks[wake_source_index].gpio_number, sid_pal_gpio_wakeup_callbacks[wake_source_index].callback_arg);
            }
        }
    }
}
...
#if (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED != 0)
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_pal_gpio_ext_ifc_wakeup_enable(const uint32_t gpio_number, const sid_pal_gpio_irq_trigger_t edge, const sid_pal_gpio_irq_handler_t callback, void * const callback_arg)
{
    sid_error_t err;
    ...
    do
    {
...
#if SID_PAL_GPIO_STM32_RESERVE_RTC_WAKEUP_LINE
        if (RTC_PWR_WAKEUP_FLAG == pwr_wakeup_flag)
        {
            /* The GPIO pin shares the wakeup line with RTC, and RTC wakeup is in use */
            err = SID_ERROR_BUSY;
            break;
        }
#endif /* SID_PAL_GPIO_STM32_RESERVE_RTC_WAKEUP_LINE */
        ...
        /* Set the callback for the selected pin */
        const uint32_t wake_source_index = get_wakeup_source_index_from_flag(pwr_wakeup_flag);
        ...
        sid_pal_gpio_wakeup_callbacks[wake_source_index].callback     = callback;
        sid_pal_gpio_wakeup_callbacks[wake_source_index].callback_arg = callback_arg;
        sid_pal_gpio_wakeup_callbacks[wake_source_index].gpio_number  = gpio_number;
        sid_pal_gpio_wakeup_callbacks[wake_source_index].trigger      = edge;
...
#if (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED != 0)
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_pal_gpio_ext_ifc_wakeup_disable(const uint32_t gpio_number)
{
    sid_error_t err;
    uint32_t pwr_wakeup_pin;
    uint32_t pwr_wakeup_flag;
    ...
    do
    {
        ...
        err = get_pwr_wakeup_pin_flag(gpio_number, SID_PAL_GPIO_STM32WBAxx_WAKEUP_POLARITY_HIGH, &pwr_wakeup_pin, &pwr_wakeup_flag); /* The actual polarity doesn't matter here */
        if (err != SID_ERROR_NONE)
        {
            break;
        }
        ...
        /* Clear the callback for the selected pin */
        const uint32_t wake_source_index = get_wakeup_source_index_from_flag(pwr_wakeup_flag);
        ...
        SID_STM32_UTIL_fast_memset(&sid_pal_gpio_wakeup_callbacks[wake_source_index], 0x00u, sizeof(sid_pal_gpio_wakeup_callbacks[wake_source_index]));

        __set_PRIMASK(primask_bit);

        /* Disable wakeup for the selected pin and trigger edge */
...
#if (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED != 0)
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_pal_gpio_ext_ifc_standby_retention_enable(const uint32_t gpio_number)
{
    sid_error_t err;
    HAL_StatusTypeDef hal_ret;
...
#if (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED != 0)
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_pal_gpio_ext_ifc_standby_retention_disable(const uint32_t gpio_number)
{
    sid_error_t err;
    HAL_StatusTypeDef hal_ret;
    ...
    do
    {
        ...
        /* Disable GPIO retention for the selected wakeup pin */
        hal_ret = HAL_PWREx_DisableStandbyIORetention(pwr_gpio_port, pwr_gpio_pin);
        if (hal_ret != HAL_OK)
        {
            err = SID_ERROR_GENERIC;
            break;
...
#if (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED != 0)
SID_STM32_SPEED_OPTIMIZED sid_error_t sid_pal_gpio_ext_ifc_restore_output_after_standby(const uint32_t gpio_number, const sid_pal_gpio_output_t output_mode, const sid_pal_gpio_pull_t pull)
{
    if(GPIO_PORT_PIN_NOT_CONNECTED == gpio_number)
    {
        return SID_ERROR_NONE;
    ...
    if ((output_mode > SID_PAL_GPIO_OUTPUT_OPEN_DRAIN) || (pull > SID_PAL_GPIO_PULL_DOWN))
    {
        return SID_ERROR_INVALID_ARGS;
    }
    ...
    /* Step 1 - set NSS pin as input - the state is still retained by Standby circuitry */
    SID_PAL_GPIO_STM32_LOG_DEBUG("Set P%s%d input mode to Input", NUM_TO_GPIO_PORT_STR(gpio_number), GET_GPIO_PIN_POSITION(NUM_TO_GPIO_PIN(gpio_number)));
    LL_GPIO_SetPinMode(st_port, st_pin, LL_GPIO_MODE_INPUT);

    /* Step 2 - capture current pin state */
    const uint8_t pin_value = ((st_port->IDR & st_pin) != 0x00u) ? GPIO_PIN_SET : GPIO_PIN_RESET;

    /* Step 3 - set output state identical to the current input state */
    ...
    /* Step 4 - configure NSS pin as output */
    /* Step 4.1 - Configure pull resistors */
    uint32_t pin_pull = LL_GPIO_PULL_NO;
    switch (pull)
    {
        case SID_PAL_GPIO_PULL_NONE:
    ...
    /* Step 4.2 - set output type */
    uint32_t output_type;
    if (output_mode == SID_PAL_GPIO_OUTPUT_OPEN_DRAIN)
    {
        SID_PAL_GPIO_STM32_LOG_DEBUG("Set P%s%d output mode to Open Drain", NUM_TO_GPIO_PORT_STR(gpio_number), GET_GPIO_PIN_POSITION(NUM_TO_GPIO_PIN(gpio_number)));
        output_type = LL_GPIO_OUTPUT_OPENDRAIN;
    }
    ...
    /* Step 4.3 - set pin as output */
    SID_PAL_GPIO_STM32_LOG_DEBUG("Set P%s%d direction to Output", NUM_TO_GPIO_PORT_STR(gpio_number), GET_GPIO_PIN_POSITION(NUM_TO_GPIO_PIN(gpio_number)));
    LL_GPIO_SetPinSpeed(st_port, st_pin, LL_GPIO_SPEED_FREQ_HIGH);
    LL_GPIO_SetPinMode(st_port, st_pin, LL_GPIO_MODE_OUTPUT);

    /* Step 5 - re-eanble retention in Standby mode */
    return sid_pal_gpio_ext_ifc_standby_retention_enable(gpio_number);
}
#endif /* (CFG_LPM_LEVEL != 0) && (CFG_LPM_STDBY_SUPPORTED != 0) */
