...
static volatile int32_t  nesting_depth    = 0;
#if SID_PAL_CRITICAL_REGION_NMI_SIMULATION_EN
static          uint32_t basepri_on_entry = 0u;
#else
static          uint32_t primask_on_entry = 0u;
...
SID_STM32_SPEED_OPTIMIZED void sid_pal_enter_critical_region()
{
    ...
    /* 2. Store PRIMASK or BASEPRI on entry to the root critical section */
    if (0u == nesting_depth)
    {
#if SID_PAL_CRITICAL_REGION_NMI_SIMULATION_EN
        /**
    ...
    /* 3. Increment re-entrancy counter */
    nesting_depth++;
    SID_PAL_ASSERT(nesting_depth <= SID_PAL_CRITICAL_REGION_RECURSION_LIMIT); /* Check for systematic failures */
    __COMPILER_BARRIER(); /* Ensure that the above code will be completed before any further actions if the compiler decides to inline this function */
...
SID_STM32_SPEED_OPTIMIZED void sid_pal_exit_critical_region()
{
    ...
    /* 2. Safety Check */
    SID_PAL_ASSERT(nesting_depth > 0);

    /* 3. Decrement re-entrancy counter */
    nesting_depth--;
